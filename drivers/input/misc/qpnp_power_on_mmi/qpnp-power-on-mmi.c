...
static void kpd_bark_work_func(struct work_struct *work) {
  ...
  if (pon->kpd_hw_warmreset) {
    ...
    qpnp_pon_store_extra_reset_info(
        RESET_EXTRA_RESET_KUNPOW_REASON,
...
static int qpnp_pon_mmi_probe(struct platform_device *pdev) {
  ...
  pon = devm_kzalloc(&pdev->dev, sizeof(struct qpnp_pon_mmi), GFP_KERNEL);
  if (!pon)
    return -ENOMEM;
  ...
  if (pon->kpd_bark_irq < 0) {
    dev_err(&pon->pdev->dev, "Unable to get kpdpwr-bark irq\n");
    return -ENODEV;
  }

  dev_info(&pon->pdev->dev, "Request %d kpdpwr-bark  IRQ\n", pon->kpd_bark_irq);
  rc =
      devm_request_irq(&pon->pdev->dev, pon->kpd_bark_irq, qpnp_kpdpwr_bark_irq,
                       IRQF_TRIGGER_RISING, "qpnp_kpdpwr_bark", pon);
  if (rc < 0) {
