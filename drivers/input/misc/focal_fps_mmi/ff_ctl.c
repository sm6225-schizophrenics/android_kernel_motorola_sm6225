...
static int drm_check_dt(ff_ctl_context_t *g_context_t) {
  ...
  struct device_node *np =
      of_find_compatible_node(NULL, NULL, "focaltech,fingerprint");
...
static int ff_ctl_free_driver(void) {
  ...
  if (g_context->active_panel) {
    drm_panel_notifier_unregister(g_context->active_panel,
                                  &g_context->fb_notifier);
...
static int ff_ctl_init_driver(void) {
  ...
  do {
    ...
    err = request_irq(g_context->irq_num, ff_ctl_device_irq,
                      IRQF_TRIGGER_RISING | IRQF_ONESHOT, "ff_irq",
                      (void *)g_context);
    if (err) {
  ...
  if (g_context->active_panel) {
    ...
    err = drm_panel_notifier_register(g_context->active_panel,
                                      &g_context->fb_notifier);
    if (err)
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd,
                         unsigned long arg) {
  int err = 0;
  uint32_t level = 0;
...
//#else
  ...
  FF_LOGI("FocalTech fingerprint device control driver registered err=%d.",
          err);
  FF_LOGV("'%s' leave.", __func__);
  return err;
}
