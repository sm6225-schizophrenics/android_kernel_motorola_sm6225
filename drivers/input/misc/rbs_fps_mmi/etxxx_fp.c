 ...
 *Jacob.Kung		20201207		register platform driver with
 * MTK platform Jacob.Kung		20210331		add register FB
 * notifier Jacob.Kung		20210512		add register DRM
 * notifier
 * -----------------------------------------------------------
 *
 **/
...
#elif defined(CONFIG_DRM_PANEL_NOTIFICATIONS) &&                               \
...
static struct egis_key_map_t key_maps[] = {
    {EV_KEY, EGIS_NAV_INPUT_UP},         {EV_KEY, EGIS_NAV_INPUT_DOWN},
    {EV_KEY, EGIS_NAV_INPUT_LEFT},       {EV_KEY, EGIS_NAV_INPUT_RIGHT},
    {EV_KEY, EGIS_NAV_INPUT_CLICK},      {EV_KEY, EGIS_NAV_INPUT_DOUBLE_CLICK},
    {EV_KEY, EGIS_NAV_INPUT_LONG_PRESS}, {EV_KEY, EGIS_NAV_INPUT_FINGER_DOWN},
...
void egisfp_interrupt_timer_call(unsigned long _data) {
  struct egisfp_interrupt_desc_t *fps_int =
      (struct egisfp_interrupt_desc_t *)_data;
  struct egisfp_dev_t *egis_dev;
...
irqreturn_t fp_eint_func(int irq, void *dev_id) {
  ...
  if (!egis_dev->fps_ints.int_count)
    mod_timer(&egis_dev->fps_ints.timer,
              jiffies + msecs_to_jiffies(egis_dev->fps_ints.detect_period));
  egis_dev->fps_ints.int_count++;
...
int egisfp_interrupt_init(struct egisfp_dev_t *egis_dev, int int_mode,
  ...
  INFO_PRINT(" %s :  request_irq_done = %d gpio_irq = %d  pin = %d \n",
             __func__, egis_dev->request_irq_done, egis_dev->gpio_irq,
             egis_dev->irqPin);
  ...
  if (egis_dev->request_irq_done == 0) {
    ...
    if (int_mode == EDGE_TRIGGER_RISING) {
      ...
      err = request_irq(egis_dev->gpio_irq, fp_eint_func, IRQ_TYPE_EDGE_RISING,
                        "fp_detect-eint", egis_dev);
    } else if (int_mode == EDGE_TRIGGER_FALLING) {
      INFO_PRINT(" %s : EDGE_TRIGGER_FALLING \n", __func__);
      err = request_irq(egis_dev->gpio_irq, fp_eint_func, IRQ_TYPE_EDGE_FALLING,
                        "fp_detect-eint", egis_dev);
    } else if (int_mode == LEVEL_TRIGGER_LOW) {
      INFO_PRINT(" %s : LEVEL_TRIGGER_LOW \n", __func__);
      err = request_irq(egis_dev->gpio_irq, fp_eint_func_ll, IRQ_TYPE_LEVEL_LOW,
                        "fp_detect-eint", egis_dev);
    } else if (int_mode == LEVEL_TRIGGER_HIGH) {
      INFO_PRINT(" %s : LEVEL_TRIGGER_HIGH \n", __func__);
      err = request_irq(egis_dev->gpio_irq, fp_eint_func_ll,
                        IRQ_TYPE_LEVEL_HIGH, "fp_detect-eint", egis_dev);
    }
...
unsigned int egisfp_interrupt_poll(struct file *file,
...
int do_egisfp_reset(struct egisfp_dev_t *egis_dev) {
  ...
  if (ret)
    ...
    DEBUG_PRINT(" %s : reset_pin = %d \n", __func__,
                gpio_get_value(egis_dev->rstPin));
  return ret;
}
...
int do_egisfp_power_onoff(struct egisfp_dev_t *egis_dev,
                          struct egisfp_ioctl_cmd_t *ioctl_data) {
  int ret = 0;
#ifdef ET721_FOD
  ...
  if (egis_dev->pwr_by_gpio)
    DEBUG_PRINT(" %s : power_pin value = %d \n", __func__,
                gpio_get_value(egis_dev->vcc_33v_Pin));
  else
    DEBUG_PRINT(" %s : regulator enable = %d output voltage %d \n", __func__,
                regulator_is_enabled(egis_dev->vcc),
                regulator_get_voltage(egis_dev->vcc));
...
static void egis_get_io_stus(struct egisfp_dev_t *egis_dev) {
  ...
  INFO_PRINT(" %s : reset_pin value = %d irq_pin value = %d \n", __func__,
             gpio_get_value(egis_dev->rstPin),
             gpio_get_value(egis_dev->irqPin));
}
...
long egisfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  ...
  case INT_TRIGGER_INIT:
    ...
    retval = egisfp_interrupt_init(egis_dev, data.int_mode, data.detect_period,
  ...
  case FP_RESET_SET:
    ...
    retval = do_egisfp_reset_set(
        egis_dev,
        data.int_mode); // Use data.int_mode as reset setting. 1 = on, 0 = off.
    break;
  ...
  if (egis_dev != NULL) {
    if (!egis_dev->platforminit_done) {
      if (egis_dev->ctrl_power) {
        if (egis_dev->pwr_by_gpio) {
          ...
          if (regulator_count_voltages(egis_dev->vcc) > 0) {
            status = regulator_set_voltage(egis_dev->vcc,
                                           egis_dev->regulator_voltage_min,
                                           egis_dev->regulator_voltage_max);
            if (status) {
            ...
            status =
                regulator_set_load(egis_dev->vcc, egis_dev->regulator_current);
            if (status) {
      ...
      if (egis_dev->dd) {
        ...
        egis_dev->reset_high =
            pinctrl_lookup_state(egis_dev->pinctrl, "egis_rst_high");
        ...
        egis_dev->reset_low =
            pinctrl_lookup_state(egis_dev->pinctrl, "egis_rst_low");
        ...
        if (pinctrl_select_state(egis_dev->pinctrl, egis_dev->reset_low))
          goto egisfp_pinctrl_fail;
        egis_dev->irq_active =
            pinctrl_lookup_state(egis_dev->pinctrl, "egis_irq_active");
        ...
        if (egis_dev->pwr_by_gpio && egis_dev->ctrl_power) {
          egis_dev->vcc_high =
              pinctrl_lookup_state(egis_dev->pinctrl, "egis_vcc_high");
          ...
          egis_dev->vcc_low =
              pinctrl_lookup_state(egis_dev->pinctrl, "egis_vcc_low");
      ...
      if (egis_dev->pwr_by_gpio && egis_dev->ctrl_power)
        INFO_PRINT(" %s : successful status = %d gpio num -> vcc = %d rst = %d "
                   "Irq = %d \n",
                   __func__, status, egis_dev->vcc_33v_Pin, egis_dev->rstPin,
                   egis_dev->irqPin);
      else
        INFO_PRINT(
            " %s : successful status = %d gpio num -> rst = %d Irq = %d \n",
...
int egisfp_check_ioctl_permission(struct egisfp_dev_t *egis_dev,
                                  unsigned int cmd) {
...
static int egisfp_fb_callback(struct notifier_block *nb, unsigned long val,
                              void *data) {
  struct egisfp_dev_t *egis_dev = NULL;
  char *envp[2];
  int ret = 0;
...
int egisfp_parse_dt(struct egisfp_dev_t *egis_dev) {
  ...
  if (node) {
    ...
    if (egis_dev->ctrl_power) {
      ...
      if (egis_dev->pwr_by_gpio) {
        egis_dev->vcc_33v_Pin =
            of_get_named_gpio(node, "egistec,gpio_vcc_en", 0);
        ...
        ret = of_property_read_u32_array(node, "egis-fp,vcc-voltage",
                                         voltage_supply, 2);
        if (ret < 0) {
        ...
        ret = of_property_read_u32_array(node, "egis-fp,vcc-current",
                                         &current_supply, 1);
        if (ret < 0) {
...
const struct file_operations egisfp_fops = {.owner = THIS_MODULE,
...
int egisfp_remove(struct platform_device *pdev) {
  ...
  if (egis_dev->call_back_registered) {
    ...
    if (egis_dev->active_panel) {
      drm_panel_notifier_unregister(egis_dev->active_panel,
                                    &egis_dev->notifier);
...
int egisfp_probe(struct platform_device *pdev) {
  ...
  for (i = 0; i < ARRAY_SIZE(key_maps); i++)
    input_set_capability(egis_dev->input_dev, key_maps[i].type,
                         key_maps[i].code);
...
#else
  setup_timer(&egis_dev->fps_ints.timer, egisfp_interrupt_timer_call,
              (unsigned long)&egis_dev->fps_ints);
#endif
  ...
  if (egis_dev->active_panel) {
    ...
    status = drm_panel_notifier_register(egis_dev->active_panel,
                                         &egis_dev->notifier);
    if (status)
      ERROR_PRINT("  %s : drm_panel_notifier_register fail: %d ", __func__,
                  status);
    else
