 ...
 * functions body
 *****************************************************************************/
void sys_delay(int ms) { msleep(ms); }
...
void *fts_malloc(size_t size) { return kzalloc(size, GFP_KERNEL); }

void fts_free_proc(void *p) { return kfree(p); }
...
int short_get_adcdata_incell(u8 retval, u8 ch_num, int byte_num, int *adc_buf) {
  ...
  for (times = 0; times < FACTORY_TEST_RETRY; times++) {
    ...
    if ((ret >= 0) && (retval == short_state))
      ...
      FTS_TEST_DBG("reg%x=%x,retry:%d", FACTORY_REG_SHORT_TEST_STATE,
                   short_state, times);
...
bool get_fw_wp(u8 wp_ch_sel, enum wp_type water_proof_type) {
  ...
  case WATER_PROOF_OFF_RX:
    fw_wp_state =
        ((0x01 == (wp_ch_sel & 0x03)) || (0x02 == (wp_ch_sel & 0x03)));
    break;
  default:
    break;
  }
...
bool compare_mc_sc(bool tx_check, bool rx_check, int *data, int *min,
                   int *max) {
  int i = 0;
  bool result = true;
  struct fts_test *tdata = fts_ftest;
    ...
    for (i = tdata->sc_node.rx_num; i < tdata->sc_node.node_num; i++) {
      ...
      if ((data[i] < min[i]) || (data[i] > max[i])) {
        FTS_TEST_SAVE_INFO("test fail,tx%d=%5d,range=(%5d,%5d)\n",
                           i - tdata->sc_node.rx_num + 1, data[i], min[i],
                           max[i]);
        result = false;
      }
    }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
    snprintf(file_name, 128, "%s_%ld_%ld%s", prefix, (long)tdata->tv.tv_sec,
             ((long)tdata->tv.tv_nsec) / 1000, suffix);
#else
    snprintf(file_name, 128, "%s_%ld_%ld%s", prefix, (long)tdata->tv.tv_sec,
             (long)tdata->tv.tv_usec, suffix);
#endif
...
static int fts_test_malloc_free_data_txt(struct fts_test *tdata,
                                         bool allocate) {
#if TXT_SUPPORT
  if (true == allocate) {
...
#if defined(TEST_SAVE_FAIL_RESULT) && TEST_SAVE_FAIL_RESULT
  fts_test_save_fail_result(tdata, "testresult_fail", ".txt", tdata->testresult,
                            tdata->testresult_len);
#endif
...
static int fts_test_proc_open(struct inode *inode, struct file *file) {
  return single_open_size(file, fts_test_proc_show, PDE_DATA(inode),
                          PAGE_SIZE * 10);
}

static ssize_t fts_test_proc_write(struct file *filp, const char __user *buff,
...
static int fts_test_func_init(struct fts_ts_data *ts_data) {
  ...
  proc->proc_entry = proc_create_data(FTS_PROC_TP_DIFFER, 0444, NULL,
                                      &tp_test_proc_fops, ts_data);
