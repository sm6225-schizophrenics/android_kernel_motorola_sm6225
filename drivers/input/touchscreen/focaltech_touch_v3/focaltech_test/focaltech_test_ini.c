...
static int fts_atoi(char *nptr) { return (int)fts_atol(nptr); }
...
static int __maybe_unused fts_test_get_ini_default(struct ini_data *ini,
                                                   char *fwname) {
  int ret = 0;
  int inisize = 0;
...
static int ini_get_string_value(char *section_name, char *key_name,
                                char *rval) {
  ...
  for (i = 0; i < tx_num + 1; i++) {
    ...
    for (j = 0; j < divider_pos; j++) {
      if (',' == str[j]) {
        thr_pos = i * rx_num + k;
        if (thr_pos >= node_num) {
          FTS_TEST_ERROR("key:%s %d,dthr_num(%d>=%d) fail", key_name, i,
                         thr_pos, node_num);
          break;
        }
...
static int get_test_threshold_incell(void) {
  ...
  if (tdata->func->rawdata2_support) {
    fts_init_buffer(thr->rawdata2_max, thr->basic.rawdata2_max, node_num, false,
                    0, 0);
    fts_init_buffer(thr->rawdata2_min, thr->basic.rawdata2_min, node_num, false,
                    0, 0);
  }
...
static int get_test_threshold_mc_sc(void) {
  ...
  fts_init_buffer(thr->mcap_cmb_max, thr->basic.mcap_cmb_max, node_num, false,
                  0, 0);
  fts_init_buffer(thr->mcap_cmb_min, thr->basic.mcap_cmb_min, node_num, false,
                  0, 0);
  ...
  fts_init_buffer(thr->scap_cb_hi_max, thr->basic.scap_cb_hi_max, sc_num, false,
                  0, 0);
  fts_init_buffer(thr->scap_cb_hov_min, thr->basic.scap_cb_hov_min, sc_num,
                  false, 0, 0);
  fts_init_buffer(thr->scap_cb_hov_max, thr->basic.scap_cb_hov_max, sc_num,
                  false, 0, 0);
  ...
  fts_init_buffer(thr->scap_cb_off_cf_max, thr->basic.scap_cb_off_cf_max,
                  sc_num, false, 0, 0);
  fts_init_buffer(thr->scap_cb_on_cf_min, thr->basic.scap_cb_on_cf_min, sc_num,
                  false, 0, 0);
  fts_init_buffer(thr->scap_cb_on_cf_max, thr->basic.scap_cb_on_cf_max, sc_num,
                  false, 0, 0);
  fts_init_buffer(thr->scap_cb_hi_cf_min, thr->basic.scap_cb_hi_cf_min, sc_num,
                  false, 0, 0);
  fts_init_buffer(thr->scap_cb_hi_cf_max, thr->basic.scap_cb_hi_cf_max, sc_num,
                  false, 0, 0);
