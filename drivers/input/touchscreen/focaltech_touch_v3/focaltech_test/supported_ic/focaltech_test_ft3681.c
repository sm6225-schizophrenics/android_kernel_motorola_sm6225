...
static int scap_cb_cc(struct fts_test *tdata, int *scap_cb, bool *result) {
  ...
  FTS_TEST_FUNC_ENTER();
  if (!thr->scap_cb_on_min || !thr->scap_cb_on_max || !thr->scap_cb_off_min ||
      !thr->scap_cb_off_max || !thr->scap_cb_hi_min || !thr->scap_cb_hi_max ||
      ...
      if ((rx_check && (i < tdata->sc_node.rx_num)) ||
        ...
        if ((scb_tmp[i] < thr->scap_cb_on_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            scb_tmp[i], thr->scap_cb_on_min[i],
        ...
        if ((scb_cf[i] < thr->scap_cb_on_cf_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test2 fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            scb_cf[i], thr->scap_cb_on_cf_min[i],
    ...
    for (i = 0; i < tdata->sc_node.node_num; i++) {
      ...
      if ((rx_check && (i < tdata->sc_node.rx_num)) ||
        ...
        if ((scb_tmp[i] < thr->scap_cb_off_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            scb_tmp[i], thr->scap_cb_off_min[i],
        ...
        if ((scb_cf[i] < thr->scap_cb_off_cf_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test2 fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            scb_cf[i], thr->scap_cb_off_cf_min[i],
    ...
    for (i = 0; i < tdata->sc_node.node_num; i++) {
      ...
      if ((rx_check && (i < tdata->sc_node.rx_num)) ||
        ...
        if ((scb_tmp[i] < thr->scap_cb_hi_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            scb_tmp[i], thr->scap_cb_hi_min[i],
        ...
        if ((scb_cf[i] < thr->scap_cb_hi_cf_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test2 fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            scb_cf[i], thr->scap_cb_hi_cf_min[i],
...
static int scap_cb_ccbypass(struct fts_test *tdata, int *scap_cb,
                            bool *result) {
  int ret = 0;
  int i = 0;
  u8 wc_sel = 0;
...
static int ft5662_rawdata_test(struct fts_test *tdata, bool *test_result) {
  ...
  /* compare */
  result =
      compare_array(rawdata, thr->rawdata_h_min, thr->rawdata_h_max, false);
...
static int ft5662_uniformity_test(struct fts_test *tdata, bool *test_result) {
  ...
  if (thr->basic.uniformity_check_tx) {
    ...
    /* compare */
    result = compare_array(rl_tmp, thr->tx_linearity_min, thr->tx_linearity_max,
                           false);
    ...
    for (i = 0; i < tdata->sc_node.node_num; i++) {
      ...
      if ((rx_check && (i < tdata->sc_node.rx_num)) ||
        ...
        if ((srawdata_tmp[i] < thr->scap_rawdata_on_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            srawdata_tmp[i], thr->scap_rawdata_on_min[i],
    ...
    for (i = 0; i < tdata->sc_node.node_num; i++) {
      ...
      if ((rx_check && (i < tdata->sc_node.rx_num)) ||
        ...
        if ((srawdata_tmp[i] < thr->scap_rawdata_off_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            srawdata_tmp[i], thr->scap_rawdata_off_min[i],
    ...
    for (i = 0; i < tdata->sc_node.node_num; i++) {
      ...
      if ((rx_check && (i < tdata->sc_node.rx_num)) ||
        ...
        if ((srawdata_tmp[i] < thr->scap_rawdata_hi_min[i]) ||
          ...
          FTS_TEST_SAVE_ERR("test fail,CH%d=%5d,range=(%5d,%5d)\n", i + 1,
                            srawdata_tmp[i], thr->scap_rawdata_hi_min[i],
...
static void save_data_ft5662(char *buf, int *data_length) {
  ...
  if (true == test_item->rawdata_test) {
    cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                    "Rawdata Test", CODE_M_RAWDATA_TEST, tx, rx, line_num, 2);
    line_num += tx;
  }
  ...
  if (true == test_item->rawdata_uniformity_test) {
    if (thr->basic.uniformity_check_tx) {
      cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
                      "%s, %d, %d, %d, %d, %d, ", "Rawdata Uniformity Test",
                      CODE_M_RAWDATA_UNIFORMITY_TEST, tx, rx, line_num, 1);
      line_num += tx;
    }
    if (thr->basic.uniformity_check_rx) {
      cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
                      "%s, %d, %d, %d, %d, %d, ", "Rawdata Uniformity Test",
                      CODE_M_RAWDATA_UNIFORMITY_TEST, tx, rx, line_num, 2);
      line_num += tx;
    }
  }
  ...
  if (true == test_item->scap_cb_test) {
    ...
    if (tdata->csv_item_scb & 0x01) {
      cnt +=
          snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                   "SCAP CB Test", CODE_M_SCAP_CB_TEST, 2, sc_rx, line_num, 1);
      line_num += 2;
      cnt +=
          snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                   "SCAP CB Test", CODE_M_SCAP_CB_TEST, 2, tmp_rx, line_num, 2);
      line_num += 2;
    }
    ...
    if (tdata->csv_item_scb & 0x02) {
      cnt +=
          snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                   "SCAP CB Test", CODE_M_SCAP_CB_TEST, 2, sc_rx, line_num, 3);
      line_num += 2;
      cnt +=
          snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                   "SCAP CB Test", CODE_M_SCAP_CB_TEST, 2, tmp_rx, line_num, 4);
      line_num += 2;
    }
    ...
    if (tdata->csv_item_scb & 0x04) {
      cnt +=
          snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                   "SCAP CB Test", CODE_M_SCAP_CB_TEST, 2, sc_rx, line_num, 5);
      line_num += 2;
      cnt +=
          snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                   "SCAP CB Test", CODE_M_SCAP_CB_TEST, 2, tmp_rx, line_num, 6);
      line_num += 2;
    }
  }
  ...
  if (true == test_item->panel_differ_test) {
    cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                    "Panel Differ Test", CODE_M_PANELDIFFER_TEST, tx, rx,
                    line_num, 1);
    line_num += tx;
  }
  ...
  if (true == test_item->noise_test) {
    cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                    "Noise Test", CODE_M_NOISE_TEST, tx, rx, line_num, 1);
    line_num += tx;

    for (i = 1; thr->basic.noise_polling && (i < tdata->fre_num); i++) {
      cnt +=
          snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ",
                   "Noise Test", CODE_M_NOISE_TEST, tx, rx, line_num, 1 + i);
      line_num += tx;
    }
