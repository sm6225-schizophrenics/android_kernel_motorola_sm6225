...
static int fts_get_ic_information(struct fts_ts_data *ts_data) {
  ...
  do {
    ...
    if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
      FTS_DEBUG("chip id read invalid, read:0x%02x%02x", chip_id[0],
                chip_id[1]);
    } else {
  ...
  case STYLUS_DEFAULT:
    if (pevt->tip && pevt->p) {
      if ((ts_data->log_level >= 2) || (!pevt->down))
        FTS_DEBUG("[PEN]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d DOWN", pevt->x,
                  pevt->y, pevt->p, pevt->tip, pevt->flag, pevt->tilt_x,
                  pevt->tilt_y);
      ...
      FTS_DEBUG("[PEN]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d UP", pevt->x,
                pevt->y, pevt->p, pevt->tip, pevt->flag, pevt->tilt_x,
                pevt->tilt_y);
      input_report_abs(pen_dev, ABS_X, pevt->x);
      input_report_abs(pen_dev, ABS_Y, pevt->y);
  ...
  case STYLUS_HOVER:
    if (ts_data->log_level >= 1)
      FTS_DEBUG("[PEN][%02X]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d,%d",
                pen_buf[2], pevt->x, pevt->y, pevt->p, pevt->tip, pevt->flag,
                pevt->tilt_x, pevt->tilt_y, pevt->azimuth);
    input_report_abs(pen_dev, ABS_X, pevt->x);
    input_report_abs(pen_dev, ABS_Y, pevt->y);
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data,
                                    u8 *touch_buf) {
  int ret = 0;
  u8 gesture_en = 0xFF;
#ifdef FOCALTECH_PALM_SENSOR_EN
  ...
  case TOUCH_EVENT_NUM:
    ...
    for (i = 0; i < event_num; i++) {
      ...
      if (pointid >= max_touch_num) {
        FTS_ERROR("touch point ID(%d) beyond max_touch_number(%d)", pointid,
                  max_touch_num);
        return -EINVAL;
      }
...
static int fts_palm_sensor_init(struct fts_ts_data *data) {
#ifdef CONFIG_HAS_WAKELOCK
  wake_lock_init(&data->palm_gesture_wakelock, WAKE_LOCK_SUSPEND,
                 "palm_detect_wl");
#else
  ...
  timer_setup(&data->palm_release_fimer, fts_palm_sensor_release_timer_handler,
              0);
...
static int fts_parse_dt(struct device *dev,
  ...
  if (pdata->have_key) {
    ...
    ret = of_property_read_u32_array(np, "focaltech,keys", pdata->keys,
                                     pdata->key_number);
    if (ret < 0)
...
#ifdef CONFIG_FTS_VDD_GPIO_CONTROL
  pdata->vdd_gpio = of_get_named_gpio_flags(np, "focaltech,vdd-gpio", 0,
  ...
  /* reset, irq gpio info */
  pdata->reset_gpio = of_get_named_gpio_flags(np, "focaltech,reset-gpio", 0,
  ...
  pdata->edge_ctrl = of_property_read_bool(np, "focaltech,edge-ctrl");
  if (pdata->edge_ctrl)
    FTS_INFO("support focaltech edge mode");

  pdata->interpolation_ctrl =
      of_property_read_bool(np, "focaltech,interpolation-ctrl");
  if (pdata->interpolation_ctrl)
    FTS_INFO("support focaltech interpolation mode");

  pdata->report_rate_ctrl =
      of_property_read_bool(np, "focaltech,report_rate-ctrl");
  if (pdata->report_rate_ctrl)
    FTS_INFO("support focaltech report rate switch mode");

  pdata->sample_ctrl = of_property_read_bool(np, "focaltech,sample-ctrl");
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data) {
  ...
  if (ts_data->spi->dev.of_node &&
      !mmi_device_is_available(ts_data->spi->dev.of_node)) {
...
static void __exit fts_ts_exit(void) { spi_unregister_driver(&fts_ts_driver); }
