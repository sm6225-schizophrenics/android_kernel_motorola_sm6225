...
static bool fts_fwupg_check_flash_status(u16 flash_status, int retries,
  ...
  FTS_ERROR("flash status fail,ok:%04x,read:%04x,retries:%d", flash_status,
            read_status, i);
  return false;
}
...
int fts_fwupg_erase(u32 delay) {
  ...
  flag =
      fts_fwupg_check_flash_status(FTS_CMD_FLASH_STATUS_ERASE_OK,
...
int fts_flash_read_buf(u32 saddr, u8 *buf, u32 len) {
  ...
  for (i = 0; i < packet_number; i++) {
    ...
    if (upg->ts_data->bus_type == BUS_TYPE_I2C) {
      ...
      wbuf[4] = BYTE_OFF_8(packet_len);
      wbuf[5] = BYTE_OFF_0(packet_len);
      ret = fts_read(wbuf, FTS_CMD_READ_LEN_SPI, buf + offset, packet_len);
      if (ret < 0) {
...
static int __maybe_unused fts_read_file_default(char *file_name,
                                                u8 **file_buf) {
  int ret = 0;
...
int fts_enter_test_environment(bool test_state) { return 0; }
#if FTS_AUTO_LIC_UPGRADE_EN
...
static bool fts_lic_need_upgrade(struct fts_upgrade *upg) {
  ...
  FTS_DEBUG("lcd initial code version in tp:%x, host:%x", initcode_ver_in_tp,
...
static int fts_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver) {
  ...
  if (upg->fw_length < upg->func->paramcfgveroff) {
    FTS_ERROR("fw len(%x) < paramcfg ver offset(%x)", upg->fw_length,
...
static int fts_param_ide_in_host(struct fts_upgrade *upg) {
  ...
  if (upg->fw_length < upg->func->paramcfgoff + FTS_FW_IDE_SIG_LEN) {
    FTS_INFO("fw len(%x) < paramcfg offset(%x), no IDE", upg->fw_length,
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg) {
  ...
  snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin", FTS_FW_NAME_PREX_WITH_REQUEST,
