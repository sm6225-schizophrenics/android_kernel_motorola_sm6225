...
int fts_write(u8 *writebuf, u32 writelen) {
  ...
  for (i = 0; i < SPI_RETRY_NUMBER; i++) {
    ...
    if ((0 == ret) && ((rxbuf[3] & 0xA0) == 0)) {
      ...
      FTS_DEBUG("data write(addr:%x),status:%x,retry:%d,ret:%d", writebuf[0],
                rxbuf[3], i, ret);
      ret = -EIO;
  ...
  if (ret < 0) {
    FTS_ERROR("data write(addr:%x) fail,status:%x,ret:%d", writebuf[0],
              rxbuf[3], ret);
  }
...
int fts_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen) {
  ...
  for (i = 0; i < SPI_RETRY_NUMBER; i++) {
    ...
    if ((0 == ret) && ((rxbuf[3] & 0xA0) == 0)) {
      ...
      FTS_DEBUG("data read(addr:%x) status:%x,retry:%d,ret:%d", cmd[0],
                rxbuf[3], i, ret);
      ret = -EIO;
      udelay(CS_HIGH_DELAY);
    }
  ...
  if (ret < 0) {
    FTS_ERROR("data read(addr:%x) %s,status:%x,ret:%d", cmd[0],
              (i >= SPI_RETRY_NUMBER) ? "crc abnormal" : "fail", rxbuf[3], ret);
  }
...
int fts_read_reg(u8 addr, u8 *value) { return fts_read(&addr, 1, value, 1); }
...
int fts_spi_transfer_direct(u8 *writebuf, u32 writelen, u8 *readbuf,
  ...
  ret = fts_spi_transfer(txbuf, rxbuf, txlen);
  if (ret < 0) {
    FTS_ERROR("data read(addr:%x) fail,status:%x,ret:%d", txbuf[0], rxbuf[3],
              ret);
    goto err_spi_dir;
  }
