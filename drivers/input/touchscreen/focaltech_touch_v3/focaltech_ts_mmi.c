...
// extern int ts_mmi_dev_register(struct device *parent, struct ts_mmi_methods
// *mdata); extern void ts_mmi_dev_unregister(struct device *parent);
#endif
...
static ssize_t fts_edge_store(struct device *dev, struct device_attribute *attr,
                              const char *buf, size_t size);
static ssize_t fts_edge_show(struct device *dev, struct device_attribute *attr,
                             char *buf);
...
static DEVICE_ATTR(edge, (S_IRUGO | S_IWUSR | S_IWGRP), fts_edge_show,
                   fts_edge_store);
static DEVICE_ATTR(interpolation, (S_IRUGO | S_IWUSR | S_IWGRP),
                   fts_interpolation_show, fts_interpolation_store);
static DEVICE_ATTR(sample, (S_IRUGO | S_IWUSR | S_IWGRP), fts_sample_show,
                   fts_sample_store);
...
int fts_set_edge_mode(struct fts_mode_info mode) {
  ...
  FTS_INFO("Write edge value (%02x %02x)", reg_value_8c, reg_value_8d);
  return 0;
}
...
static ssize_t fts_edge_store(struct device *dev, struct device_attribute *attr,
  ...
  if (!memcmp(ts_data->set_mode.edge_mode, edge_cmd, sizeof(edge_cmd))) {
    FTS_INFO("The value (%02x %02x) is same,so not write.", edge_cmd[0],
             edge_cmd[1]);
    ret = size;
    goto exit;
  }
  ...
  msleep(20);
  ret = size;
  FTS_INFO("Success to set edge = %02x, rotation = %02x", edge_cmd[1],
           edge_cmd[0]);
...
static ssize_t fts_edge_show(struct device *dev, struct device_attribute *attr,
  ...
  return scnprintf(buf, PAGE_SIZE, "0x%02x 0x%02x",
                   ts_data->set_mode.edge_mode[1],
                   ts_data->set_mode.edge_mode[0]);
}
...
static int fts_mmi_panel_state(struct device *dev, enum ts_mmi_pm_mode from,
...
exit:
  ...
  if (pdata->interpolation_ctrl && ts_data->get_mode.interpolation) {
    ret = fts_write_reg(FTS_CMD_REPORT_RATE_ADDR,
                        ts_data->get_mode.report_rate_mode);
    if (ret >= 0) {
      ...
      FTS_INFO(
          ...
          ts_data->get_mode.report_rate_mode == REPORT_RATE_CMD_240HZ
              ? "REPORT_RATE_240HZ"
              : (ts_data->get_mode.report_rate_mode == REPORT_RATE_CMD_360HZ
                     ? "REPORT_RATE_360HZ"
                     : "Unsupported"));
    }
  }
...
static ssize_t fts_mmi_pocket_mode_store(struct device *dev,
  ...
  if (ts_data->set_mode.pocket_mode == ts_data->get_mode.pocket_mode) {
    FTS_INFO("The value = %d is same, so not to write",
             ts_data->get_mode.pocket_mode);
    goto exit;
  }
  ...
  FTS_INFO("Success set %d to pocket mode, 0x8B = 0x%02x\n",
           ts_data->set_mode.pocket_mode, read_data);
exit:
  mutex_unlock(&ts_data->mode_lock);
  FTS_FUNC_EXIT();
  return size;
...
static int fts_mmi_extend_attribute_group(struct device *dev,
