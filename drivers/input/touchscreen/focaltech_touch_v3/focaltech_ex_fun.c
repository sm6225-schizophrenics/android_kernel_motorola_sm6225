...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
  proc->proc_entry =
      proc_create_data(PROC_NAME, 0777, NULL, &fts_proc_fops, ts_data);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
  ts_data->proc_ta.proc_entry =
      proc_create_data("fts_ta", 0777, NULL, &fts_procta_fops, ts_data);
...
static ssize_t fts_tprwreg_show(struct device *dev,
  ...
  if (rw_op.len < 0) {
    ...
    if (RWREG_OP_READ == rw_op.type) {
      if (rw_op.res == 0) {
        count =
            snprintf(buf, PAGE_SIZE, "Read %02X: %02X\n", rw_op.reg, rw_op.val);
      } else {
    ...
    if (RWREG_OP_READ == rw_op.type) {
      ...
      if (rw_op.res) {
        count +=
            snprintf(buf + count, PAGE_SIZE, "failed, ret: %d\n", rw_op.res);
      } else {
        if (rw_op.opbuf) {
...
static ssize_t fts_driverinfo_show(struct device *dev,
  ...
  count +=
      snprintf(buf + count, PAGE_SIZE, "IC ID:0x%02x%02x\n",
...
static ssize_t fts_log_level_show(struct device *dev,
  ...
  mutex_lock(&input_dev->mutex);
  count +=
      snprintf(buf + count, PAGE_SIZE, "log level:%d\n", ts_data->log_level);
  mutex_unlock(&input_dev->mutex);
...
static ssize_t fts_touchsize_show(struct device *dev,
  ...
  mutex_lock(&input_dev->mutex);
  count +=
      snprintf(buf + count, PAGE_SIZE, "touch size:%d\n", ts_data->touch_size);
  mutex_unlock(&input_dev->mutex);
 ...
 * data, hex] rw-flag: 0, write; 1, read read:  echo 10005           > rw_reg
 * ---read reg 0x00-0x05 write: echo 000050102030405 > rw_reg ---write reg
 * 0x00-0x05 as 01,02,03,04,05 Get result: cat rw_reg
 */
