...
/* entity code */ /* bits */
    ...
    8,
#endif
    TOUCH_REPORT_NUM_OF_ACTIVE_OBJECTS,
    8,
    TOUCH_REPORT_FOREACH_ACTIVE_OBJECT,
    TOUCH_REPORT_OBJECT_N_INDEX,
    8,
    TOUCH_REPORT_OBJECT_N_CLASSIFICATION,
    8,
    TOUCH_REPORT_OBJECT_N_X_POSITION,
    16,
    TOUCH_REPORT_OBJECT_N_Y_POSITION,
    16,
...
static void syna_dev_helper_work(struct work_struct *work) {
  ...
  struct syna_tcm *tcm = container_of(helper, struct syna_tcm, helper);
...
static int syna_dev_enable_lowpwr_gesture(struct syna_tcm *tcm, bool en) {
  ...
  if (en) {
    ...
    retval = syna_tcm_set_dynamic_config(
        tcm->tcm_dev, DC_ENABLE_WAKEUP_GESTURE_MODE, 1, RESP_IN_ATTN);
    if (retval < 0) {
     ...
     */
    retval = syna_tcm_set_dynamic_config(
        tcm->tcm_dev, DC_ENABLE_WAKEUP_GESTURE_MODE, 0, RESP_IN_ATTN);
    if (retval < 0) {
...
static void syna_dev_report_input_events(struct syna_tcm *tcm) {
  ...
  for (idx = 0; idx < max_objects; idx++) {
    if (tcm->prev_obj_status[idx] == LIFT && object_data[idx].status == LIFT)
      status = NOP;
    else
...
static irqreturn_t syna_dev_isr(int irq, void *data) {
  ...
  if (tcm->report_to_queue[code] == EFP_ENABLE) {
    ...
    syna_cdev_update_report_queue(tcm, code, &tcm->tcm_dev->external_buf);
  ...
  if (code == REPORT_TOUCH) {
    ...
    retval =
        syna_tcm_parse_touch_report(tcm->tcm_dev, tcm->event_data.buf,
                                    tcm->event_data.data_length, &tcm->tp_data);
    if (retval < 0) {
...
#ifdef DEV_MANAGED_API
  retval =
      devm_request_threaded_irq(dev, attn->irq_id, NULL, syna_dev_isr,
                                attn->irq_flags, PLATFORM_DRIVER_NAME, tcm);
#else /* Legacy API */
  retval = request_threaded_irq(attn->irq_id, NULL, syna_dev_isr,
                                attn->irq_flags, PLATFORM_DRIVER_NAME, tcm);
#endif
  if (retval < 0) {
...
static int syna_dev_set_up_app_fw(struct syna_tcm *tcm) {
...
#ifdef USE_CUSTOM_TOUCH_REPORT_CONFIG
  retval = syna_tcm_set_touch_report_config(
      tcm_dev, custom_touch_format, (unsigned int)sizeof(custom_touch_format));
  if (retval < 0) {
  ...
  retval = syna_tcm_set_custom_touch_entity_callback(
      tcm_dev, syna_dev_parse_custom_touch_data_cb, (void *)tcm);
  if (retval < 0) {
...
int syna_reflash_do_reflash(struct syna_tcm *tcm, char *name) {
  ...
  /* get firmware image */
  retval = request_firmware(&fw_entry, fwname, tcm->pdev->dev.parent);
  if (retval < 0) {
  ...
  retval = syna_tcm_romboot_do_multichip_reflash(
      tcm_dev, fw_image, fw_image_size, RESP_IN_ATTN, false);
#else
  /* do firmware update for the common device */
  retval = syna_tcm_do_fw_update(tcm_dev, fw_image, fw_image_size, RESP_IN_ATTN,
                                 false);
#endif
  if (retval < 0) {
...
struct void syna_dev_reflash_startup_work(struct work_struct *work) {
  ...
  if (reflash_data->fw_name[0] != 0) {
    ...
    /* get firmware image */
    retval = request_firmware(&fw_entry, reflash_data->fw_name,
                              tcm->pdev->dev.parent);
    if (retval < 0) {
      LOGE("Fail to request %s\n", reflash_data->fw_name);
...
static int syna_dev_fb_notifier_cb(struct notifier_block *nb,
  ...
  if (action == disp_early_event_blank && transition == disp_blank_powerdown) {
...
static int syna_dev_connect(struct syna_tcm *tcm) {
  ...
  LOGI("Configuration: name(%s), lpwg(%s), hw_reset(%s), irq_ctrl(%s)\n",
       PLATFORM_DRIVER_NAME, (tcm->lpwg_enabled) ? "yes" : "no",
...
static int syna_dev_probe(struct platform_device *pdev) {
...
#ifdef CONFIG_HAS_WAKELOCK
    wake_lock_init(&gesture_wakelock, WAKE_LOCK_SUSPEND,
  ...
  if (active_panel) {
    ...
    retval = drm_panel_notifier_register(active_panel, &tcm->fb_notifier);
    if (retval < 0) {
  ...
  tcm->helper.workqueue = create_singlethread_workqueue("synaptics_tcm_helper");
  INIT_WORK(&tcm->helper.work, syna_dev_helper_work);
  /* set up custom touch data parsing method */
  syna_tcm_set_reset_occurrence_callback(tcm_dev, syna_dev_reset_detected_cb,
                                         (void *)tcm);
#endif
...
#if defined(USE_DRM_PANEL_NOTIFIER)
  if (active_panel)
    drm_panel_notifier_unregister(active_panel, &tcm->fb_notifier);
#else
