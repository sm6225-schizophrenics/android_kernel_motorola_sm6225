...
static int syna_tcm_v2_set_max_read_size(struct tcm_dev *tcm_dev) {
  ...
  retval = syna_tcm_v2_execute_cmd_request(
      tcm_dev, CMD_TCM2_SET_MAX_READ_LENGTH, data, sizeof(data));
  if (retval < 0) {
...
static void syna_tcm_v2_dispatch_report(struct tcm_dev *tcm_dev) {
  ...
  if (tcm_msg->report_code == REPORT_IDENTIFY) {
    ...
    retval = syna_tcm_v2_parse_idinfo(
...
static void syna_tcm_v2_dispatch_response(struct tcm_dev *tcm_dev) {
  ...
  retval = syna_tcm_buf_alloc(&tcm_dev->resp_buf, resp_data_length + 1);
  if (retval < 0) {
...
static int syna_tcm_v2_write(struct tcm_dev *tcm_dev, unsigned char command,
  ...
  /* copy payload, if any */
  if (payload_len) {
    retval = syna_pal_mem_cpy(&tcm_msg->out.buf[MESSAGE_HEADER_SIZE],
  ...
  retval = syna_tcm_write(tcm_dev, tcm_msg->out.buf, xfer_len);
  if (retval < 0) {
...
static int syna_tcm_v2_continued_read(struct tcm_dev *tcm_dev,
  ...
  retry:
    ...
    retval = syna_tcm_v2_write(tcm_dev, command, NULL, 0, false);
    if (retval < 0) {
    ...
    retval = syna_tcm_v2_read(tcm_dev, xfer_length, &tmp_buf, &tmp_buf_size);
    if (retval < 0) {
    ...
    retval = syna_pal_mem_cpy(&tcm_msg->in.buf[offset],
                              ...
                              tmp_buf_size - MESSAGE_HEADER_SIZE, xfer_length);
    if (retval < 0) {
...
static int syna_tcm_v2_get_response(struct tcm_dev *tcm_dev) {
  ...
  retval = syna_tcm_v2_read(tcm_dev, 0, &tmp_buf, &tmp_buf_size);
  if (retval < 0) {
  ...
  retval = syna_pal_mem_cpy(tcm_msg->in.buf, tcm_msg->in.buf_size, tmp_buf,
  ...
  if (tcm_msg->payload_length > 0) {
    ...
    retval = syna_tcm_v2_continued_read(tcm_dev, tcm_msg->payload_length);
    if (retval < 0) {
...
static inline int syna_tcm_v2_send_cmd(struct tcm_dev *tcm_dev,
  ...
  retval = syna_tcm_v2_write(tcm_dev, command, payload, length, resend);
  if (retval < 0)
...
static int syna_tcm_v2_execute_cmd_request(struct tcm_dev *tcm_dev,
  ...
  retry:
    ...
    retval = syna_tcm_v2_send_cmd(tcm_dev, command, &payload[offset],
                                  xfer_length, (retry_cnt > 0));
    if (retval < 0)
      goto exit;
...
static int syna_tcm_v2_read_message(struct tcm_dev *tcm_dev,
  ...
  /* request a command */
  retval =
      syna_tcm_v2_execute_cmd_request(tcm_dev, CMD_TCM2_GET_REPORT, NULL, 0);
  if (retval < 0) {
...
static int syna_tcm_v2_write_message(struct tcm_dev *tcm_dev,
  ...
  retval = syna_tcm_v2_execute_cmd_request(tcm_dev, command, payload,
                                           payload_length);
  if (retval < 0) {
...
int syna_tcm_v2_detect(struct tcm_dev *tcm_dev, unsigned char *data,
  ...
  retval = syna_tcm_v2_write_message(tcm_dev, CMD_IDENTIFY, NULL, 0, &resp_code,
                                     RESP_IN_POLLING);
  if (retval < 0) {
