 ...
 */
static int syna_tcm_romboot_send_command(
    struct tcm_dev *tcm_dev, unsigned char *out, unsigned int out_size,
    unsigned char *in, unsigned int in_size, unsigned int delay_ms_resp) {
  int retval;
  ...
  if (out_size < sizeof(struct flash_param)) {
    LOGE("Invalid size of out data, %d, min. size:%d\n", out_size,
         (int)sizeof(struct flash_param));
    return _EINVAL;
  }

  retval =
      tcm_dev->write_message(tcm_dev, CMD_SPI_MASTER_WRITE_THEN_READ_EXTENDED,
                             out, out_size, &resp_code, delay_ms_resp);
  if (retval < 0) {
  ...
  retval = syna_pal_mem_cpy((unsigned char *)in, in_size, tcm_dev->resp_buf.buf,
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_romboot_multichip_send_command(
    struct tcm_dev *tcm_dev, unsigned char command, unsigned char *out,
  ...
  if (flash_param.command != 0x00) {
    ...
    if (out_size > 0) {
      retval = syna_pal_mem_cpy(&payld_buf[offset + 2], payld_size - offset - 2,
    ...
    LOGD("Packet: %02x %02x %02x %02x %02x %02x %02x %02x\n", flash_param.byte0,
         flash_param.byte1, flash_param.byte2, flash_param.read_size[0],
         flash_param.read_size[1], flash_param.command, payld_buf[offset],
         payld_buf[offset + 1]);
  }

  retval = syna_pal_mem_cpy(payld_buf, payld_size, &flash_param,
                            sizeof(flash_param), sizeof(flash_param));
  if (retval < 0) {
    LOGE("Fail to copy flash_param header to payld_buf\n");
    goto exit;
  }

  retval = syna_tcm_romboot_send_command(tcm_dev, payld_buf, payld_size, in,
                                         in_size, delay_ms);
  if (retval < 0) {
...
static int syna_tcm_romboot_multichip_get_resp(struct tcm_dev *tcm_dev,
  ...
  retval = syna_tcm_romboot_multichip_send_command(tcm_dev, CMD_NONE, NULL, 0,
  ...
  if (resp) {
    retval =
        syna_pal_mem_cpy(resp, resp_size, &tmp_buf[1], xfer_len - 1, length);
    if (retval < 0) {
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_romboot_multichip_write_flash(
    struct tcm_dev *tcm_dev, struct tcm_romboot_data_blob *romboot_data,
    unsigned int address, const unsigned char *wr_data, unsigned int wr_len,
    unsigned int wr_delay_ms) {
  int retval;
  unsigned int offset;
  ...
  while (remaining_length) {
    ...
    retval = syna_tcm_buf_alloc(&romboot_data->out, xfer_length + 2);
    if (retval < 0) {
    ...
    retval = syna_pal_mem_cpy(&romboot_data->out.buf[2],
                              romboot_data->out.buf_size - 2, &wr_data[offset],
    ...
    retval = syna_tcm_romboot_multichip_send_command(
        tcm_dev, CMD_WRITE_FLASH, romboot_data->out.buf, xfer_length + 2, NULL,
        0, delay_ms);
    if (retval < 0) {
    ...
    retval = syna_tcm_romboot_multichip_get_status(
        tcm_dev, &resp_code, &resp_length, ROMBOOT_DELAY_MS);
    if (retval < 0) {
    ...
    retval = syna_tcm_romboot_multichip_get_resp(tcm_dev, resp_length, NULL, 0,
                                                 ROMBOOT_DELAY_MS);
    if (retval < 0) {
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_romboot_multichip_erase_flash(
    struct tcm_dev *tcm_dev, struct tcm_romboot_data_blob *romboot_data,
    unsigned int address, unsigned int size, unsigned int erase_delay_ms) {
  int retval;
  ...
  LOGD("Page start = %d (0x%04x), Page count = %d (0x%04x)\n", page_start,
       page_start, page_count, page_count);
  ...
  retval = syna_tcm_romboot_multichip_send_command(tcm_dev, CMD_ERASE_FLASH,
                                                   out_buf, size_erase_cmd,
                                                   NULL, 0, erase_delay_ms);
  if (retval < 0) {
 ...
 */
static int
syna_tcm_romboot_multichip_get_boot_info(struct tcm_dev *tcm_dev,
  ...
  retval = syna_tcm_romboot_multichip_send_command(
      tcm_dev, CMD_GET_BOOT_INFO, NULL, 0, NULL, 0, ROMBOOT_DELAY_MS);
  if (retval < 0) {
  ...
  retval = syna_tcm_romboot_multichip_get_status(
      tcm_dev, &resp_code, &resp_data_len, ROMBOOT_DELAY_MS);
  if (retval < 0) {
    LOGE("Fail to get the response of command 0x%x\n", CMD_GET_BOOT_INFO);
    return retval;
  }
  ...
  retval = syna_tcm_romboot_multichip_get_resp(
      tcm_dev, copy_size, (unsigned char *)boot_info,
 ...
 */
static int
syna_tcm_romboot_preparation(struct tcm_dev *tcm_dev,
  ...
  if (IS_APP_FW_MODE(tcm_dev->dev_mode)) {
    ...
    if (is_multichip)
      retval = syna_tcm_switch_fw_mode(tcm_dev, MODE_MULTICHIP_TDDI_BOOTLOADER,
                                       FW_MODE_SWITCH_DELAY_MS);
    else
      retval = syna_tcm_switch_fw_mode(tcm_dev, MODE_TDDI_BOOTLOADER,
                                       FW_MODE_SWITCH_DELAY_MS);
  ...
  retval = syna_tcm_romboot_multichip_get_boot_info(tcm_dev, boot_info);
  if (retval < 0) {
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_romboot_jedec_send_command(
    struct tcm_dev *tcm_dev, unsigned char flash_command, unsigned char *out,
    unsigned int out_size, unsigned char *in, unsigned int in_size,
    unsigned int delay_ms) {
  int retval;
  unsigned char *payld_buf = NULL;
  ...
  LOGD("Flash command: 0x%02x, total size: %d, wr: %d, rd: %d\n", flash_command,
       payld_size, out_size, in_size);
  LOGD("Packet: %02x %02x %02x %02x %02x %02x\n", flash_param.byte0,
       flash_param.byte1, flash_param.byte2, flash_param.read_size[0],
       flash_param.read_size[1], flash_param.command);
  ...
  if (out && (out_size > 0)) {
    retval = syna_pal_mem_cpy(payld_buf + sizeof(flash_param),
                              payld_size - sizeof(flash_param), out, out_size,
                              out_size);
    if (retval < 0) {
...
static int syna_tcm_romboot_jedec_get_status(struct tcm_dev *tcm_dev,
  ...
  for (idx = 0; idx < STATUS_CHECK_RETRY; idx++) {
    retval = syna_tcm_romboot_jedec_send_command(
        tcm_dev, JEDEC_READ_STATUS, NULL, 0, &status, sizeof(status), delay_ms);
    if (retval < 0) {
      LOGE("Failed to write JEDEC_READ_STATUS\n");
      return retval;
    }

    syna_pal_sleep_us(JEDEC_STATUS_CHECK_US_MIN, JEDEC_STATUS_CHECK_US_MAX);
...
static int syna_tcm_romboot_jedec_erase_flash(struct tcm_dev *tcm_dev,
  ...
  retval = syna_tcm_romboot_jedec_send_command(tcm_dev, JEDEC_WRITE_ENABLE,
                                               NULL, 0, NULL, 0, delay_ms);
  if (retval < 0) {
    LOGE("Failed to write JEDEC_WRITE_ENABLE\n");
    return retval;
  }

  retval = syna_tcm_romboot_jedec_send_command(tcm_dev, JEDEC_CHIP_ERASE, NULL,
                                               0, NULL, 0, delay_ms);
  if (retval < 0) {
    LOGE("Failed to write JEDEC_WRITE_ENABLE\n");
    return retval;
...
static int syna_tcm_romboot_jedec_write_flash(struct tcm_dev *tcm_dev,
  ...
  while (remaining_length) {
    ...
    retval = syna_pal_mem_cpy(&buf[3], sizeof(buf) - 3, &data[offset],
                              data_size - offset, xfer_length);
    if (retval < 0) {
    ...
    retval = syna_tcm_romboot_jedec_send_command(
        tcm_dev, JEDEC_PAGE_PROGRAM, buf, sizeof(buf), NULL, 0, delay_ms);
    if (retval < 0) {
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_romboot_erase_flash(
    struct tcm_dev *tcm_dev, struct tcm_romboot_data_blob *romboot_data,
    struct block_data *blk, unsigned int delay_ms, bool is_multichip) {
  int retval;
  ...
  if (is_multichip)
    retval = syna_tcm_romboot_multichip_erase_flash(
        tcm_dev, romboot_data, blk->flash_addr, blk->size, delay_ms);
  else
    retval = syna_tcm_romboot_jedec_erase_flash(tcm_dev, delay_ms);
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_romboot_write_flash(
    struct tcm_dev *tcm_dev, struct tcm_romboot_data_blob *romboot_data,
    struct block_data *blk, unsigned int delay_ms, bool is_multichip) {
  int retval;
  ...
  if (is_multichip)
    retval = syna_tcm_romboot_multichip_write_flash(
        tcm_dev, romboot_data, blk->flash_addr, blk->data, blk->size, delay_ms);
  else
    retval = syna_tcm_romboot_jedec_write_flash(tcm_dev, blk->flash_addr,
                                                blk->data, blk->size, delay_ms);
...
int syna_tcm_romboot_do_ihex_update(struct tcm_dev *tcm_dev,
  ...
  syna_pal_mem_set(&romboot_data.ihex_info, 0x00, sizeof(struct ihex_info));

  ihex_info = &romboot_data.ihex_info;

  ihex_info->bin = syna_pal_mem_alloc(flash_size, sizeof(unsigned char));
  if (!ihex_info->bin) {
  ...
  retval = syna_tcm_romboot_preparation(tcm_dev, &romboot_data, is_multichip);
  if (retval < 0) {
  ...
  for (idx = 0; idx < IHEX_MAX_BLOCKS; idx++) {
    ...
    retval = syna_tcm_romboot_erase_flash(tcm_dev, &romboot_data, block,
                                          erase_delay_ms, is_multichip);
    if (retval < 0) {
  ...
  for (idx = 0; idx < IHEX_MAX_BLOCKS; idx++) {
    ...
    retval = syna_tcm_romboot_write_flash(tcm_dev, &romboot_data, block,
                                          wr_delay_ms, is_multichip);
    if (retval < 0) {
...
int syna_tcm_romboot_do_multichip_reflash(struct tcm_dev *tcm_dev,
  ...
  syna_pal_mem_set(&romboot_data, 0x00, sizeof(struct tcm_romboot_data_blob));
  ...
  for (idx = 0; idx < AREA_MAX; idx++) {
    ...
    LOGD("Erase %s block - address: 0x%x (%d), size: %d\n",
         AREA_ID_STR(block->id), block->flash_addr, block->flash_addr,
         block->size);
...
int syna_tcm_get_romboot_info(struct tcm_dev *tcm_dev,
  ...
  retval =
      tcm_dev->write_message(tcm_dev, CMD_GET_ROMBOOT_INFO, NULL, 0, &resp_code,
  ...
  copy_size =
      MIN(sizeof(struct tcm_romboot_info), tcm_dev->resp_buf.data_length);

  /* copy romboot_info to caller */
  retval = syna_pal_mem_cpy(
      (unsigned char *)rom_boot_info, sizeof(struct tcm_romboot_info),
      tcm_dev->resp_buf.buf, tcm_dev->resp_buf.buf_size, copy_size);
  if (retval < 0) {
