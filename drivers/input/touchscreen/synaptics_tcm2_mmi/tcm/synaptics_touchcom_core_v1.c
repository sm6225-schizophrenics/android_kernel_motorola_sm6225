...
static int syna_tcm_v1_parse_idinfo(struct tcm_dev *tcm_dev,
  ...
  retval = syna_pal_mem_cpy((unsigned char *)id_info,
                            sizeof(struct tcm_identification_info), data, size,
...
static void syna_tcm_v1_dispatch_report(struct tcm_dev *tcm_dev) {
  ...
  if (tcm_msg->report_code == REPORT_IDENTIFY) {
    ...
    retval = syna_tcm_v1_parse_idinfo(
        tcm_dev, &tcm_msg->in.buf[MESSAGE_HEADER_SIZE],
        tcm_msg->in.buf_size - MESSAGE_HEADER_SIZE, tcm_msg->payload_length);
    if (retval < 0) {
  ...
  retval = syna_pal_mem_cpy(
      tcm_dev->report_buf.buf, tcm_dev->report_buf.buf_size,
...
static void syna_tcm_v1_dispatch_response(struct tcm_dev *tcm_dev) {
  ...
  retval = syna_pal_mem_cpy(tcm_dev->resp_buf.buf, tcm_dev->resp_buf.buf_size,
                            &tcm_msg->in.buf[MESSAGE_HEADER_SIZE],
...
static int syna_tcm_v1_write(struct tcm_dev *tcm_dev, unsigned char command,
  ...
  if (command != CMD_CONTINUE_WRITE) {
    ...
    if (payload_len > 0) {
      retval = syna_pal_mem_cpy(&tcm_msg->out.buf[3], tcm_msg->out.buf_size - 3,
    ...
    retval = syna_pal_mem_cpy(&tcm_msg->out.buf[1], tcm_msg->out.buf_size - 1,
                              payload, payload_len, payload_len);
    if (retval < 0) {
  ...
  if (extra_crc) {
    retval =
        syna_pal_mem_cpy(&tcm_msg->out.buf[size], tcm_msg->out.buf_size - size,
                         crc16, sizeof(crc16), sizeof(crc16));
    if (retval < 0) {
  ...
  retval = syna_tcm_write(tcm_dev, tcm_msg->out.buf, size);
  if (retval < 0) {
...
static int syna_tcm_v1_continued_read(struct tcm_dev *tcm_dev,
  ...
  for (idx = 0; idx < chunks; idx++) {
     ...
     */
    retval =
        syna_tcm_v1_read(tcm_dev, xfer_length + 2, tcm_msg->temp.buf,
                         tcm_msg->temp.buf_size, (tcm_msg->has_crc) && last);
    if (retval < 0) {
    ...
    retval = syna_pal_mem_cpy(
        &tcm_msg->in.buf[offset], tcm_msg->in.buf_size - offset,
        &tcm_msg->temp.buf[2], tcm_msg->temp.buf_size - 2, xfer_length);
    if (retval < 0) {
      LOGE("Fail to copy payload\n");
      goto exit;
    }
...
static int syna_tcm_v1_read_message(struct tcm_dev *tcm_dev,
  ...
  /* read in the message header from device */
  retval = syna_tcm_v1_read(tcm_dev, len, tcm_msg->in.buf, tcm_msg->in.buf_size,
                            false);
  if (retval < 0) {
  ...
  if (tcm_msg->status_report_code != STATUS_IDLE)
    LOGD("Status code: 0x%02x, length: %d (%02x %02x %02x %02x)\n",
         tcm_msg->status_report_code, tcm_msg->payload_length, header->data[0],
...
do_dispatch:
  ...
  if (tcm_msg->payload_length > 0) {
    retval =
        syna_tcm_buf_alloc(&tcm_dev->external_buf, tcm_msg->payload_length);
    if (retval < 0) {
      LOGE("Fail to allocate memory, external_buf invalid\n");
    } else {
      retval = syna_pal_mem_cpy(
          &tcm_dev->external_buf.buf[0], tcm_msg->payload_length,
          &tcm_msg->in.buf[MESSAGE_HEADER_SIZE],
          tcm_msg->in.buf_size - MESSAGE_HEADER_SIZE, tcm_msg->payload_length);
      if (retval < 0)
  ...
  if (tcm_msg->predict_reads && do_predict) {
    if (tcm_dev->max_rd_size < MESSAGE_HEADER_SIZE)
      ...
      tcm_msg->predict_length =
          MIN(tcm_msg->payload_length,
...
static int syna_tcm_v1_write_message(struct tcm_dev *tcm_dev,
  ...
  for (idx = 0; idx < chunks; idx++) {
    ...
    if (idx == 0) {
      retval =
          syna_tcm_v1_write(tcm_dev, tcm_msg->command, &payload[0], xfer_length,
                            (tcm_msg->has_crc) && last, crc16);
    } else {
      retval = syna_tcm_v1_write(tcm_dev, CMD_CONTINUE_WRITE,
                                 &payload[idx * chunk_space], xfer_length,
                                 (tcm_msg->has_crc) && last, crc16);
    }
...
int syna_tcm_v1_detect(struct tcm_dev *tcm_dev, unsigned char *data,
  ...
  if (header->code == REPORT_IDENTIFY) {
    ...
    retval = syna_tcm_v1_continued_read(tcm_dev, payload_length);
    if (retval < 0) {
    ...
    retval = syna_tcm_v1_write_message(tcm_dev, CMD_IDENTIFY, NULL, 0,
                                       &resp_code, RESP_IN_POLLING);
    if (retval < 0) {
      /* in case the identify command is not working,
       * send a rest command as the workaround
       */
      retval = syna_tcm_v1_write_message(tcm_dev, CMD_RESET, NULL, 0,
                                         &resp_code, RESET_DELAY_MS);
      if (retval < 0) {
  ...
  retval = syna_tcm_v1_parse_idinfo(
      tcm_dev, &tcm_msg->in.buf[MESSAGE_HEADER_SIZE],
      tcm_msg->in.buf_size - MESSAGE_HEADER_SIZE, payload_length);
  if (retval < 0) {
    LOGE("Fail to identify device\n");
    syna_tcm_buf_unlock(&tcm_msg->in);
    return retval;
