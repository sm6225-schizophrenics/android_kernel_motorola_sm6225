...
#define SYNA_TCM_CORE_LIB_VERSION 0x0116
 ...
 */
#define IS_APP_FW_MODE(mode) (mode == MODE_APPLICATION_FIRMWARE)

#define IS_NOT_APP_FW_MODE(mode) (!IS_APP_FW_MODE(mode))

#define IS_BOOTLOADER_MODE(mode)                                               \
  ((mode == MODE_BOOTLOADER) || (mode == MODE_TDDI_BOOTLOADER) ||              \
   (mode == MODE_TDDI_HDL_BOOTLOADER) ||                                       \
   (mode == MODE_MULTICHIP_TDDI_BOOTLOADER))

#define IS_ROM_BOOTLOADER_MODE(mode) (mode == MODE_ROMBOOTLOADER)
 ...
 */
typedef int (*tcm_custom_touch_entity_callback_t)(
    const unsigned char code, const unsigned char *config,
    unsigned int *config_offset, const unsigned char *report,
    unsigned int *report_offset, unsigned int report_size, void *callback_data);
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
typedef int (*tcm_custom_gesture_callback_t)(
    const unsigned char code, const unsigned char *config,
    unsigned int *config_offset, const unsigned char *report,
    unsigned int *report_offset, unsigned int report_size, void *callback_data);
...
static inline int syna_tcm_buf_realloc(struct tcm_buffer *pbuf,
  ...
  if (size > pbuf->buf_size) {
    ...
    retval = syna_pal_mem_cpy(pbuf->buf, size, temp_src, temp_size, temp_size);
    if (retval < 0) {
...
static inline int syna_tcm_buf_copy(struct tcm_buffer *dest,
  ...
  retval = syna_pal_mem_cpy(dest->buf, dest->buf_size, src->buf, src->buf_size,
                            src->data_length);
  if (retval < 0) {
 ...
 */
static inline int syna_tcm_read(struct tcm_dev *tcm_dev, unsigned char *rd_data,
                                unsigned int rd_len) {
  struct syna_hw_interface *hw_if;
 ...
 *    the crc-6 value
 */
static inline unsigned char syna_tcm_crc6(unsigned char *p, unsigned int bits) {
  unsigned short r = 0x003F << 2;
  unsigned short x;
  static unsigned short crc6_table[16] = {0,    268,  536,  788,  1072, 1340,
                                          1576, 1828, 2144, 2412, 2680, 2932,
                                          3152, 3420, 3656, 3908};
 ...
 *    the crc-16 value
 */
static inline unsigned short syna_tcm_crc16(unsigned char *p, unsigned int len,
                                            unsigned short val) {
