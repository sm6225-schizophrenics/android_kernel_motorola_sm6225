...
int syna_tcm_allocate_device(struct tcm_dev **ptcm_dev_ptr,
  ...
  *ptcm_dev_ptr = NULL;
  ...
  tcm_dev = (struct tcm_dev *)syna_pal_mem_alloc(1, sizeof(struct tcm_dev));
  if (!tcm_dev) {
...
int syna_tcm_detect_device(struct tcm_dev *tcm_dev) {
  ...
  retval = syna_tcm_read(tcm_dev, data, (unsigned int)sizeof(data));
  if (retval < 0) {
  ...
  retval = syna_tcm_detect_protocol(tcm_dev, data, (unsigned int)sizeof(data));
  if (retval < 0) {
  ...
  case MODE_APPLICATION_FIRMWARE:
    LOGI("Device in Application FW, build id: %d, %s\n",
         tcm_dev->packrat_number, tcm_dev->id_info.part_number);
    break;
...
int syna_tcm_identify(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_IDENTIFY, NULL, 0, &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
  ...
  retval = syna_pal_mem_cpy(
      (unsigned char *)id_info, sizeof(struct tcm_identification_info),
      tcm_dev->resp_buf.buf, tcm_dev->resp_buf.buf_size,
...
int syna_tcm_reset(struct tcm_dev *tcm_dev) {
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_RESET, NULL, 0, &resp_code,
                                  resp_handling);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x\n", CMD_RESET);
    goto exit;
...
int syna_tcm_enable_report(struct tcm_dev *tcm_dev, unsigned char report_code,
  ...
  retval = tcm_dev->write_message(tcm_dev, command, &report_code, 1, &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x to %s 0x%02x report\n", command,
         (en) ? "enable" : "disable", report_code);
    goto exit;
  }
...
static int syna_tcm_run_rom_bootloader_fw(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_REBOOT_TO_ROM_BOOTLOADER, NULL,
                                  0, &resp_code, fw_switch_delay);
  if (retval < 0) {
...
static int syna_tcm_run_bootloader_fw(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_RUN_BOOTLOADER_FIRMWARE, NULL, 0,
                                  &resp_code, fw_switch_delay);
  if (retval < 0) {
...
static int syna_tcm_run_application_fw(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_RUN_APPLICATION_FIRMWARE, NULL,
                                  0, &resp_code, fw_switch_delay);
  if (retval < 0) {
...
int syna_tcm_get_boot_info(struct tcm_dev *tcm_dev,
  ...
  retval =
      tcm_dev->write_message(tcm_dev, CMD_GET_BOOT_INFO, NULL, 0, &resp_code,
                             tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x\n", CMD_GET_BOOT_INFO);
    goto exit;
  ...
  retval = syna_pal_mem_cpy((unsigned char *)&tcm_dev->boot_info,
                            sizeof(struct tcm_boot_info), tcm_dev->resp_buf.buf,
                            tcm_dev->resp_buf.buf_size, copy_size);
  if (retval < 0) {
...
int syna_tcm_get_app_info(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_GET_APPLICATION_INFO, NULL, 0,
                                  &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
  ...
  /* save the app_info */
  retval = syna_pal_mem_cpy(
      (unsigned char *)info, sizeof(struct tcm_application_info),
  ...
  retval = syna_pal_mem_cpy(
      (unsigned char *)app_info, sizeof(struct tcm_application_info),
      tcm_dev->resp_buf.buf, tcm_dev->resp_buf.buf_size, copy_size);
  if (retval < 0) {
...
show_info:
  ...
  syna_pal_mem_cpy((unsigned char *)tcm_dev->config_id, MAX_SIZE_CONFIG_ID,
                   info->customer_config_id, MAX_SIZE_CONFIG_ID,
                   MAX_SIZE_CONFIG_ID);
  ...
  LOGD("App info: max_objs: %d, max_x:%d, max_y: %d, img: %dx%d\n",
       tcm_dev->max_objects, tcm_dev->max_x, tcm_dev->max_y, tcm_dev->rows,
       tcm_dev->cols);
...
int syna_tcm_get_static_config(struct tcm_dev *tcm_dev, unsigned char *buf,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_GET_STATIC_CONFIG, NULL, 0,
                                  &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
  ...
  /* copy app_info to caller */
  retval = syna_pal_mem_cpy((unsigned char *)buf, buf_size,
                            tcm_dev->resp_buf.buf, tcm_dev->resp_buf.buf_size,
...
int syna_tcm_set_static_config(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_SET_STATIC_CONFIG, config_data,
                                  config_data_size, &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
...
int syna_tcm_get_dynamic_config(struct tcm_dev *tcm_dev, unsigned char id,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_GET_DYNAMIC_CONFIG, &out,
                                  sizeof(out), &resp_code, delay_ms_resp);
  if (retval < 0) {
...
int syna_tcm_set_dynamic_config(struct tcm_dev *tcm_dev, unsigned char id,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_SET_DYNAMIC_CONFIG, out,
                                  sizeof(out), &resp_code, delay_ms_resp);
  if (retval < 0) {
...
int syna_tcm_rezero(struct tcm_dev *tcm_dev) {
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_REZERO, NULL, 0, &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x\n", CMD_REZERO);
    goto exit;
...
int syna_tcm_set_config_id(struct tcm_dev *tcm_dev, unsigned char *config_id,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_SET_CONFIG_ID, config_id, size,
                                  &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x\n", CMD_SET_CONFIG_ID);
...
int syna_tcm_sleep(struct tcm_dev *tcm_dev, bool en) {
  ...
  retval = tcm_dev->write_message(tcm_dev, command, NULL, 0, &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
...
int syna_tcm_get_features(struct tcm_dev *tcm_dev,
  ...
  retval =
      tcm_dev->write_message(tcm_dev, CMD_GET_FEATURES, NULL, 0, &resp_code,
                             tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x\n", CMD_GET_FEATURES);
    goto exit;
  ...
  retval =
      syna_pal_mem_cpy((unsigned char *)info, sizeof(struct tcm_features_info),
                       tcm_dev->resp_buf.buf, tcm_dev->resp_buf.buf_size,
...
int syna_tcm_run_production_test(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_PRODUCTION_TEST, &test_code, 1,
                                  &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x\n", CMD_PRODUCTION_TEST);
...
int syna_tcm_send_command(struct tcm_dev *tcm_dev, unsigned char command,
                          ...
                          unsigned char *code, struct tcm_buffer *resp,
                          unsigned int delay_ms_resp) {
  int retval = 0;
  ...
  retval = tcm_dev->write_message(tcm_dev, command, payload, payload_length,
                                  code, delay_ms_resp);
