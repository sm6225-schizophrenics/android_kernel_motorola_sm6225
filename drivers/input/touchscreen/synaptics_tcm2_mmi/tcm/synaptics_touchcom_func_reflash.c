 ...
 */
static int
syna_tcm_set_up_flash_access(struct tcm_dev *tcm_dev,
  ...
  if (IS_APP_FW_MODE(id_info.mode)) {
    ...
    retval = syna_tcm_switch_fw_mode(tcm_dev, MODE_BOOTLOADER,
 ...
 */
static int
syna_tcm_check_flash_app_config(struct block_data *app_config,
 ...
 */
static int
syna_tcm_check_flash_block(struct tcm_dev *tcm_dev,
                           struct tcm_reflash_data_blob *reflash_data,
...
static int syna_tcm_get_flash_data_location(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_GET_DATA_LOCATION, &payload,
                                  sizeof(payload), &resp_code,
...
static int syna_tcm_reflash_send_command(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, command, payload, payload_len,
                                  &resp_code, delay_ms_resp);
  if (retval < 0) {
    LOGE("Fail to send command 0x%02x\n", command);
    goto exit;
  }
...
static int syna_tcm_read_flash(struct tcm_dev *tcm_dev, unsigned int address,
  ...
  LOGD("Flash address: 0x%x (words: 0x%x), size: %d (words: %d)\n", address,
       flash_addr_words, rd_len, length_words);
  ...
  retval = syna_tcm_reflash_send_command(tcm_dev, CMD_READ_FLASH, out,
                                         sizeof(out), delay_ms);
  if (retval < 0) {
  ...
  retval = syna_pal_mem_cpy(rd_data, rd_len, tcm_dev->resp_buf.buf,
                            tcm_dev->resp_buf.buf_size, rd_len);
  if (retval < 0) {
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_read_flash_boot_config(
    struct tcm_dev *tcm_dev, struct tcm_reflash_data_blob *reflash_data,
  ...
  retval =
      syna_tcm_read_flash(tcm_dev, addr, rd_data->buf, length, rd_delay_us);
  if (retval < 0) {
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_read_flash_app_config(
    struct tcm_dev *tcm_dev, struct tcm_reflash_data_blob *reflash_data,
    struct tcm_buffer *rd_data, unsigned int rd_delay_us) {
  int retval;
  unsigned int temp;
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_read_flash_disp_config(
    struct tcm_dev *tcm_dev, struct tcm_reflash_data_blob *reflash_data,
    struct tcm_buffer *rd_data, unsigned int rd_delay_us) {
  int retval;
  unsigned int temp;
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_read_flash_custom_otp(
    struct tcm_dev *tcm_dev, struct tcm_reflash_data_blob *reflash_data,
    struct tcm_buffer *rd_data, unsigned int rd_delay_us) {
  int retval;
  unsigned int temp;
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_read_flash_custom_data(
    struct tcm_dev *tcm_dev, struct tcm_reflash_data_blob *reflash_data,
    unsigned int address, unsigned int size, struct tcm_buffer *rd_data,
...
int syna_tcm_read_flash_area(struct tcm_dev *tcm_dev, enum flash_area area,
  ...
  case AREA_FORCE_TUNING:
  case AREA_OPEN_SHORT_TUNING:
    retval = syna_tcm_get_flash_data_location(tcm_dev, area, &addr, &length);
    if (retval < 0) {
  ...
  switch (area) {
  case AREA_BOOT_CONFIG:
    retval = syna_tcm_read_flash_boot_config(tcm_dev, &reflash_data, rd_data,
                                             rd_delay_us);
    if (retval < 0) {
    ...
    break;
  case AREA_APP_CONFIG:
    retval = syna_tcm_read_flash_app_config(tcm_dev, &reflash_data, rd_data,
                                            rd_delay_us);
    if (retval < 0) {
    ...
    break;
  case AREA_DISP_CONFIG:
    retval = syna_tcm_read_flash_disp_config(tcm_dev, &reflash_data, rd_data,
                                             rd_delay_us);
    if (retval < 0) {
  ...
  case AREA_CUSTOM_OTP:
    retval = syna_tcm_read_flash_custom_otp(tcm_dev, &reflash_data, rd_data,
                                            rd_delay_us);
    if (retval < 0) {
  ...
  case AREA_FORCE_TUNING:
  case AREA_OPEN_SHORT_TUNING:
    retval = syna_tcm_read_flash_custom_data(tcm_dev, &reflash_data, addr,
                                             length, rd_data, rd_delay_us);
    break;
  default:
...
exit:
  retval = syna_tcm_switch_fw_mode(tcm_dev, MODE_APPLICATION_FIRMWARE,
                                   FW_MODE_SWITCH_DELAY_MS);
  if (retval < 0)
...
static int syna_tcm_write_flash(struct tcm_dev *tcm_dev,
  ...
  while (remaining_length) {
    ...
    retval = syna_tcm_reflash_send_command(tcm_dev, CMD_WRITE_FLASH,
                                           reflash_data->out.buf,
                                           xfer_length + 2, delay_ms);
    if (retval < 0) {
 ...
 */
static int
syna_tcm_write_flash_block(struct tcm_dev *tcm_dev,
                           struct tcm_reflash_data_blob *reflash_data,
  ...
  retval = syna_tcm_write_flash(tcm_dev, reflash_data, flash_addr, data, size,
                                wr_delay_us);
  if (retval < 0) {
...
static int syna_tcm_erase_flash(struct tcm_dev *tcm_dev,
  ...
  retval = syna_tcm_reflash_send_command(tcm_dev, CMD_ERASE_FLASH, out_buf,
                                         size_erase_cmd, erase_delay_ms);
  if (retval < 0) {
 ...
 */
static int
syna_tcm_erase_flash_block(struct tcm_dev *tcm_dev,
                           struct tcm_reflash_data_blob *reflash_data,
  ...
  retval =
      syna_tcm_erase_flash(tcm_dev, reflash_data, flash_addr, size, delay_ms);
  if (retval < 0) {
 ...
 */
static int
syna_tcm_update_flash_block(struct tcm_dev *tcm_dev,
                            struct tcm_reflash_data_blob *reflash_data,
                            struct block_data *block, unsigned int delay_ms) {
  int retval;
  ...
  retval =
      syna_tcm_erase_flash_block(tcm_dev, reflash_data, block, erase_delay_ms);
  if (retval < 0) {
  ...
  retval =
      syna_tcm_write_flash_block(tcm_dev, reflash_data, block, wr_blk_delay_ms);
  if (retval < 0) {
...
static int syna_tcm_do_reflash_tddi(struct tcm_dev *tcm_dev,
  ...
  for (idx = 0; idx < AREA_MAX; idx++) {
    ...
    retval = syna_tcm_erase_flash_block(tcm_dev, reflash_data, block,
                                        erase_delay_ms);
    if (retval < 0) {
      LOGE("Fail to erase %s area\n", AREA_ID_STR(block->id));
      goto exit;
  ...
  for (idx = 0; idx < AREA_MAX; idx++) {
    ...
    retval = syna_tcm_write_flash_block(tcm_dev, reflash_data, block,
                                        wr_blk_delay_ms);
    if (retval < 0) {
 ...
 */
static int
syna_tcm_do_reflash_generic(struct tcm_dev *tcm_dev,
                            struct tcm_reflash_data_blob *reflash_data,
  ...
  case UPDATE_FIRMWARE_CONFIG:
    ...
    retval = syna_tcm_update_flash_block(tcm_dev, reflash_data, block,
                                         wait_delay_ms);
    if (retval < 0) {
...
int syna_tcm_do_fw_update(struct tcm_dev *tcm_dev, const unsigned char *image,
  ...
  if ((IS_APP_FW_MODE(tcm_dev->dev_mode)) && (app_status != APP_STATUS_OK)) {
    LOGN("Bad application firmware, need to update firmware and config\n");
    type = UPDATE_FIRMWARE_CONFIG;
    goto reflash;
  }

  type =
      (enum update_area)syna_tcm_compare_image_id_info(tcm_dev, &reflash_data);
...
reflash:
  ...
  if (tcm_dev->dev_mode == MODE_BOOTLOADER) {
    retval = syna_tcm_do_reflash_generic(tcm_dev, &reflash_data, type,
                                         wait_delay_ms);
  } else if (tcm_dev->dev_mode == MODE_TDDI_BOOTLOADER) {
    retval =
        syna_tcm_do_reflash_tddi(tcm_dev, &reflash_data, type, wait_delay_ms);
  } else {
