...
int syna_tcm_parse_touch_report(struct tcm_dev *tcm_dev, unsigned char *report,
  ...
  while (idx < config_size) {
    ...
    case TOUCH_REPORT_GESTURE_ID:
      if (tcm_dev->cb_custom_gesture) {
        retval = tcm_dev->cb_custom_gesture(
            TOUCH_REPORT_GESTURE_ID, config_data, &idx, report, &offset,
            report_size, tcm_dev->cbdata_gesture);
      } else {
        bits = config_data[idx++];
        retval =
            syna_tcm_get_touch_data(report, report_size, offset, bits, &data);
    ...
    case TOUCH_REPORT_GESTURE_DATA:
      if (tcm_dev->cb_custom_gesture) {
        retval = tcm_dev->cb_custom_gesture(
            TOUCH_REPORT_GESTURE_DATA, config_data, &idx, report, &offset,
            report_size, tcm_dev->cbdata_gesture);
      } else {
        bits = config_data[idx++];
        retval = syna_tcm_get_gesture_data(report, report_size, offset, bits,
    ...
    default:
      ...
      if (tcm_dev->cb_custom_touch_entity) {
        retval = tcm_dev->cb_custom_touch_entity(code, config_data, &idx,
                                                 report, &offset, report_size,
...
int syna_tcm_set_touch_report_config(struct tcm_dev *tcm_dev,
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_SET_TOUCH_REPORT_CONFIG, data,
                                  size, &resp_code,
...
int syna_tcm_preserve_touch_report_config(struct tcm_dev *tcm_dev) {
  ...
  retval = tcm_dev->write_message(tcm_dev, CMD_GET_TOUCH_REPORT_CONFIG, NULL, 0,
                                  &resp_code,
                                  tcm_dev->msg_data.default_resp_reading);
  if (retval < 0) {
  ...
  retval = syna_pal_mem_cpy(
      tcm_dev->touch_config.buf, tcm_dev->touch_config.buf_size,
      tcm_dev->resp_buf.buf, tcm_dev->resp_buf.buf_size, size);
  if (retval < 0) {
