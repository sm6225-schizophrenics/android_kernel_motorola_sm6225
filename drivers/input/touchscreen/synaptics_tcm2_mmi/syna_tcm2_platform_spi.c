 ...
 */
static int syna_spi_request_gpio(int gpio, bool config, int dir, int state,
                                 char *label) {
  int retval;
...
static int syna_spi_config_gpio(struct syna_hw_interface *hw_if) {
  ...
  if (rst->reset_gpio >= 0) {
    retval = syna_spi_request_gpio(rst->reset_gpio, true, 1,
  ...
  if (bus->switch_gpio >= 0) {
    retval = syna_spi_request_gpio(bus->switch_gpio, true, 1, bus->switch_state,
                                   str_io_switch_gpio);
    if (retval < 0) {
...
static int syna_spi_config_psu(struct syna_hw_interface *hw_if) {
  ...
  if (pwr->psu == PSU_GPIO) {
    /* set up power gpio */
    if (pwr->vdd_gpio >= 0) {
      retval = syna_spi_request_gpio(pwr->vdd_gpio, true, 1,
    ...
    if (pwr->avdd_gpio >= 0) {
      retval = syna_spi_request_gpio(pwr->avdd_gpio, true, 1,
