...
static int syna_ts_mmi_methods_get_productinfo(struct device *dev,
                                               void *cdata) {
  struct syna_tcm *tcm;
...
static int syna_ts_mmi_methods_get_build_id(struct device *dev, void *cdata) {
  ...
  return scnprintf(TO_CHARP(cdata), TS_MMI_MAX_ID_LEN, "%s",
...
static int syna_ts_mmi_methods_get_flashprog(struct device *dev, void *idata) {
  ...
  if (IS_NOT_FW_MODE(tcm->tcm_dev->id_info.mode) || app_status != APP_STATUS_OK)
    TO_INT(idata) = 1;
  else
    TO_INT(idata) = 0;
...
static int syna_ts_mmi_panel_state(struct device *dev, enum ts_mmi_pm_mode from,
...
static int syna_ts_mmi_charger_mode(struct device *dev, int mode) {
  ...
  retval = syna_tcm_get_dynamic_config(
      tcm->tcm_dev, DC_ENABLE_CHARGER_CONNECTED, &cval, RESP_IN_ATTN);
  if (retval < 0) {
  ...
  if (cval != mode) {
    retval = syna_tcm_set_dynamic_config(
        tcm->tcm_dev, DC_ENABLE_CHARGER_CONNECTED, mode, RESP_IN_ATTN);
    if (retval < 0) {
