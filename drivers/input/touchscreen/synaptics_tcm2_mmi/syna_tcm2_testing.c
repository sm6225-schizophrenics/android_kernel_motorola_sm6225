...
#define STR_MAX_SIZE 512
  ...
  while (offset < size) {
    ...
    retval = syna_pal_mem_cpy(str_buf, STR_MAX_SIZE, &buf[offset], str_size,
                              str_size);
    if (retval < 0) {
...
static void syna_testing_log_test_data(unsigned char *data,
                                       ...
                                       unsigned char test_item, int rows,
                                       int cols) {
  ...
  if ((test_item >= TEST_PID01_TRX_TRX_SHORTS) &&
      (test_item <= TEST_PID03_TRX_GROUND_SHORTS)) {
    goto open_short;
...
open_short:
  ...
  offset = 0;
  cnt =
      snprintf(log_ptr + offset, log_size - offset, "TEST PT$%02d:", test_item);
  offset += cnt;
  for (i = 0; i < data_size; i++) {
    cnt = snprintf(log_ptr + offset, log_size - offset, "0x%02x ",
                   *open_short_ptr);
    offset += cnt;
    open_short_ptr++;
  }
...
static bool syna_testing_compare_frame(unsigned char *data,
                                       unsigned int data_size, int rows,
                                       int cols, const short *limits_hi,
                                       const short *limits_lo) {
  bool result = false;
  short *data_ptr = NULL;
  short limit;
  int i, j;
...
static int syna_testing_device_id(struct syna_tcm *tcm) {
  ...
  if (limit_panel) {
    strptr =
        strnstr(info.part_number, device_id_limit_v, strlen(info.part_number));
    if (strptr != NULL)
      result = true;
    else {
    ...
    strptr =
        strnstr(info.part_number, device_id_limit_c, strlen(info.part_number));
    if (strptr != NULL)
      result = true;
    else {
      LOGE("Device ID mismatched, FW: %s (limit: %s)\n", info.part_number,
  ...
  if (limit_panel) {
    for (idx = 0; idx < sizeof(config_id_limit_v); idx++) {
      if (config_id_limit_v[idx] != info.customer_config_id[idx]) {
        LOGE("Fail on byte.%d (data: %02X, limit: %02X)\n", idx,
             info.customer_config_id[idx], config_id_limit_v[idx]);
        result = false;
      }
    }
  } else {
    for (idx = 0; idx < sizeof(config_id_limit_c); idx++) {
      if (config_id_limit_c[idx] != info.customer_config_id[idx]) {
        LOGE("Fail on byte.%d (data: %02X, limit: %02X)\n", idx,
             info.customer_config_id[idx], config_id_limit_c[idx]);
        result = false;
      }
    }
  }
...
static ssize_t syna_testing_check_id_show(struct kobject *kobj,
  ...
  if (!tcm->is_connected) {
    retval = snprintf(buf, PAGE_SIZE, "Device is NOT connected\n");
    goto exit;
  }
  ...
  retval = snprintf(buf, PAGE_SIZE - count, "Device ID check: %s\n",
                    (retval < 0) ? "fail" : "pass");
  ...
  retval = snprintf(buf, PAGE_SIZE - count, "Config ID check: %s\n",
                    (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt01(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(tcm->tcm_dev, TEST_PID01_TRX_TRX_SHORTS,
                                        &test_data);
  if (retval < 0) {
  ...
  if (limit_panel) {
    result = syna_testing_compare_byte_vector(
        test_data.buf, test_data.data_length, pt01_limits_v,
        ARRAY_SIZE(pt01_limits_v));
  } else {
    result = syna_testing_compare_byte_vector(
        test_data.buf, test_data.data_length, pt01_limits_c,
        ARRAY_SIZE(pt01_limits_c));
  }

  syna_testing_log_test_data(test_data.buf, test_data.data_length,
                             TEST_PID01_TRX_TRX_SHORTS, tcm->tcm_dev->rows,
                             tcm->tcm_dev->cols);
...
static ssize_t syna_testing_pt01_show(struct kobject *kobj,
  ...
  if (!tcm->is_connected) {
    count = snprintf(buf, PAGE_SIZE, "Device is NOT connected\n");
    goto exit;
  }

  retval = syna_testing_pt01(tcm);

  count = snprintf(buf, PAGE_SIZE, "TEST PT$01: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt02(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(
      tcm->tcm_dev, TEST_PID02_TRX_SENSOR_OPENS, &test_data);
  if (retval < 0) {
  ...
  if (limit_panel) {
    result = syna_testing_compare_byte_vector(
        test_data.buf, test_data.data_length, pt02_limits_v,
        ARRAY_SIZE(pt02_limits_v));

  } else {
    result = syna_testing_compare_byte_vector(
        test_data.buf, test_data.data_length, pt02_limits_c,
        ARRAY_SIZE(pt02_limits_c));
  }

  syna_testing_log_test_data(test_data.buf, test_data.data_length,
                             TEST_PID02_TRX_SENSOR_OPENS, tcm->tcm_dev->rows,
                             tcm->tcm_dev->cols);
...
static ssize_t syna_testing_pt02_show(struct kobject *kobj,
  ...
  count = snprintf(buf, PAGE_SIZE, "TEST PT$02: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt03(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(
      tcm->tcm_dev, TEST_PID03_TRX_GROUND_SHORTS, &test_data);
  if (retval < 0) {
  ...
  if (limit_panel) {
    result = syna_testing_compare_byte_vector(
        test_data.buf, test_data.data_length, pt03_limits_v,
        ARRAY_SIZE(pt03_limits_v));
  } else {
    result = syna_testing_compare_byte_vector(
        test_data.buf, test_data.data_length, pt03_limits_c,
        ARRAY_SIZE(pt03_limits_c));
  }

  syna_testing_log_test_data(test_data.buf, test_data.data_length,
                             TEST_PID03_TRX_GROUND_SHORTS, tcm->tcm_dev->rows,
                             tcm->tcm_dev->cols);
...
static ssize_t syna_testing_pt03_show(struct kobject *kobj,
  ...
  count = snprintf(buf, PAGE_SIZE, "TEST PT$03: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt05(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(tcm->tcm_dev, TEST_PID05_FULL_RAW_CAP,
                                        &test_data);
  if (retval < 0) {
  ...
  if (limit_panel) {
    result = syna_testing_compare_frame(test_data.buf, test_data.data_length,
  ...
  syna_testing_log_test_data(test_data.buf, test_data.data_length,
                             TEST_PID05_FULL_RAW_CAP, tcm->tcm_dev->rows,
                             tcm->tcm_dev->cols);
...
static ssize_t syna_testing_pt05_show(struct kobject *kobj,
  ...
  count = snprintf(buf, PAGE_SIZE, "TEST PT$05: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt0a(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(tcm->tcm_dev, TEST_PID10_DELTA_NOISE,
                                        &test_data);
  if (retval < 0) {
  ...
  syna_testing_log_test_data(test_data.buf, test_data.data_length,
                             TEST_PID10_DELTA_NOISE, tcm->tcm_dev->rows,
                             tcm->tcm_dev->cols);
...
static ssize_t syna_testing_pt0a_show(struct kobject *kobj,
  ...
  count = snprintf(buf, PAGE_SIZE, "TEST PT$0A: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt10(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(tcm->tcm_dev, TEST_PID16_SENSOR_SPEED,
                                        &test_data);
  if (retval < 0) {
...
static ssize_t syna_testing_pt10_show(struct kobject *kobj,
  ...
  count = snprintf(buf, PAGE_SIZE, "TEST PT$10: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt11(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(tcm->tcm_dev, TEST_PID17_ADC_RANGE,
                                        &test_data);
  if (retval < 0) {
...
static ssize_t syna_testing_pt11_show(struct kobject *kobj,
  ...
  count = snprintf(buf, PAGE_SIZE, "TEST PT$11: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static int syna_testing_pt12(struct syna_tcm *tcm) {
  ...
  retval = syna_tcm_run_production_test(tcm->tcm_dev, TEST_PID18_HYBRID_ABS_RAW,
                                        &test_data);
  if (retval < 0) {
  ...
  syna_testing_log_test_data(test_data.buf, test_data.data_length,
                             TEST_PID18_HYBRID_ABS_RAW, tcm->tcm_dev->rows,
                             tcm->tcm_dev->cols);
...
static ssize_t syna_testing_pt12_show(struct kobject *kobj,
  ...
  count = snprintf(buf, PAGE_SIZE, "TEST PT$12: %s\n",
                   (retval < 0) ? "fail" : "pass");
...
static ssize_t syna_testing_get_rawdata_show(struct kobject *kobj,
...
#if SYNA_TESTING_RESULT_IN_CSV
  tcm->testing_log_data =
      vmalloc(SYNA_LOG_SIZE_MAX); // kzalloc(SAVE_BUF_SIZE, GFP_KERNEL);
  ...
  offset = 0;
  tmp = snprintf(buf + offset, PAGE_SIZE - offset, "Config ID Test: %s\n",
                 (retval < 0) ? "fail" : "pass");
  offset += tmp;

  // TRX short Test (TEST_PID01_TRX_TRX_SHORTS)
  retval = syna_testing_pt01(tcm);
  tmp = snprintf(buf + offset, PAGE_SIZE - offset, "TEST PT$01: %s\n",
                 (retval < 0) ? "fail" : "pass");
  offset += tmp;

  // Trx gnd test	 TEST_PID03_TRX_GROUND_SHORTS
  retval = syna_testing_pt03(tcm);
  tmp = snprintf(buf + offset, PAGE_SIZE - offset, "TEST PT$03: %s\n",
                 (retval < 0) ? "fail" : "pass");
  offset += tmp;

  // Full raw cap	 TEST_PID05_FULL_RAW_CAP
  retval = syna_testing_pt05(tcm);
  tmp = snprintf(buf + offset, PAGE_SIZE - offset, "TEST PT$05: %s\n",
                 (retval < 0) ? "fail" : "pass");
  offset += tmp;

  // noise test	 TEST_PID10_DELTA_NOISE
  retval = syna_testing_pt0a(tcm);
  tmp = snprintf(buf + offset, PAGE_SIZE - offset, "TEST PT$0a: %s\n",
                 (retval < 0) ? "fail" : "pass");
  offset += tmp;
...
int syna_testing_create_dir(struct syna_tcm *tcm, struct kobject *sysfs_dir) {
  ...
  g_testing_dir = kobject_create_and_add("testing", sysfs_dir);
  if (!g_testing_dir) {
