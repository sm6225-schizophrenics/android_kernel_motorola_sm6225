...
#define NEED_TO_SET_PINCTRL                                                    \
  (touch_cdev->pdata.power_off_suspend && touch_cdev->mdata->pinctrl &&        \
   IS_DEEPSLEEP_MODE)
#define NEED_TO_SET_POWER                                                      \
  (touch_cdev->pdata.power_off_suspend && touch_cdev->mdata->power &&          \
   IS_DEEPSLEEP_MODE)
...
static int ts_mmi_panel_off(struct ts_mmi_dev *touch_cdev) {
  ...
  if (touch_cdev->pdata.gestures_enabled ||
      touch_cdev->pdata.cli_gestures_enabled ||
      touch_cdev->pdata.support_liquid_detection ||
      touch_cdev->pdata.palm_enabled) {
...
static int ts_mmi_panel_event_handle(struct ts_mmi_dev *touch_cdev,
                                     enum ts_mmi_panel_event event) {
  int ret = 0;
  ...
  case TS_MMI_EVENT_PRE_DISPLAY_ON:
    ...
    } else if (touch_cdev->pdata.reset && touch_cdev->mdata->reset) {
...
#else
static int ts_mmi_panel_cb(struct notifier_block *nb, unsigned long event,
                           void *evd) {
  int idx = -1;
  int ret = 0;
  enum ts_mmi_panel_event panel_event;
  struct ts_mmi_dev *touch_cdev = container_of(nb, struct ts_mmi_dev, panel_nb);
  ...
  dev_dbg(
      DEV_MMI, "%s: %s event(%lu), ctrl_dsi=%d, idx=%d\n", __func__,
      EVENT_PRE_DISPLAY_OFF
          ? "EVENT_PRE_DISPLAY_OFF"
          : (EVENT_DISPLAY_OFF
                 ? "EVENT_DISPLAY_OFF"
...
static void ts_mmi_queued_resume(struct ts_mmi_dev *touch_cdev) {
   ...
   * So touchscreen class need let vendor driver know baseline update work need
   * to be done or not before vendor resume is called.
   */
...
static void ts_mmi_worker_func(struct work_struct *w) {
  ...
  struct ts_mmi_dev *touch_cdev = container_of(dw, struct ts_mmi_dev, work);
  int ret, cmd = 0;
#if defined(CONFIG_DRM_PANEL_NOTIFICATIONS) ||                                 \
    defined(CONFIG_DRM_PANEL_EVENT_NOTIFICATIONS)
...
static int ts_mmi_refresh_rate_cb(struct notifier_block *nb,
  ...
  struct ts_mmi_dev *touch_cdev = container_of(nb, struct ts_mmi_dev, freq_nb);
...
static int ts_mmi_charger_cb(struct notifier_block *self, unsigned long event,
                             void *ptr) {
  struct ts_mmi_dev *touch_cdev =
      container_of(self, struct ts_mmi_dev, ps_notif);
  struct power_supply *psy = ptr;
  int ret;
  bool present;

  if (!((event == PSY_EVENT_PROP_CHANGED) && psy && psy->desc->get_property &&
        psy->desc->name && !strncmp(psy->desc->name, "usb", sizeof("usb"))))
    return 0;
  ...
  dev_dbg(DEV_MMI, "%s: event=%lu, usb status: cur=%d, prev=%d\n", __func__,
          event, present, touch_cdev->ps_is_present);
...
static int ts_mmi_fps_cb(struct notifier_block *self, unsigned long event,
                         void *p) {
  int fps_state = *(int *)p;
  struct ts_mmi_dev *touch_cdev =
      container_of(self, struct ts_mmi_dev, fps_notif);
...
static int ts_mmi_fps_notifier_register(struct ts_mmi_dev *touch_cdev,
                                        bool enable) {
  int ret;
  ...
  if (enable) {
    ...
    if (ret < 0) {
      dev_err(DEV_TS, "Failed to register fps_notifier: %d\n", ret);
      return ret;
    }
    ...
    if (ret < 0) {
      dev_err(DEV_TS, "Failed to unregister fps_notifier: %d\n", ret);
    }
...
static int ts_mmi_lpd_cb(struct notifier_block *self, unsigned long event,
                         void *p) {
  int lpd_state = *(int *)p;
  struct ts_mmi_dev *touch_cdev =
      container_of(self, struct ts_mmi_dev, lpd_notif);
...
static int ts_mmi_lpd_notifier_register(struct ts_mmi_dev *touch_cdev,
                                        bool enable) {
  int ret;
  ...
  if (enable) {
    ...
    if (ret < 0) {
      dev_err(DEV_TS, "Failed to register lpd_notifier: %d\n", ret);
      return ret;
    }
    ...
    if (ret < 0) {
      dev_err(DEV_TS, "Failed to unregister lpd_notifier: %d\n", ret);
    }
...
int ts_mmi_notifiers_register(struct ts_mmi_dev *touch_cdev) {
  ...
  ret =
      kfifo_alloc(&touch_cdev->cmd_pipe, sizeof(unsigned int) * 10, GFP_KERNEL);
  if (ret)
  ...
  if (touch_cdev->pdata.usb_detection) {
    ...
    if (psy) {
      ...
      dev_info(DEV_MMI, "%s: USB initial status=%d\n", __func__,
               touch_cdev->ps_is_present);
    }
  }

  /*
  Because the touch pm_mode default is TS_MMI_PM_DEEPSLEEP, when the first
  suspend occurs, the interrupt will not be turned off, which results in I2C
  error. Need to set the initial pm_mode of the touch to PM_ACTIVE.
  */
  touch_cdev->pm_mode = TS_MMI_PM_ACTIVE;
