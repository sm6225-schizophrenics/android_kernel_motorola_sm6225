...
#define TOUCH_MMI_GET_ATTR_RW(name, fmt)                                       \
  ...
  static DEVICE_ATTR(name, (S_IWUSR | S_IWGRP | S_IRUGO), name##_show,         \
                     name##_store)
...
static DEVICE_ATTR(forcereflash, (S_IWUSR | S_IWGRP), NULL,
                   ts_mmi_forcereflash_store);
...
static DEVICE_ATTR(doreflash, (S_IWUSR | S_IWGRP), NULL,
                   ts_mmi_doreflash_store);
 ...
 */
static ssize_t gesture_show(struct device *dev, struct device_attribute *attr,
                            char *buf) {
  struct ts_mmi_dev *touch_cdev = dev_get_drvdata(dev);
...
static ssize_t gesture_store(struct device *dev, struct device_attribute *attr,
                             const char *buf, size_t size) {
  struct ts_mmi_dev *touch_cdev = dev_get_drvdata(dev);
  ...
  dev_info(dev, "%s: gesture_mode_type = 0x%02x \n", __func__,
           touch_cdev->gesture_mode_type);

  return size;
}
static DEVICE_ATTR(gesture, (S_IWUSR | S_IWGRP | S_IRUGO), gesture_show,
                   gesture_store);
#endif
...
static ssize_t liquid_detection_ctl_show(struct device *dev,
  ...
  return scnprintf(buf, PAGE_SIZE,
                   "function status: %02x, detection result: %02x\n",
...
static ssize_t pill_region_show(struct device *dev,
  ...
  ret =
      scnprintf(buf, PAGE_SIZE, "0x%02x 0x%x 0x%x", touch_cdev->pill_region[0],
                touch_cdev->pill_region[1], touch_cdev->pill_region[2]);
...
static DEVICE_ATTR(pill_region, (S_IWUSR | S_IWGRP | S_IRUGO), pill_region_show,
                   pill_region_store);
...
static ssize_t active_region_show(struct device *dev,
  ...
  ret = scnprintf(buf, PAGE_SIZE, "%u %u %u %u", touch_cdev->active_region[0],
                  touch_cdev->active_region[1], touch_cdev->active_region[2],
                  touch_cdev->active_region[3]);
...
static DEVICE_ATTR(active_region, (S_IWUSR | S_IWGRP | S_IRUGO),
                   active_region_show, active_region_store);

static int ts_mmi_sysfs_create_edge_entries(struct ts_mmi_dev *touch_cdev,
                                            bool create) {
  int ret = 0;
  ...
  if (create) {
    ...
    if (touch_cdev->pdata.poison_slot_ctrl) {
      ...
      touch_cdev->poison_trigger_distance =
          TOUCHSCREEN_MMI_DEFAULT_POISON_TRIGGER_DISTANCE;
...
static int ts_mmi_get_vendor_info(struct ts_mmi_dev *touch_cdev) {
  int ret = 0;
...
static int get_gesture_type_handler(struct device *parent,
...
static int report_liquid_detection_status_handler(struct device *parent,
                                                  int status) {
  struct ts_mmi_dev *touch_cdev = ts_mmi_dev_to_cdev(parent);
  if (!touch_cdev)
    return -ENODEV;
...
static int ts_mmi_clip_event_handler(struct device *parent,
  ...
  inside = (tev->x >= touch_cdev->clip.xUL) &&
           (tev->x <= touch_cdev->clip.xBR) &&
 ...
 */
int ts_mmi_dev_register(struct device *parent, struct ts_mmi_methods *mdata) {
  ...
  touch_cdev = devm_kzalloc(parent, sizeof(struct ts_mmi_dev), GFP_KERNEL);
  if (!touch_cdev)
    return -ENOMEM;
  ...
  if (IS_ERR_OR_NULL(touch_cdev->pinctrl_node)) {
    ...
    touch_cdev->pinctrl_on_state =
        pinctrl_lookup_state(touch_cdev->pinctrl_node, "ts_mmi_on_state");
    touch_cdev->pinctrl_off_state =
        pinctrl_lookup_state(touch_cdev->pinctrl_node, "ts_mmi_off_state");
    if (!IS_ERR_OR_NULL(touch_cdev->pinctrl_on_state) ||
  ...
  DEV_MMI = device_create(touchscreens_class, parent, touch_cdev->class_dev_no,
  ...
  touch_cdev->mdata->exports.report_liquid_detection_status =
      report_liquid_detection_status_handler;
  ...
  if (touch_cdev->mdata->extend_attribute_group) {
    ...
    if (touch_cdev->extern_group) {
      ret = sysfs_create_group(&DEV_MMI->kobj, touch_cdev->extern_group);
      if (ret)
