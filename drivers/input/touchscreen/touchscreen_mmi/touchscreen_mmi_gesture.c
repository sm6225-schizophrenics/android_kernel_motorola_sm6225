...
struct ts_mmi_touch_events_data {
  struct ts_mmi_dev *touch_cdev;
#ifdef TS_MMI_TOUCH_GESTURE_POISON_EVENT
  /* Touch slot is poisoned, do not report this slot point until slot release or
   * IC resumed. */
...
#ifdef TS_MMI_TOUCH_GESTURE_LOG_EVENT
static inline void
ts_mmi_touch_log_event(struct touch_event_with_time_data *dst,
                       const struct touch_event_data *src) {
  memcpy(&dst->event, src, sizeof(struct touch_event_data));
  dst->time = ktime_get();
}

static inline void
ts_mmi_touch_clear_event(struct touch_event_with_time_data *dst) {
...
static inline void
ts_mmi_touch_event_report_event_release(int index,
...
static inline int
ts_mmi_touch_event_get_gs_distance(struct ts_mmi_dev *touch_cdev) {
  return (ts_mmi_touch_can_handle_gs_distance())
             ? touch_cdev->gs_distance
             : TOUCHSCREEN_MMI_DEFAULT_GS_DISTANCE;
}
...
static inline bool ts_mmi_touch_can_handle_suppression(void) {
  ...
  return ((touch_cdev->pdata.suppression_ctrl) &&
          ...
          (touch_cdev->pdata.max_x > 0) && (touch_cdev->pdata.max_y > 0));
}

static inline bool
ts_mmi_touch_event_is_in_suppression(struct touch_event_data *tev,
                                     struct ts_mmi_dev *touch_cdev) {
  ...
  return ((tev->x < gs_distance) ||
          (tev->x > (touch_cdev->pdata.max_x - gs_distance)))
             ? true
             : false;
}
#endif /* TS_MMI_TOUCH_GESTURE_SUPPRESSION */
...
static inline bool
ts_mmi_touch_event_is_in_poison_trigger(struct touch_event_data *tev,
                                        struct ts_mmi_dev *touch_cdev) {
  ...
  if (touch_cdev->suppression == TS_MMI_DISABLE_SUPPRESSION_ALL)
    return false;
  if (touch_cdev->suppression == TS_MMI_DISABLE_SUPPRESSION_LEFT)
    return (tev->x > (touch_cdev->pdata.max_x - trigger_distance)) ? true
                                                                   : false;
  if (touch_cdev->suppression == TS_MMI_DISABLE_SUPPRESSION_RIGHT)
    return (tev->x < trigger_distance) ? true : false;

  return ((tev->x < trigger_distance) ||
          (tev->x > (touch_cdev->pdata.max_x - trigger_distance)))
             ? true
             : false;
}

static inline bool need_handle_poison_event(int id) {
  return ((events_data->poison_events[id].center.event.type ==
           TS_COORDINATE_ACTION_PRESS) ||
          (events_data->poison_events[id].center.event.type ==
           TS_COORDINATE_ACTION_MOVE));
}

static inline bool is_far_from_poison_center(struct touch_event_data *tev) {
  int delta_y =
      abs(tev->y - events_data->poison_events[tev->id].center.event.y);
...
static inline void update_poison_center(struct touch_event_data *tev) {
  return ts_mmi_touch_log_event(&events_data->poison_events[tev->id].center,
                                tev);
}
#endif /* TS_MMI_TOUCH_GESTURE_POISON_EVENT */
...
static int ts_mmi_palm_handler(bool value) {
  ...
  if (value) {
    input_report_abs(palm_sensor_pdata->input_sensor_dev, ABS_DISTANCE, 1);
    pr_info("%s: palm report 1\n", __func__);
  } else {
    input_report_abs(palm_sensor_pdata->input_sensor_dev, ABS_DISTANCE, 0);
...
#ifdef TS_MMI_TOUCH_GESTURE_POISON_EVENT
static int ts_mmi_touch_event_poison_slot_handler(struct touch_event_data *tev,
                                                  struct input_dev *input_dev) {
  struct ts_mmi_dev *touch_cdev = events_data->touch_cdev;
    ...
    if (need_handle_poison_event(tev->id)) {
      if (events_data->poison_events[tev->id].is_slot_poisoned) {
        if (ts_mmi_touch_event_is_in_poison_trigger(tev, touch_cdev)) {
          ...
          pr_info("%s: Poison slot(%d) temperary rescued because outof trigger "
                  "area\n",
                  __func__, tev->id);
        }
      } else if (is_far_from_poison_center(tev) &&
                 ts_mmi_touch_event_is_in_suppression(tev, touch_cdev)) {
        update_poison_center(tev);
      } else if (is_stack_at_poison_center(tev) &&
                 ts_mmi_touch_event_is_in_suppression(tev, touch_cdev)) {
        dev_info(DEV_TS,
                 "%s: slot(%d) is stacked near poison center. Mark this slot "
                 "poisoned\n",
                 __func__, tev->id);
...
static int ts_mmi_touch_event_edge_handler(struct touch_event_data *tev,
                                           struct input_dev *input_dev) {
#ifdef TS_MMI_TOUCH_GESTURE_POISON_EVENT
...
static int ts_mmi_touch_event_handler(struct touch_event_data *tev,
  ...
  case TS_COORDINATE_ACTION_PRESS:
    ...
    pr_info("%s: [P]Finger %d: Down, x=%d, y=%d, major=%d, minor=%d", __func__,
            tev->id, tev->x, tev->y, tev->major, tev->minor);
#endif
...
static int ts_mmi_sensor_set_enable(struct sensors_classdev *sensors_cdev,
  ...
  struct ts_mmi_sensor_platform_data *sensor_pdata =
      container_of(sensors_cdev, struct ts_mmi_sensor_platform_data, ps_cdev);
  struct ts_mmi_dev *touch_cdev = sensor_pdata->touch_cdev;
...
int ts_mmi_gesture_init(struct ts_mmi_dev *touch_cdev) {
  ...
  sensor_pdata =
      devm_kzalloc(&sensor_input_dev->dev,
  ...
  events_data =
      devm_kzalloc(DEV_TS, sizeof(struct ts_mmi_touch_events_data), GFP_KERNEL);
  if (!events_data) {
    dev_err(DEV_TS, "%s: Failed to allocate memory", __func__);
  ...
  err =
      sensors_classdev_register(&sensor_input_dev->dev, &sensor_pdata->ps_cdev);
  if (err)
...
int ts_mmi_palm_init(struct ts_mmi_dev *touch_cdev) {
  ...
  __set_bit(EV_ABS, sensor_input_dev->evbit);
  __set_bit(EV_SYN, sensor_input_dev->evbit);
  input_set_abs_params(sensor_input_dev, ABS_DISTANCE, 0, 5, 0, 0);
