...
int ts_mmi_parse_dt(struct ts_mmi_dev *touch_cdev,
  ...
  if (!of_property_read_string(of_node, "mmi,class-entry-name",
                               &ppdata->class_entry_name))
    dev_info(DEV_TS, "%s: class-entry-name property %s\n", __func__,
             ppdata->class_entry_name);

  if (!of_property_read_string(of_node, "mmi,bound-display",
                               &ppdata->bound_display))
  ...
  if (!of_property_read_u32_array(of_node, "mmi,fod_coords", coords, 2)) {
    ...
    dev_info(DEV_TS, "%s: get fod_coords property x:%d y:%d\n", __func__,
             ppdata->fod_x, ppdata->fod_y);
  }

#ifdef CONFIG_BOARD_USES_DOUBLE_TAP_CTRL
  if (!of_property_read_u32(of_node, "mmi,supported_gesture_type",
                            &ppdata->supported_gesture_type))
  ...
  if (!of_property_read_u32_array(of_node, "mmi,clip-area", coords, 4)) {
    ...
    dev_info(DEV_TS, "%s: clip %sarea: (%u,%u) (%u,%u)\n", __func__,
             touch_cdev->clip.inversion ? "IN " : "OUT ", touch_cdev->clip.xUL,
             touch_cdev->clip.yUL, touch_cdev->clip.xBR, touch_cdev->clip.yBR);
  }
...
static int ts_mmi_check_dt(struct ts_mmi_dev *touch_cdev,
                           struct device_node *np) {
...
int ts_mmi_check_drm_panel(struct ts_mmi_dev *touch_cdev,
                           struct device_node *of_node) {
  int ret;
