...
static int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin) {
  ...
  if (cfg_bin->bin_data_len != cfg_bin->head.bin_len) {
    ts_err("cfg_bin len check failed,%d != %d", cfg_bin->head.bin_len,
  ...
  cfg_bin->cfg_pkgs = kzalloc(
      sizeof(struct goodix_cfg_package) * cfg_bin->head.pkg_num, GFP_KERNEL);
  if (!cfg_bin->cfg_pkgs) {
  ...
  for (i = 0; i < cfg_bin->head.pkg_num; i++) {
    ...
    if (i == cfg_bin->head.pkg_num - 1) {
      offset1 =
          cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN] +
          (cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN + 1]
           << 8);
      ...
      offset2 =
          cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN + 2] +
          (cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN + 3]
           << 8);

      if (offset2 <= offset1) {
        ts_err("offset error,pkg:%d, offset1:%d, offset2:%d", i, offset1,
               offset2);
        goto exit;
      }
    ...
    /*get cfg pkg head*/
    memcpy(&cfg_bin->cfg_pkgs[i].cnst_info, &cfg_bin->bin_data[offset1],
...
static int goodix_get_reg_and_cfg(struct goodix_ts_core *cd, u8 sensor_id,
  ...
  for (i = 0; i < cfg_bin->head.pkg_num; i++) {
    ...
    if (sensor_id != cfg_pkg->cnst_info.sensor_id) {
      ts_info("pkg:%d, sensor id contrast FAILED, bin %d != %d", i,
              cfg_pkg->cnst_info.sensor_id, sensor_id);
      continue;
    }
    cfg_type = cfg_pkg->cnst_info.cfg_type;
    if (cfg_type >= GOODIX_MAX_CONFIG_GROUP) {
      ts_err("usupported config type %d", cfg_pkg->cnst_info.cfg_type);
      goto err_out;
    }

    cfg_len = cfg_pkg->pkg_len - TS_PKG_CONST_INFO_LEN - TS_PKG_REG_INFO_LEN;
    if (cfg_len > GOODIX_CFG_MAX_SIZE) {
      ts_err("config len exceed limit %d > %d", cfg_len, GOODIX_CFG_MAX_SIZE);
      goto err_out;
    }
    ...
    ts_info("get config type %d, len %d, for sensor id %d", cfg_type, cfg_len,
            sensor_id);
  }
  return 0;
...
int goodix_get_config_proc(struct goodix_ts_core *cd) {
    ...
    defined(CONFIG_GTP_LIMIT_USE_SUPPLIER)
  if (cd->supplier) {
