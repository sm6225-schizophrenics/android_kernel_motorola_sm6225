...
static int goodix_i2c_read(struct device *dev, unsigned int reg,
  ...
  while (pos != len) {
    ...
    if (unlikely(retry == GOODIX_BUS_RETRY_TIMES)) {
      ts_err("I2c read failed,dev:%02x,reg:%04x,size:%u", client->addr, reg,
             len);
      r = -EAGAIN;
      goto read_exit;
    }
...
static int goodix_i2c_write(struct device *dev, unsigned int reg,
  ...
  while (pos != len) {
    if (unlikely(len - pos > I2C_MAX_TRANSFER_SIZE - GOODIX_REG_ADDR_SIZE))
      transfer_length = I2C_MAX_TRANSFER_SIZE - GOODIX_REG_ADDR_SIZE;
    else
      transfer_length = len - pos;
    ...
    memcpy(&msg.buf[GOODIX_REG_ADDR_SIZE], &data[pos], transfer_length);
    ...
    if (unlikely(retry == GOODIX_BUS_RETRY_TIMES)) {
      ts_err("I2c write failed,dev:%02x,reg:%04x,size:%u", client->addr, reg,
             len);
      r = -EAGAIN;
      goto write_exit;
    }
