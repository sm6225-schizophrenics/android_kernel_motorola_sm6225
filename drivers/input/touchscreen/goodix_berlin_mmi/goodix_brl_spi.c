...
static int goodix_spi_read_bra(struct device *dev, unsigned int addr,
  ...
  if (!rx_buf || !tx_buf) {
    ts_err("alloc tx/rx_buf failed, size:%d", SPI_READ_PREFIX_LEN + len);
    return -ENOMEM;
  }
...
static int goodix_spi_read(struct device *dev, unsigned int addr,
  ...
  if (!rx_buf || !tx_buf) {
    ts_err("alloc tx/rx_buf failed, size:%d", SPI_READ_PREFIX_LEN - 1 + len);
    return -ENOMEM;
  }
...
static int goodix_spi_write(struct device *dev, unsigned int addr,
  ...
  if (!tx_buf) {
    ts_err("alloc tx_buf failed, size:%d", SPI_WRITE_PREFIX_LEN + len);
    return -ENOMEM;
  }
