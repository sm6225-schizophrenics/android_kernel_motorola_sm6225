...
#define MINOR_NUMBER_START                                                     \
  ((const unsigned int)0) /* the minor number starts at */
#define NUMBER_MINOR_NUMBER                                                    \
  ((const unsigned int)1) /* the number of minor numbers */
#define DDA_MAX_BUFFER 32
#define MAX_IO_CONTROL_REPORT 16

/* for DDA goodix_penraw */
enum { DATA_TYPE_RAW = 0 };
...
static long penraw_ioctl(struct file *file, unsigned int cmd,
                         unsigned long arg) {
  unsigned long flags;
  ...
  case PENRAW_GET_VALUES:
    ...
    if (MAX_IO_CONTROL_REPORT <= num) {
      pen_buffer_rp =
          (unsigned char)((wp + (DDA_MAX_BUFFER - MAX_IO_CONTROL_REPORT)) %
                          DDA_MAX_BUFFER);
    } else {
      pen_buffer_rp = 0;
    }
...
void goodix_stylus_dda_init(void) {
  ...
  dda_pen_report_buf.buffer_wp = 0;
  memset(&dda_cdev_ctrl.penraw_char_dev, 0,
         sizeof(dda_cdev_ctrl.penraw_char_dev));
...
int goodix_stylus_dda_register_cdevice(void) {
  ...
  penraw_dev =
      device_create(dda_cdev_ctrl.penraw_char_dev_class, NULL,
