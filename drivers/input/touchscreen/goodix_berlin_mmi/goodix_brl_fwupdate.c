...
static int get_fw_version_info(struct goodix_fw_version *fw_version) {
  ...
  return hw_ops->read_version(goodix_fw_update_ctrl.core_data, fw_version);
}
...
static int goodix_parse_firmware(struct firmware_data *fw_data) {
  ...
  if (firmware->size != fw_summary->size + FW_FILE_CHECKSUM_OFFSET) {
    ts_err("Bad firmware, size not match, %zu != %d", firmware->size,
           fw_summary->size + 6);
    r = -EINVAL;
    goto err_size;
  }
  ...
  if (fw_summary->subsys_num > FW_SUBSYS_MAX_NUM) {
    ts_err("Bad firmware, invalid subsys num: %d", fw_summary->subsys_num);
    r = -EINVAL;
    goto err_size;
  }
  ...
  for (i = 0; i < fw_summary->subsys_num; i++) {
    ...
    ts_debug("Subsystem flash_addr:%08X", fw_summary->subsys[i].flash_addr);
    ts_debug("Subsystem Ptr:%p", fw_summary->subsys[i].data);
  }

  if (fw_summary->chip_type == CHIP_TYPE_BRA && ic_type != IC_TYPE_BERLIN_A) {
...
static int goodix_load_isp(struct firmware_data *fw_data) {
  ...
  r = goodix_reg_write_confirm(isp_ram_reg, (u8 *)fw_isp->data, fw_isp->size);
  if (r < 0) {
  ...
  if (memcmp(&isp_fw_version.patch_pid[3], "ISP", 3)) {
    ts_err("patch id error %c%c%c != %s", isp_fw_version.patch_pid[3],
           isp_fw_version.patch_pid[4], isp_fw_version.patch_pid[5], "ISP");
    return -3;
  }
...
static int goodix_send_flash_cmd(struct goodix_flash_cmd *flash_cmd) {
  ...
  for (i = 0; i < retry; i++) {
    ...
    ts_info("flash cmd ack error retry %d, ack 0x%x, ret %d", i, tmp_cmd.ack,
            ret);
  }
  ...
  for (i = 0; i < retry; i++) {
    ...
    ts_info(
        "flash cmd status not ready, retry %d, ack 0x%x, status 0x%x, ret %d",
...
static int goodix_flash_package(u8 subsys_type, u8 *pkg, u32 flash_addr,
  ...
  do {
    ...
    goodix_append_checksum(&(flash_cmd.buf[2]), 9, CHECKSUM_MODE_U8_LE);
...
static int goodix_flash_subsystem(struct fw_subsys_info *subsys) {
  ...
  while (total_size > 0) {
    ...
    ts_info("Flash firmware to %08x,size:%u bytes", subsys_base_addr + offset,
            data_size);

    memcpy(fw_packet, &subsys->data[offset], data_size);
    /* set checksum for package data */
    goodix_append_checksum(fw_packet, data_size, CHECKSUM_MODE_U16_LE);

    r = goodix_flash_package(subsys->type, fw_packet, subsys_base_addr + offset,
                             data_size + 4);
    if (r) {
      ts_err("failed flash to %08x,size:%u bytes", subsys_base_addr + offset,
             data_size);
      break;
    }
    offset += data_size;
...
static int goodix_flash_firmware(struct fw_update_ctrl *fw_ctrl) {
  ...
  for (i = 1; i < fw_num && retry;) {
    ...
    if (r == 0) {
      ...
      retry--;
      ts_err("--- End flash subsystem%d: Fail, errno:%d, retry:%d ---", i, r,
...
static struct attribute *goodix_fwu_attrs[] = {
    &dev_attr_update_en.attr, &dev_attr_fwsize.attr, &dev_attr_result.attr};
