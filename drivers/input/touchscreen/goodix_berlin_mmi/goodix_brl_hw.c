...
enum { PROCESS_REF = 0, PROCESS_RAW, PROCESS_DIFF, PROCESS_B_ARRAY };
...
static int brl_send_cmd(struct goodix_ts_core *cd, struct goodix_ts_cmd *cmd) {
  ...
  while (retry++ < GOODIX_CMD_RETRY) {
    ...
    for (i = 0; i < GOODIX_CMD_RETRY; i++) {
      ...
      ts_info("cmd ack data %*ph", (int)sizeof(cmd_ack), cmd_ack.buf);
      if (cmd_ack.ack == CMD_ACK_OK) {
        usleep_range(2000, 2100);
        return 0;
      }
      if (cmd_ack.ack == CMD_ACK_BUSY || cmd_ack.ack == 0x00) {
...
#else
static int inline brl_set_display_mode(struct goodix_ts_core *cd, int mode) {
  return -EINVAL;
}
#endif
...
static int wait_cmd_status(struct goodix_ts_core *cd, u8 target_status,
  ...
  ts_err("cmd status not ready, retry %d, ack 0x%x, status 0x%x, ret %d", i,
         cmd_ack.ack, cmd_ack.state, ret);
  return -EINVAL;
}
...
static int brl_send_config(struct goodix_ts_core *cd, u8 *cfg, int len) {
  ...
  if (len > misc->fw_buffer_max_len) {
    ts_err("config len exceed limit %d > %d", len, misc->fw_buffer_max_len);
    return -EINVAL;
  }
...
static int brl_read_config(struct goodix_ts_core *cd, u8 *cfg, int size) {
  ...
  ret = hw_ops->read(cd, misc->fw_buffer_addr, cfg_head.buf, sizeof(cfg_head));
  if (ret) {
  ...
  if (cfg_head.cfg_len > misc->fw_buffer_max_len || cfg_head.cfg_len > size) {
...
static int convert_ic_info(struct goodix_ic_info *info, const u8 *data) {
  ...
  if (parm->active_scan_rate_num > MAX_SCAN_RATE_NUM) {
    ts_err("invalid scan rate num %d > %d", parm->active_scan_rate_num,
           MAX_SCAN_RATE_NUM);
    return -EINVAL;
  }
...
static void print_ic_info(struct goodix_ic_info *ic_info) {
  ...
  ts_info("Cmd:                           0x%04X, %d", misc->cmd_addr,
          misc->cmd_max_len);
  ts_info("Cmd-Reply:                     0x%04X, %d", misc->cmd_reply_addr,
          misc->cmd_reply_len);
  ts_info("FW-State:                      0x%04X, %d", misc->fw_state_addr,
          misc->fw_state_len);
  ts_info("FW-Buffer:                     0x%04X, %d", misc->fw_buffer_addr,
          misc->fw_buffer_max_len);
  ts_info("frame_data_addr:               0x%04x", misc->frame_data_addr);
  ts_info("Touch-Data:                    0x%04X, %d", misc->touch_data_addr,
          misc->touch_data_head_len);
  ts_info("point_struct_len:              %d", misc->point_struct_len);
  ts_info("mutual_rawdata_addr:           0x%04X", misc->mutual_rawdata_addr);
  ts_info("mutual_diffdata_addr:          0x%04X", misc->mutual_diffdata_addr);
  ts_info("self_rawdata_addr:             0x%04X", misc->self_rawdata_addr);
  ts_info("self_diffdata_addr:            0x%04X", misc->self_diffdata_addr);
...
static int goodix_touch_handler(struct goodix_ts_core *cd,
  ...
  if (cd->set_mode.liquid_detection) {
    ...
    if (cd->liquid_status != underwater_flag) {
      ...
      /* call class method */
      cd->imports->report_liquid_detection_status(cd->bus->dev,
                                                  cd->liquid_status ? 1 : 0);
  ...
  if (unlikely(touch_num > 2)) {
    ret = hw_ops->read(cd, misc->touch_data_addr + pre_buf_len,
                       &buffer[pre_buf_len], (touch_num - 2) * BYTES_PER_POINT);
    if (ret) {
  ...
  if (touch_num > 0) {
    ...
    if (point_type == POINT_TYPE_STYLUS ||
      ...
      ret = checksum_cmp(&buffer[IRQ_EVENT_HEAD_LEN], BYTES_PER_POINT * 2 + 2,
                         CHECKSUM_MODE_U8_LE);
      if (ret) {
...
static void goodix_cache_debug_log(struct goodix_ts_core *cd) {
  ...
  u8 frame_type = 0;
  u8 *frame_ptr =
      misc->frame_data_addr - misc->touch_data_addr + cd->trigger_buf;
...
static int brl_event_handler(struct goodix_ts_core *cd,
  ...
  pre_read_len =
      IRQ_EVENT_HEAD_LEN + BYTES_PER_POINT * 2 + COOR_DATA_CHECKSUM_SIZE;
#ifdef CONFIG_GTP_GHOST_LOG_CAPTURE
  if (atomic_read(&cd->trigger_enable) == 1) {
    mutex_lock(&cd->frame_log_lock);
    ret = hw_ops->read(cd, misc->touch_data_addr, cd->trigger_buf,
                       sizeof(cd->trigger_buf));
...
#ifdef CONFIG_GTP_FOD
    memcpy(ts_event->gesture_data, &pre_buf[8], GOODIX_GESTURE_DATA_LEN);
#endif
  } else {
...
struct goodix_ts_hw_ops *goodix_get_hw_ops(void) { return &brl_hw_ops; }
