...
static int __do_register_ext_module(struct goodix_ext_module *module) {
  ...
  if (!list_empty(&goodix_modules.head)) {
    list_for_each_entry_safe(ext_module, next, &goodix_modules.head, list) {
      if (ext_module == module) {
        ts_info("Module [%s] already exists", module->name);
  ...
  if (module->funcs && module->funcs->init) {
    if (module->funcs->init(goodix_modules.core_data, module) < 0) {
      ts_err("Module [%s] init error", module->name ? module->name : " ");
      mutex_unlock(&goodix_modules.mutex);
      return -EFAULT;
    }
  }
  ...
  ts_info("Module [%s] registered,priority:%u", module->name, module->priority);
  return 0;
}
...
static const struct sysfs_ops goodix_ext_ops = {
    .show = goodix_ext_sysfs_show, .store = goodix_ext_sysfs_store};
...
struct kobj_type *goodix_get_default_ktype(void) { return &goodix_ext_ktype; }
...
static ssize_t goodix_ts_chip_info_show(struct device *dev,
  ...
  if (hw_ops->read_version) {
    ...
    if (!ret) {
      ...
      cnt = snprintf(&buf[0], PAGE_SIZE, "rom_pid:%s\nrom_vid:%02x%02x%02x\n",
                     temp_pid, chip_ver.rom_vid[0], chip_ver.rom_vid[1],
                     chip_ver.rom_vid[2]);
      cnt += snprintf(
          &buf[cnt], PAGE_SIZE, "patch_pid:%s\npatch_vid:%02x%02x%02x%02x\n",
          chip_ver.patch_pid, chip_ver.patch_vid[0], chip_ver.patch_vid[1],
          chip_ver.patch_vid[2], chip_ver.patch_vid[3]);
      cnt +=
          snprintf(&buf[cnt], PAGE_SIZE, "sensorid:%d\n", chip_ver.sensor_id);
    }
  }
  ...
  *out_buf_len = m_size;
  ...
  for (i = 0; i < buf_size; i++) {
    ...
    if (high == 0xff || low == 0xff) {
      ts_err("failed convert: 0x%x, 0x%x", buf[i + 1], buf[i + 2]);
      return -EINVAL;
    }
...
static ssize_t goodix_ts_send_cfg_store(struct device *dev,
  ...
  if (ret < 0) {
    ts_err("cfg file [%s] not available,errno:%d", GOODIX_DEFAULT_CFG_NAME,
           ret);
    goto exit;
  } else {
  ...
  if (goodix_ts_convert_0x_data(cfg_img->data, cfg_img->size, config->data,
                                &config->len)) {
...
static ssize_t goodix_ts_reg_rw_show(struct device *dev,
  ...
  if (!rw_addr || !rw_len) {
    ts_err("address(0x%x) and length(%d) can't be null", rw_addr, rw_len);
    return -EINVAL;
  }
...
static ssize_t goodix_ts_reg_rw_store(struct device *dev,
  ...
  if (ret < 0) {
    ts_err("failed write addr(%x) data %*ph", rw_addr, rw_len, store_buf);
    goto err_out;
  }

  ts_info("%s write to addr (%x) with data %*ph", "success", rw_addr, rw_len,
          store_buf);
...
int goodix_ts_blocking_notify(enum ts_notify_event evt, void *v) {
  ...
  ret = blocking_notifier_call_chain(&ts_notifier_list, (unsigned long)evt, v);
  return ret;
}
...
static int goodix_parse_dt(struct device_node *node,
  ...
  r = of_property_read_u32(node, "goodix,irq-flags", &board_data->irq_flags);
  if (r) {
  ...
  if (!r) {
    ...
    if (strlen(name_tmp) < sizeof(board_data->avdd_name))
      strncpy(board_data->avdd_name, name_tmp, sizeof(board_data->avdd_name));
    else
      ts_info("invalied avdd name length: %ld > %ld", strlen(name_tmp),
  ...
  if (!r) {
    ...
    if (strlen(name_tmp) < sizeof(board_data->iovdd_name))
      strncpy(board_data->iovdd_name, name_tmp, sizeof(board_data->iovdd_name));
    else
      ts_info("invalied iovdd name length: %ld > %ld", strlen(name_tmp),
  ...
  board_data->stylus_mode_ctrl =
      of_property_read_bool(node, "goodix,stylus_mode-ctrl");
  if (board_data->stylus_mode_ctrl) {
    ...
    if (!r) {
      ...
      strncpy(board_data->stylus_clk_src, name_tmp,
              sizeof(board_data->stylus_clk_src));
    }
  }

  board_data->sensitivity_ctrl =
      of_property_read_bool(node, "goodix,sensitivity-ctrl");
  if (board_data->sensitivity_ctrl)
    ts_info("support goodix sensitivity mode");

  board_data->leather_mode_ctrl =
      of_property_read_bool(node, "goodix,leather_mode-ctrl");
  if (board_data->leather_mode_ctrl)
    ts_info("support goodix leather mode");

  board_data->film_mode_ctrl =
      of_property_read_bool(node, "goodix,film_mode-ctrl");
  if (board_data->film_mode_ctrl)
    ts_info("support goodix film mode");

  board_data->interpolation_ctrl =
      of_property_read_bool(node, "goodix,interpolation-ctrl");
  if (board_data->interpolation_ctrl)
    ts_info("support goodix interpolation mode");

  board_data->sample_ctrl = of_property_read_bool(node, "goodix,sample-ctrl");
  if (board_data->sample_ctrl)
    ts_info("support goodix sample mode");

  board_data->report_rate_ctrl =
      of_property_read_bool(node, "goodix,report_rate-ctrl");
  if (board_data->report_rate_ctrl)
    ts_info("support goodix report rate switch mode");

  board_data->edge_ctrl = of_property_read_bool(node, "goodix,edge-ctrl");
...
static void goodix_ts_report_pen(struct input_dev *dev,
  ...
  if (pen_data->coords.status == TS_TOUCH) {
    ...
    ts_debug("pen_data:x %d, y %d, p %d, tilt_x %d tilt_y %d key[%d %d]",
             pen_data->coords.x, pen_data->coords.y, pen_data->coords.p,
             pen_data->coords.tilt_x, pen_data->coords.tilt_y,
...
static void goodix_ts_report_finger(struct input_dev *dev,
  ...
  for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
    if (touch_data->coords[i].status == TS_TOUCH) {
      ts_debug("report: id %d, x %d, y %d, w %d", i, touch_data->coords[i].x,
               touch_data->coords[i].y, touch_data->coords[i].w);
      if (touchdown[i] == 0) {
  ...
  ts_debug("fod_enable= %d, gesture_type =%x, touch_num= %d",
           core_data->fod_enable, ts_event->gesture_type, touch_num);
#endif
...
static int goodix_ts_request_handle(struct goodix_ts_core *cd,
  ...
  if (ts_event->request_code == REQUEST_TYPE_CONFIG)
    ...
    ts_info("can not handle request type 0x%x", ts_event->request_code);
  if (ret)
    ts_err("failed handle request 0x%x", ts_event->request_code);
  else
    ts_info("success handle ic request 0x%x", ts_event->request_code);
  return ret;
}
/**
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data) {
  ...
  if (likely(!ret)) {
    ...
    if (core_data->board_data.pen_enable && ts_event->event_type == EVENT_PEN) {
...
#ifdef CONFIG_GTP_ENABLE_PM_QOS
  cpu_latency_qos_update_request(&core_data->goodix_pm_qos,
...
static int goodix_ts_irq_setup(struct goodix_ts_core *core_data) {
  ...
  ret = devm_request_threaded_irq(
      &core_data->pdev->dev, core_data->irq, NULL, goodix_ts_threadirq_func,
...
static int goodix_ts_stylus_clk_init(struct goodix_ts_core *core_data) {
  ...
  if (!strcmp(ts_bdata->stylus_clk_src, STYLUS_CLK_SRC_PMIC)) {
    ...
    core_data->stylus_clk_active =
        pinctrl_lookup_state(core_data->pinctrl, PINCTRL_STYLUS_CLK_ACTIVE);
    ...
    core_data->stylus_clk_suspend =
        pinctrl_lookup_state(core_data->pinctrl, PINCTRL_STYLUS_CLK_SUSPEND);
...
static int goodix_ts_gpio_setup(struct goodix_ts_core *core_data) {
  ...
  r = devm_gpio_request_one(&core_data->pdev->dev, ts_bdata->irq_gpio, GPIOF_IN,
                            "ts_irq_gpio");
  if (r < 0) {
...
static int goodix_ts_input_dev_config(struct goodix_ts_core *core_data) {
  ...
  input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, ts_bdata->panel_max_x,
                       0, 0);
  input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, ts_bdata->panel_max_y,
                       0, 0);
  input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, ts_bdata->panel_max_w,
                       0, 0);
#ifdef CONFIG_ENABLE_GTP_PALM_CANCEL
...
static int goodix_ts_pen_dev_config(struct goodix_ts_core *core_data) {
  ...
  input_set_abs_params(pen_dev, ABS_PRESSURE, 0, ts_bdata->panel_max_p, 0, 0);
...
static int goodix_start_later_init(struct goodix_ts_core *ts_core) {
  ...
  init_thrd =
      kthread_run(goodix_later_init_thread, ts_core, "goodix_init_thread");
  if (IS_ERR_OR_NULL(init_thrd)) {
    ts_err("Failed to create update thread:%ld", PTR_ERR(init_thrd));
    return -EFAULT;
  }
  return 0;
}
...
static int goodix_ts_probe(struct platform_device *pdev) {
  ...
  core_data =
      devm_kzalloc(&pdev->dev, sizeof(struct goodix_ts_core), GFP_KERNEL);
  if (!core_data) {
  ...
  if (IS_ENABLED(CONFIG_OF) && bus_interface->dev->of_node) {
    ...
    ret = goodix_parse_dt(bus_interface->dev->of_node, &core_data->board_data);
    if (ret) {
...
#ifdef GOODIX_PALM_SENSOR_EN
  timer_setup(&core_data->palm_release_timer,
              goodix_palm_sensor_release_timer_handler, 0);
