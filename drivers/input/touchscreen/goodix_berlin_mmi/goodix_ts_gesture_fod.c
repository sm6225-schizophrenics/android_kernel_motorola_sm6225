...
const struct goodix_ext_attribute gesture_attrs[] = {
    ...
    __EXTMOD_ATTR(double_en, 0666, gsx_double_type_show, gsx_double_type_store),
    __EXTMOD_ATTR(single_en, 0666, gsx_single_type_show, gsx_single_type_store),
    __EXTMOD_ATTR(fod_en, 0666, gsx_fod_type_show, gsx_fod_type_store)};
...
static int gsx_gesture_init(struct goodix_ts_core *cd,
  ...
  gsx->ts_core->gesture_type =
      GESTURE_SINGLE_TAP | GESTURE_FOD_PRESS | GESTURE_DOUBLE_TAP;
  cd->zerotap_data[0] = 0;
...
static int gsx_gesture_ist(struct goodix_ts_core *cd,
  ...
  if (cd->board_data.gesture_wait_pm) {
    ...
    ret = wait_event_interruptible_timeout(
        cd->pm_wq, atomic_read(&cd->pm_resume), msecs_to_jiffies(700));
    if (!ret) {
  ...
  if (cd->imports && cd->imports->report_gesture) {
    if (cd->gesture_type & GESTURE_SINGLE_TAP &&
        gs_event.gesture_type == GOODIX_GESTURE_SINGLE_TAP) {
      ...
      if (!ret)
        PM_WAKEUP_EVENT(cd->gesture_wakelock, 3000);
      goto gesture_ist_exit;
    } else if (cd->gesture_type & GESTURE_DOUBLE_TAP &&
               gs_event.gesture_type == GOODIX_GESTURE_DOUBLE_TAP) {
      ...
      // goodix firmware do not send coordinate, need mmi touch to define a
      // vaild coordinate thru dts
      mmi_event.evcode = 2;
      mmi_event.evdata.x = 0;
      mmi_event.evdata.y = 0;

      ts_info("Get FOD-DOWN gesture:%d interval:%d", fod_down,
              fod_down_interval);
      ...
      fod_down++;
               ...
               gs_event.gesture_type == GOODIX_GESTURE_FOD_UP) {
      ...
      ts_debug("not support gesture type[%02X] to wakeup, suspended =%d",
               gs_event.gesture_type, atomic_read(&cd->suspended));
      fod_down = 0;
    }
  }
#else
...
int gesture_module_init(void) {
  ...
  /* gesture sysfs init */
  ret = kobject_init_and_add(&gsx_gesture->module.kobj, def_kobj_type, def_kobj,
                             "gesture");
  if (ret) {
  ...
  for (i = 0; i < ARRAY_SIZE(gesture_attrs) && !ret; i++)
    ret = sysfs_create_file(&gsx_gesture->module.kobj, &gesture_attrs[i].attr);
  if (ret) {
    ts_err("failed create gst sysfs files");
    while (--i >= 0)
      sysfs_remove_file(&gsx_gesture->module.kobj, &gesture_attrs[i].attr);
