...
static struct goodix_ic_report_rate_config report_rate_config_info = {
    ...
    defined(PRODUCT_TUNDRA) || defined(PRODUCT_EQS) ||                         \
    defined(PRODUCT_RTWO) || defined(PRODUCT_BRONCO) ||                        \
    defined(PRODUCT_ZEEKR) || defined(PRODUCT_LYNKCO) ||                       \
    defined(PRODUCT_BANGKK) || defined(PRODUCT_CTWO)
    .rate_config_count = 2,
    .refresh_rate_ctrl = 0,
...
int goodix_ts_mmi_get_report_rate(struct goodix_ts_core *core_data) {
  ...
  ts_debug("refresh_rate_ctrl: %d, interpolation_ctrl: %d, interpolation_flag: "
           "%d, refresh_rate: %d",
           refresh_rate_ctrl, interpolation_ctrl, interpolation_flag,
           refresh_rate);
