...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test) {
  ...
  if (ts_core->supplier)
    snprintf(limit_file, sizeof(limit_file), "%s_%s_test_limits_%d.csv",
             ts_core->supplier, GOODIX_TEST_FILE_NAME,
...
static u32 map_die2pin(struct ts_test_params *test_params, u32 chn_num) {
  ...
  if (chn_num & DRV_CHANNEL_FLAG)
    chn_num =
        (chn_num & ~DRV_CHANNEL_FLAG) + test_params->params_info->max_sen_num;
  ...
  for (i = 0; i < short_ch_num; i++) {
    ...
    for (j = short_die_num + 1; j < max_drv_num; j++) {
      ...
      if (short_r < r_threshold) {
        ...
        goodix_save_short_res(test_params, master_pin_num, slave_pin_num,
                              short_r);
...
static int gdix_check_resistance_to_gnd(struct ts_test_params *test_params,
  ...
  if (r < r_th) {
    ...
    goodix_save_short_res(test_params, pin_num, short_type ? CHN_VDD : CHN_GND,
                          r);
    ts_err("%s%d shortcircut to %s,R=%ldK,R_Threshold=%dK",
           (pin_num & DRV_CHANNEL_FLAG) ? "DRV" : "SEN",
           (pin_num & ~DRV_CHANNEL_FLAG), short_type ? "VDD" : "GND", r, r_th);
...
static int goodix_shortcircut_analysis(struct goodix_ts_test *ts_test) {
  ...
  ts_info("short flag 0x%02x, drv&drv:%d, sen&sen:%d, drv&sen:%d, "
          "drv/GNDVDD:%d, sen/GNDVDD:%d",
          test_result.result, test_result.drv_drv_num, test_result.sen_sen_num,
          test_result.drv_sen_num, test_result.drv_gnd_avdd_num,
...
static void goodix_shortcircut_test(struct goodix_ts_test *ts_test) {
  ...
  if (ic_type == IC_TYPE_BERLIN_A) {
    ...
    test_parm_cmd.data[3] =
        (ts_test->test_params.params_info->short_diffcode_threshold >> 8) &
        0xFF;
    ...
    // test_param[0] = ts_test->test_params.params_info->dft_short_threshold &
    // 0xFF; test_param[1] =
    // (ts_test->test_params.params_info->dft_short_threshold >> 8) & 0xFF;
    // test_param[2] =
    // ts_test->test_params.params_info->short_diffcode_threshold & 0xFF;
    ...
    // test_param[4] = ts_test->test_params.params_info->short_test_dump_num &
    // 0xFF; test_param[5] =
    // (ts_test->test_params.params_info->short_test_dump_num >> 8) & 0xFF;
    // ts_test_write(ts_test, SHORT_TEST_THRESHOLD_REG, test_param,
    // sizeof(test_param));
    status = 0;
...
static int goodix_cache_rawdata(struct goodix_ts_test *ts_test) {
  ...
  for (i = 0; i < TOTAL_FRAME_NUM; i++) {
    ...
    if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
      ...
      ret = ts_test_read(ts_test, data_addr, (u8 *)ts_test->rawdata[i].data,
...
static int goodix_cache_self_rawdata(struct goodix_ts_test *ts_test) {
  ...
  if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
    ...
    ret = ts_test_read(ts_test, data_addr, (u8 *)ts_test->self_rawdata.data,
                       data_size * sizeof(s16));
    if (ret < 0)
      return ret;
  }
...
static int goodix_cache_noisedata(struct goodix_ts_test *ts_test) {
  ...
  for (cnt = 0; cnt < NOISEDATA_TEST_TIMES; cnt++) {
    ...
    if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
      ...
      ret = ts_test_read(ts_test, data_addr, (u8 *)ts_test->noisedata[cnt].data,
                         data_size * sizeof(s16));
      if (ret < 0)
        return ret;
    }
...
static int goodix_cache_self_noisedata(struct goodix_ts_test *ts_test) {
  ...
  if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
    ...
    ret = ts_test_read(ts_test, data_addr, (u8 *)ts_test->self_noisedata.data,
                       data_size * sizeof(s16));
    if (ret < 0)
      return ret;
  }
...
static int goodix_analysis_self_rawdata(struct goodix_ts_test *ts_test) {
  ...
  for (i = 0; i < data_size; i++) {
    ...
    if (val < ts_test->test_params.self_min_limits[i] ||
      ...
      ts_err("self_rawdata isn't in range, val:%d threshold:[%d,%d]", val,
...
static int goodix_analysis_self_noisedata(struct goodix_ts_test *ts_test) {
  ...
  for (i = 0; i < data_size; i++) {
    ...
    if (val > ts_test->test_params.self_noise_threshold) {
      ts_err("self noisedata isn't in range, val:%d threshold:[0,%d]", val,
...
static void goodix_data_statistics(s16 *data, size_t data_size, char *result,
  ...
  if (data_size <= 0 || res_size <= 0) {
    ts_err("input parameter is illegva:data_size=%ld, res_size=%ld", data_size,
           res_size);
    return;
  }
...
static int goodix_save_header(struct goodix_ts_test *ts_test, struct file *fp) {
  ...
  bytes +=
      sprintf(&data[bytes], "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
  ...
  if (ts_test->test_result[GTP_SHORT_TEST]) {
    if (GTP_TEST_PASS == ts_test->test_result[GTP_SHORT_TEST])
      bytes +=
          sprintf(&data[bytes], "<Item name=\"Short Test\" result=\"OK\"/>\n");
    else
      bytes +=
          sprintf(&data[bytes], "<Item name=\"Short Test\" result=\"NG\"/>\n");
  }
...
static int goodix_save_limits(struct goodix_ts_test *ts_test, struct file *fp) {
  ...
  if (ts_test->test_result[GTP_SHORT_TEST]) {
    ...
    for (i = 0; i < ts_test->short_res.short_num; i++) {
      ...
      if (chn1 == CHN_VDD)
        ...
        bytes +=
            sprintf(&data[bytes], "<ShortMess Chn1=\"Tx%d\" ", chn1 & 0x7f);
      else
        bytes +=
            sprintf(&data[bytes], "<ShortMess Chn1=\"Rx%d\" ", chn1 & 0x7f);
      if (chn2 == CHN_VDD)
        bytes += sprintf(&data[bytes],
        ...
        bytes +=
            sprintf(&data[bytes], "Chn2=\"Tx%d\" ShortResistor= \"%dKom\"/>\n",
                    chn2 & 0x7f, r);
      else
        bytes +=
            sprintf(&data[bytes], "Chn2=\"Rx%d\" ShortResistor= \"%dKom\"/>\n",
                    chn2 & 0x7f, r);
    }
  ...
  for (i = 0; i < tx * rx; i++) {
    bytes +=
        sprintf(&data[bytes], "%d,", ts_test->open_res.beyond_max_limit_cnt[i]);
    if ((i + 1) % tx == 0)
      bytes += sprintf(&data[bytes], "\n");
  }
  bytes += sprintf(&data[bytes], "</BeyondRawdataUpperLimitCnt>\n");
  ...
  for (i = 0; i < tx * rx; i++) {
    bytes +=
        sprintf(&data[bytes], "%d,", ts_test->open_res.beyond_min_limit_cnt[i]);
    if ((i + 1) % tx == 0)
      bytes += sprintf(&data[bytes], "\n");
  }
  bytes += sprintf(&data[bytes], "</BeyondRawdataLowerLimitCnt>\n");
  ...
  for (i = 0; i < tx * rx; i++) {
    bytes +=
        sprintf(&data[bytes], "%d,", ts_test->test_params.deviation_limits[i]);
    if ((i + 1) % tx == 0)
      bytes += sprintf(&data[bytes], "\n");
  }
  bytes += sprintf(&data[bytes], "</MaxAccordLimit>\n");
  ...
  if (ts_test->test_result[GTP_SELFCAP_TEST]) {
    ...
    for (i = 0; i < tx + rx; i++) {
      bytes +=
          sprintf(&data[bytes], "%d,", ts_test->test_params.self_max_limits[i]);
      if ((i + 1) % tx == 0)
        bytes += sprintf(&data[bytes], "\n");
    }
    if ((tx + rx) % tx != 0)
    ...
    for (i = 0; i < tx + rx; i++) {
      bytes +=
          sprintf(&data[bytes], "%d,", ts_test->test_params.self_min_limits[i]);
      if ((i + 1) % tx == 0)
        bytes += sprintf(&data[bytes], "\n");
    }
    if ((tx + rx) % tx != 0)
...
static int goodix_save_rawdata(struct goodix_ts_test *ts_test,
  ...
  for (i = 0; i < TOTAL_FRAME_NUM; i++) {
    ...
    bytes += sprintf(&data[bytes],
                     "<DataContent No.=\"%d\" DataCount=\"%d\" Maximum=\"%d\" "
                     "Minimum=\"%d\" Average=\"%d\">\n",
    ...
    bytes += sprintf(&data[bytes],
                     "<RawAccord No.=\"%d\" DataCount=\"%d\" Maximum=\"%d\" "
                     "Minimum=\"%d\" Average=\"%d\">\n",
                     i, len, stat_result[1], stat_result[2], stat_result[0]);
    for (j = 0; j < len; j++) {
...
static int goodix_save_self_data(struct goodix_ts_test *ts_test,
                                 struct file *fp, s16 *src_data, u8 *title,
                                 int len) {
  int i;
  int ret = 0;
  s32 bytes = 0;
  ...
  bytes += sprintf(&data[bytes],
                   "<DataContent No.=\"0\" DataCount=\"%d\" Maximum=\"%d\" "
                   "Minimum=\"%d\" Average=\"%d\">\n",
  ...
  for (i = 0; i < MAX_TEST_ITEMS; i++) {
    ...
    if (ts_test->test_result[i]) {
      if (GTP_TEST_PASS == ts_test->test_result[i])
        goodix_strncatint(ts_test->test_info, i, "%dP-", TS_RAWDATA_RESULT_MAX);
      else
        goodix_strncatint(ts_test->test_info, i, "%dF-", TS_RAWDATA_RESULT_MAX);
    }
  }

  /* calculate rawdata min avg max value*/
  if (ts_test->rawdata[0].size) {
    goodix_data_statistics(ts_test->rawdata[0].data, ts_test->rawdata[0].size,
                           statistics_data, STATISTICS_DATA_LEN);
    goodix_strncat(ts_test->test_info, statistics_data, TS_RAWDATA_RESULT_MAX);
  } else {
    ts_err("NO valiable rawdata");
    goodix_strncat(ts_test->test_info, "[0,0,0]", TS_RAWDATA_RESULT_MAX);
  }
