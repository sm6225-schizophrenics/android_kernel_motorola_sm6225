...
static DEVICE_ATTR(edge, (S_IRUGO | S_IWUSR | S_IWGRP), goodix_ts_edge_show,
                   goodix_ts_edge_store);
static DEVICE_ATTR(interpolation, (S_IRUGO | S_IWUSR | S_IWGRP),
                   goodix_ts_interpolation_show, goodix_ts_interpolation_store);
static DEVICE_ATTR(sample, (S_IRUGO | S_IWUSR | S_IWGRP), goodix_ts_sample_show,
                   goodix_ts_sample_store);
static DEVICE_ATTR(stylus_mode, (S_IRUGO | S_IWUSR | S_IWGRP),
                   goodix_ts_stylus_mode_show, goodix_ts_stylus_mode_store);
static DEVICE_ATTR(sensitivity, (S_IRUGO | S_IWUSR | S_IWGRP), NULL,
                   goodix_ts_sensitivity_store);
#ifdef CONFIG_GTP_LAST_TIME
...
static int
goodix_ts_mmi_extend_attribute_group(struct device *dev,
...
int goodix_ts_send_cmd(struct goodix_ts_core *core_data, u8 cmd, u8 len,
                       u8 subCmd, u8 subCmd2) {
  int ret = 0;
...
static int goodix_ts_mmi_set_report_rate(struct goodix_ts_core *core_data) {
  ...
  if ((((core_data->set_mode.report_rate_mode >> 8) & 0xFF) ==
       REPORT_RATE_CMD_HIGH) &&
  ...
  // send switch command
  ret = goodix_ts_send_cmd(core_data, mode >> 8, 5, mode & 0xFF, 0x00);
  if (ret < 0) {
  ...
  ts_info("Success to set %s\n",
          mode == REPORT_RATE_CMD_240HZ
              ? "REPORT_RATE_240HZ"
              : (mode == REPORT_RATE_CMD_360HZ
                     ? "REPORT_RATE_360HZ"
                     : (mode == REPORT_RATE_CMD_480HZ
                            ? "REPORT_RATE_480HZ"
                            : (mode == REPORT_RATE_CMD_576HZ
                                   ? "REPORT_RATE_576HZ"
                                   : (mode == REPORT_RATE_CMD_720HZ
                                          ? "REPORT_RATE_720HZ"
                                          : "Unsupported")))));
...
static ssize_t goodix_ts_edge_store(struct device *dev,
  ...
  ret = goodix_ts_send_cmd(core_data, EDGE_SWITCH_CMD, 6, edge_cmd[0],
                           edge_cmd[1]);
  if (ret < 0) {
  ...
  msleep(20);
  ret = size;
  ts_info("Success to set edge = %02x, rotation = %02x", edge_cmd[1],
          edge_cmd[0]);
exit:
  mutex_unlock(&core_data->mode_lock);
  return ret;
...
static ssize_t goodix_ts_edge_show(struct device *dev,
  ...
  return scnprintf(buf, PAGE_SIZE, "0x%02x 0x%02x",
                   core_data->set_mode.edge_mode[1],
                   core_data->set_mode.edge_mode[0]);
}
...
static int goodix_ts_mmi_methods_get_productinfo(struct device *dev,
                                                 void *cdata) {
  struct platform_device *pdev;
  struct goodix_ts_core *core_data;
  struct goodix_ts_board_data *ts_bdata;
...
static int goodix_ts_mmi_methods_get_build_id(struct device *dev, void *cdata) {
  ...
  return snprintf(TO_CHARP(cdata), TS_MMI_MAX_ID_LEN, "%04x",
                  le32_to_cpu(cfg_id));
}

#define TOUCH_FW_VERSION_ADDR 0x1007E
/*return firmware version*/
static int goodix_ts_mmi_methods_get_config_id(struct device *dev,
                                               void *cdata) {
  int ret;
  struct platform_device *pdev;
  struct goodix_ts_core *core_data;
  ...
  ret = core_data->hw_ops->read(core_data, TOUCH_FW_VERSION_ADDR, fw_id,
                                sizeof(fw_id));
  if (ret) {
    ts_info("failed get fw version data, %d", ret);
    return -EINVAL;
...
static int goodix_ts_mmi_methods_get_bus_type(struct device *dev, void *idata) {
  ...
  TO_INT(idata) = core_data->bus->bus_type == GOODIX_BUS_TYPE_I2C
                      ? TOUCHSCREEN_MMI_BUS_TYPE_I2C
                      : TOUCHSCREEN_MMI_BUS_TYPE_SPI;
  return 0;
}

static int goodix_ts_mmi_methods_get_irq_status(struct device *dev,
                                                void *idata) {
  struct platform_device *pdev;
  struct goodix_ts_core *core_data;
  struct goodix_ts_board_data *ts_bdata;
...
static int goodix_ts_mmi_methods_get_flashprog(struct device *dev,
                                               void *idata) {
  struct platform_device *pdev;
  struct goodix_ts_core *core_data;
...
static int goodix_ts_firmware_update(struct device *dev, char *fwname) {
  ...
  ret = goodix_do_fw_update(core_data->ic_configs[CONFIG_TYPE_NORMAL],
                            UPDATE_MODE_SRC_REQUEST | UPDATE_MODE_BLOCK |
                                UPDATE_MODE_FORCE);
  if (ret)
...
static int goodix_berlin_gesture_setup(struct goodix_ts_core *core_data) {
  ...
  */
  ...
  if (core_data->imports && core_data->imports->get_gesture_type) {
    ret = core_data->imports->get_gesture_type(core_data->bus->dev,
                                               &gesture_type);
...
static int goodix_ts_mmi_panel_state(struct device *dev,
                                     enum ts_mmi_pm_mode from,
                                     enum ts_mmi_pm_mode to) {
  struct platform_device *pdev;
  struct goodix_ts_core *core_data;
...
static int goodix_ts_mmi_post_resume(struct device *dev) {
  ...
  if (core_data->board_data.stylus_mode_ctrl &&
    ...
    if (!ret) {
      ...
      ts_info("Success to %s stylus mode",
              core_data->get_mode.stylus_mode ? "Enable" : "Disable");
    }
  }

  if (core_data->board_data.interpolation_ctrl &&
      core_data->get_mode.interpolation) {
    // send switch command
    ret = goodix_ts_send_cmd(
        core_data, (core_data->get_mode.report_rate_mode) >> 8, 5,
        (core_data->get_mode.report_rate_mode) & 0xFF, 0x00);
    if (!ret) {
      core_data->set_mode.interpolation = core_data->get_mode.interpolation;
      core_data->set_mode.report_rate_mode =
          core_data->get_mode.report_rate_mode;
      msleep(20);

      ts_info(
          "Success to %s interpolation mode\n",
          core_data->get_mode.report_rate_mode == REPORT_RATE_CMD_240HZ
              ? "REPORT_RATE_240HZ"
              : (core_data->get_mode.report_rate_mode == REPORT_RATE_CMD_360HZ
                     ? "REPORT_RATE_360HZ"
                     : (core_data->get_mode.report_rate_mode ==
                                REPORT_RATE_CMD_480HZ
                            ? "REPORT_RATE_480HZ"
                            : (core_data->get_mode.report_rate_mode ==
                                       REPORT_RATE_CMD_576HZ
                                   ? "REPORT_RATE_576HZ"
                                   : (core_data->get_mode.report_rate_mode ==
                                              REPORT_RATE_CMD_720HZ
                                          ? "REPORT_RATE_720HZ"
                                          : "Unsupported")))));
    }
  }
  ...
  if (core_data->board_data.leather_mode_ctrl &&
      core_data->get_mode.leather_mode) {
    ...
    if (!ret) {
      ...
      msleep(20);
      ts_info("Success to %s leather mode",
              core_data->get_mode.leather_mode ? "Enable" : "Disable");
    }
  }
  ...
  if (core_data->board_data.film_mode_ctrl && core_data->get_mode.film_mode) {
    ...
    if (!ret) {
      ...
      msleep(20);
      ts_info("Success to %s film mode",
              core_data->get_mode.film_mode ? "Enable" : "Disable");
    }
  }

  if (core_data->board_data.edge_ctrl) {
    ret = goodix_ts_send_cmd(core_data, EDGE_SWITCH_CMD, 6,
                             core_data->get_mode.edge_mode[0],
                             core_data->get_mode.edge_mode[1]);
    if (!ret) {
  ...
  if (core_data->get_mode.liquid_detection) {
    ...
    if (!ret) {
      core_data->set_mode.liquid_detection =
  ...
  if (core_data->zerotap_data[0]) {
    ts_info("FOD is down during PM resume  fod_enable=%d",
            core_data->fod_enable);
  }
...
static int goodix_ts_mmi_update_liquid_detect_mode(struct device *dev,
                                                   int mode) {
  int ret = 0;
  struct platform_device *pdev;
  struct goodix_ts_core *core_data;
  ...
  if (dmode >= 0) {
    ...
    if (!ret)
      ts_info("set active region: %d %d %d %d; dmode=%d\n", rdArray[0],
              rdArray[1], rdArray[Y_MIN_ID], rdArray[Y_MAX_ID], dmode);
  } else
...
static int goodix_ts_mmi_methods_get_active_region(struct device *dev,
                                                   void *uidata) {
  struct goodix_ts_core *core_data;
  struct platform_device *pdev;
...
int goodix_ts_mmi_dev_register(struct platform_device *pdev) {
  ...
  if (core_data->imports && core_data->imports->get_supplier) {
    ret = core_data->imports->get_supplier(core_data->bus->dev,
                                           &core_data->supplier);
  }
#endif
  return 0;
