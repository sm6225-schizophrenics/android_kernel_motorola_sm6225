...
static __poll_t log_file_poll(struct file *file, struct poll_table_struct *pt) {
...
static ssize_t log_file_read(struct file *file, char __user *buffer,
  ...
  if (kfifo_is_empty(&log->fifo)) {
    if (file->f_flags & O_NONBLOCK)
      return -EAGAIN;
    ret = wait_event_interruptible(log->wq, !kfifo_is_empty(&log->fifo));
    if (ret == -ERESTARTSYS)
      return -EINTR;
  }
...
int goodix_log_capture_register_misc(struct goodix_ts_core *cd) {
  ...
  rc = kfifo_alloc(&ts_log_dev.fifo, LOG_FIFO_SIZE, GFP_KERNEL);
  if (rc)
    return rc;
