...
static int tcpc_check_notify_time(struct tcpc_device *tcpc,
                                  struct tcp_notify *tcp_noti, uint8_t type,
                                  uint8_t state) {
  struct tcp_notify_work *tn_work;
...
int tcpci_set_cc(struct tcpc_device *tcpc, int pull) {
  ...
  if (pull == TYPEC_CC_DRP && tcpc->typec_legacy_cable) {
    ...
    pull = TYPEC_CC_PULL(
        rp_lvl == TYPEC_RP_DFT ? tcpc->typec_local_rp_level : rp_lvl, res);
#endif /* CONFIG_TYPEC_LEGACY3_ALWAYS_LOCAL_RP */
  }
#endif /* CONFIG_TYPEC_CHECK_LEGACY_CABLE */
...
int tcpci_set_vconn(struct tcpc_device *tcpc, int enable) {
  ...
  tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_VBUS,
...
int tcpci_notify_typec_state(struct tcpc_device *tcpc) {
  ...
  ret = tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_USB,
...
int tcpci_notify_role_swap(struct tcpc_device *tcpc, uint8_t event,
  ...
  ret = tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_MISC, event);
  return ret;
}
...
int tcpci_source_vbus(struct tcpc_device *tcpc, uint8_t type, int mv, int ma) {
...
#ifdef CONFIG_USB_POWER_DELIVERY
  if (type >= TCP_VBUS_CTRL_PD && tcpc->pd_port.pe_data.pd_prev_connected)
  ...
  ret = tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_VBUS,
...
int tcpci_notify_attachwait_state(struct tcpc_device *tcpc, bool as_sink) {
  ...
  ret = tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_VBUS, notify);
  return ret;
#else
  return 0;
...
static int __tcpci_enable_force_discharge(struct tcpc_device *tcpc, bool en,
                                          int mv) {
  int ret = 0;
...
int tcpci_notify_hard_reset_state(struct tcpc_device *tcpc, uint8_t state) {
  ...
  ret = tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_MISC,
...
int tcpci_enter_mode(struct tcpc_device *tcpc, uint16_t svid, uint8_t ops,
  ...
  ret = tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_MODE,
...
int tcpci_report_hpd_state(struct tcpc_device *tcpc, uint32_t dp_status) {
  ...
  if (PD_DP_CFG_DFP_D(dp_data->local_config)) {
    ...
    tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_MODE,
...
int tcpci_notify_uvdm(struct tcpc_device *tcpc, bool ack) {
  ...
  tcpc_check_notify_time(tcpc, &tcp_noti, TCP_NOTIFY_IDX_MODE, TCP_NOTIFY_UVDM);
  return 0;
}
