...
struct rt_regmap_ops {
  int (*regmap_block_write)(struct rt_regmap_device *rd, u32 reg, int bytes,
                            const void *src);
  int (*regmap_block_read)(struct rt_regmap_device *rd, u32 reg, int bytes,
                           void *dst);
};
...
struct rt_regmap_device {
  ...
  int (*rt_block_write[4])(struct rt_regmap_device *rd,
                           ...
                           const unsigned char *wdata, int count, int cache_idx,
                           int cache_offset);
#ifdef CONFIG_DEBUG_FS
...
static struct reg_index_offset
find_register_index(const struct rt_regmap_device *rd, u32 reg) {
...
static int rt_chip_block_write(struct rt_regmap_device *rd, u32 reg, int bytes,
                               const void *src);
...
void rt_regmap_cache_write_back(struct rt_regmap_device *rd, u32 reg) {
  ...
  for (j = 0; j < rm->size; j++) {
    if (!test_bit(j, &rd->cache_dirty[rio.index]))
      continue;
    ret =
        rt_chip_block_write(rd, rm->addr + j, 1, rd->cache_data[rio.index] + j);
    if (ret < 0) {
      dev_notice(&rd->dev, "%s block write fail(%d) @ 0x%02x\n", __func__, ret,
                 rm->addr + j);
...
static int rt_chip_block_read(struct rt_regmap_device *rd, u32 reg, int bytes,
                              void *dst) {
...
static int _rt_cache_block_write(struct rt_regmap_device *rd, u32 reg,
  ...
  for (i = rio.index, j = rio.offset, count = 0;
       i < rd->props.register_num && count < bytes; i++, j = 0, count += size) {
    rm = rd->props.rm[i];
...
static int rt_cache_block_write(struct rt_regmap_device *rd, u32 reg, int bytes,
                                const void *src) {
...
static int rt_cache_block_read(struct rt_regmap_device *rd, u32 reg, int bytes,
                               void *dest) {
...
static int _rt_regmap_reg_write(struct rt_regmap_device *rd,
  ...
  down(&rd->semaphore);
  ret = (asyn ? rt_asyn_cache_block_write : rd->regmap_ops.regmap_block_write)(
...
int rt_regmap_reg_read(struct rt_regmap_device *rd, struct rt_reg_data *rrd,
                       u32 reg) {
  rrd->reg = reg;
 ...
 */
int rt_regmap_block_write(struct rt_regmap_device *rd, u32 reg, int bytes,
                          const void *src) {
  int ret = 0;
...
int rt_asyn_regmap_block_write(struct rt_regmap_device *rd, u32 reg, int bytes,
                               const void *src) {
  int ret = 0;
 ...
 */
int rt_regmap_block_read(struct rt_regmap_device *rd, u32 reg, int bytes,
                         void *dst) {
  int ret = 0;
...
static int rt_regmap_cache_init(struct rt_regmap_device *rd) {
  ...
  down(&rd->semaphore);
  rd->cache_data = devm_kzalloc(
      &rd->dev, rd->props.register_num * sizeof(*rd->cache_data), GFP_KERNEL);
  rd->cached = devm_kzalloc(
      &rd->dev, rd->props.register_num * sizeof(*rd->cached), GFP_KERNEL);
  rd->cache_dirty = devm_kzalloc(
      &rd->dev, rd->props.register_num * sizeof(*rd->cache_dirty), GFP_KERNEL);
  ...
  if (rd->props.group == NULL) {
    rd->props.group =
        devm_kzalloc(&rd->dev, sizeof(*rd->props.group) * 2, GFP_KERNEL);
    if (!rd->props.group) {
      ret = -ENOMEM;
      goto out;
    }
...
#define erro_printf(rd, fmt, ...)                                              \
  do {                                                                         \
    ...
    ret = snprintf(rd->err_msg + len, ERR_MSG_SIZE - len, fmt, ##__VA_ARGS__); \
    rd->error_occurred = 1;                                                    \
    up(&rd->semaphore);                                                        \
...
static int get_data(const char *buf, size_t count, unsigned char *data_buffer,
...
static void rt_show_regs(struct rt_regmap_device *rd,
  ...
  for (i = 0; i < rd->props.register_num; i++) {
    down(&rd->semaphore);
    ret = rd->regmap_ops.regmap_block_read(rd, rm[i]->addr, rm[i]->size,
                                           rd->regval);
    up(&rd->semaphore);
    if (ret < 0) {
...
static int general_read(struct seq_file *seq_file, void *_data) {
  ...
  case RT_DBG_DATA:
    ...
    if (rd->dbg_data.rio.index < 0)
      ...
      ret = rd->regmap_ops.regmap_block_read(rd, rd->dbg_data.reg_addr, size,
                                             rd->regval);
    up(&rd->semaphore);
    if (ret < 0) {
      erro_printf(rd, "%s block read fail(%d) @ 0x%02x\n", __func__, ret,
...
static ssize_t general_write(struct file *file, const char __user *ubuf,
  ...
  int ret = 0;
  struct rt_debug_st *st = ((struct seq_file *)file->private_data)->private;
  struct rt_regmap_device *rd = st->info;
  struct reg_index_offset rio = {-1, -1};
  char lbuf[128];
  ssize_t res = 0;
  ...
  *ppos = 0;
  ...
  case RT_DBG_DATA:
    ...
    if (rd->dbg_data.rio.index < 0)
      ...
      ret = rd->regmap_ops.regmap_block_write(rd, rd->dbg_data.reg_addr, size,
                                              rd->regval);
    up(&rd->semaphore);
    if (ret < 0) {
...
#define RT_CREATE_GENERAL_FILE(_id, _name, _mode)                              \
  {                                                                            \
    ...
    rd->rt_debug_file[_id] = debugfs_create_file(                              \
        _name, _mode, dir, &rd->rtdbg_st[_id], &general_ops);                  \
...
static ssize_t eachreg_read(struct file *file, char __user *ubuf, size_t count,
                            loff_t *ppos) {
  int ret = 0, i = 0, j = 0;
...
static int rt_create_every_debug(struct rt_regmap_device *rd,
  ...
  rd->rt_reg_file = devm_kzalloc(
      &rd->dev, rd->props.register_num * sizeof(*rd->rt_reg_file), GFP_KERNEL);
  if (!rd->rt_reg_file)
    return -ENOMEM;

  rd->reg_st = devm_kzalloc(
      &rd->dev, rd->props.register_num * sizeof(*rd->reg_st), GFP_KERNEL);
  if (!rd->reg_st)
    return -ENOMEM;
  ...
  for (i = 0; i < rd->props.register_num; i++) {
    ...
    rd->rt_reg_file[i] =
        devm_kzalloc(&rd->dev, sizeof(*rd->rt_reg_file[i]), GFP_KERNEL);
    rd->reg_st[i] = devm_kzalloc(&rd->dev, sizeof(*rd->reg_st[i]), GFP_KERNEL);
    ...
    rd->reg_st[i]->id = i;
    rd->rt_reg_file[i] =
        debugfs_create_file(buf, 0444, dir, rd->reg_st[i], &eachreg_ops);
    if (!rd->rt_reg_file[i])
      return -EINVAL;
  }
