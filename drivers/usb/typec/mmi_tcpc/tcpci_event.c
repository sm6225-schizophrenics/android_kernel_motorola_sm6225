...
static void __pd_free_event(struct tcpc_device *tcpc,
...
static bool __pd_get_event(struct tcpc_device *tcpc,
                           struct pd_event *pd_event) {
  int index = 0;
  ...
  *pd_event = tcpc->pd_event_ring_buffer[tcpc->pd_event_head_index];
...
static inline void pd_get_attention_event(struct tcpc_device *tcpc,
                                          struct pd_event *pd_event) {
...
static inline bool reset_pe_vdm_state(struct tcpc_device *tcpc,
                                      uint32_t vdm_hdr) {
...
static inline bool pd_is_init_attention_event(struct tcpc_device *tcpc,
                                              struct pd_event *pd_event) {
...
bool pd_put_vdm_event(struct tcpc_device *tcpc, struct pd_event *pd_event,
  ...
  if (tcpc->pd_pending_vdm_event && (!ignore_evt)) {
    ...
    if (from_port_partner) {
      if (pd_event_ctrl_msg_match(&tcpc->pd_vdm_event, PD_CTRL_GOOD_CRC)) {
...
static bool __pd_get_deferred_tcp_event(struct tcpc_device *tcpc,
  ...
  *tcp_event = tcpc->tcp_event_ring_buffer[tcpc->tcp_event_head_index];
...
static bool __pd_put_deferred_tcp_event(struct tcpc_device *tcpc,
...
static inline bool __pd_put_hw_event(struct tcpc_device *tcpc,
                                     uint8_t hw_event) {
  struct pd_event evt = {
      .event_type = PD_EVT_HW_MSG,
      .msg = hw_event,
...
static inline bool __pd_put_pe_event(struct tcpc_device *tcpc,
                                     uint8_t pe_event) {
  struct pd_event evt = {
      .event_type = PD_EVT_PE_MSG,
      .msg = pe_event,
...
#ifdef CONFIG_USB_POWER_DELIVERY
#ifdef CONFIG_USB_PD_WAIT_BC12
  rv = power_supply_get_property(tcpc->usb_psy, POWER_SUPPLY_PROP_REAL_TYPE,
                                 &val);
...
void pd_put_cc_detached_event(struct tcpc_device *tcpc) {
  ...
  tcpci_notify_hard_reset_state(tcpc, TCP_HRESET_RESULT_FAIL);
...
void pd_put_recv_hard_reset_event(struct tcpc_device *tcpc) {
  ...
  tcpci_notify_hard_reset_state(tcpc, TCP_HRESET_SIGNAL_RECV);

  tcpc->pd_transmit_state = PD_TX_STATE_HARD_RESET;

  if ((!tcpc->pd_hard_reset_event_pending) && (!tcpc->pd_wait_pe_idle) &&
      tcpc->pd_pe_running) {
...
void pd_notify_pe_hard_reset_completed(struct pd_port *pd_port) {
  ...
  mutex_lock(&tcpc->access_lock);
  tcpci_notify_hard_reset_state(tcpc, TCP_HRESET_RESULT_DONE);
  mutex_unlock(&tcpc->access_lock);
}
...
void pd_notify_pe_cancel_pr_swap(struct pd_port *pd_port) {
  ...
  if (!tcpci_check_vbus_valid(tcpc) && (pd_port->request_v >= 4000)) {
...
int tcpci_event_init(struct tcpc_device *tcpc) {
  ...
  tcpc->event_task =
      kthread_run(tcpc_event_thread_fn, tcpc, "tcpc_event_%s", tcpc->desc.name);
