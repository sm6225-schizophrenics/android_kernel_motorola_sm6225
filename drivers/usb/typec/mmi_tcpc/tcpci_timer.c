...
static inline void tcpc_clear_timer_enable_mask(struct tcpc_device *tcpc,
                                                int nr) {
  unsigned long flags;
...
static inline void tcpc_set_timer_enable_mask(struct tcpc_device *tcpc,
                                              int nr) {
  unsigned long flags;
...
#define DECL_TCPC_TIMEOUT(enum, ms) TIMEOUT_VAL(ms)

#define DECL_TCPC_TIMEOUT_US(enum, us) TIMEOUT_VAL_US(us)

#define DECL_TCPC_TIMEOUT_RANGE(enum, min, max) TIMEOUT_RANGE(min, max)
...
static const uint32_t tcpc_timer_timeout[PD_TIMER_NR] = {
    ...
    DECL_TCPC_TIMEOUT(PD_TIMER_VBUS_PRESENT, CONFIG_USB_PD_VBUS_PRESENT_TOUT),
    ...
    DECL_TCPC_TIMEOUT(TYPEC_RT_TIMER_DISCHARGE,
                      CONFIG_TYPEC_CAP_DISCHARGE_TOUT),
...
#ifdef CONFIG_USB_POWER_DELIVERY
static inline void on_pe_timer_timeout(struct tcpc_device *tcpc,
                                       uint32_t timer_id) {
...
static enum hrtimer_restart
tcpc_timer_source_capability(struct hrtimer *timer) {
...
static enum hrtimer_restart
tcpc_timer_source_transition(struct hrtimer *timer) {
...
static enum hrtimer_restart
tcpc_timer_rt_role_swap_start(struct hrtimer *timer) {
...
static enum hrtimer_restart
tcpc_timer_rt_role_swap_stop(struct hrtimer *timer) {
...
static enum hrtimer_restart
tcpc_timer_rt_legacy_recycle(struct hrtimer *timer) {
...
static enum hrtimer_restart
tcpc_timer_rt_low_power_mode(struct hrtimer *timer) {
...
static enum alarmtimer_restart tcpc_timer_wakeup(struct alarm *alarm,
                                                 ktime_t now) {
  struct tcpc_device *tcpc =
...
static inline void tcpc_reset_timer_range(struct tcpc_device *tcpc, int start,
                                          int end) {
  int i;
  uint64_t mask;
...
int tcpci_timer_init(struct tcpc_device *tcpc) {
  ...
  tcpc->timer_task =
      kthread_run(tcpc_timer_thread_fn, tcpc, "tcpc_timer_%s", tcpc->desc.name);
