...
static inline bool pd_process_ctrl_msg(struct pd_port *pd_port,
  ...
  case PD_CTRL_GOOD_CRC:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_SOFT_RESET,
                                        PE_SNK_WAIT_FOR_CAPABILITIES);

  case PD_CTRL_GOTO_MIN:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_READY, PE_SNK_TRANSITION_SINK))
      return true;
    break;
  ...
  case PD_CTRL_GET_SOURCE_CAP:
    if (pd_process_ctrl_msg_get_source_cap(pd_port, PE_DR_SNK_GIVE_SOURCE_CAP))
      return true;
    break;

  case PD_CTRL_GET_SINK_CAP:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_READY, PE_SNK_GIVE_SINK_CAP))
      return true;
    break;
  ...
  case PD_CTRL_GET_SOURCE_CAP_EXT:
    if (pd_process_ctrl_msg_get_source_cap(pd_port,
  ...
  case PD_CTRL_GET_STATUS:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_READY, PE_SNK_GIVE_SINK_STATUS))
      return true;
    break;
...
static inline bool pd_process_data_msg(struct pd_port *pd_port,
  ...
  case PD_DATA_SINK_CAP:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_DR_SNK_GET_SINK_CAP, PE_SNK_READY))
      return true;
    break;
#endif /* CONFIG_USB_PD_PR_SWAP */
...
static inline bool pd_process_ext_msg(struct pd_port *pd_port,
  ...
  case PD_EXT_SOURCE_CAP_EXT:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_GET_SOURCE_CAP_EXT, PE_SNK_READY))
      return true;
    break;
...
#ifdef CONFIG_USB_PD_REV30_STATUS_LOCAL
  case PD_EXT_STATUS:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_GET_SOURCE_STATUS, PE_SNK_READY))
      return true;
    break;
#endif /* CONFIG_USB_PD_REV30_STATUS_LOCAL */
  ...
  case PD_EXT_PPS_STATUS:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_GET_PPS_STATUS, PE_SNK_READY))
      return true;
    break;
...
static inline bool pd_process_hw_msg(struct pd_port *pd_port,
  ...
  case PD_HW_VBUS_PRESENT:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_DISCOVERY,
                                        PE_SNK_WAIT_FOR_CAPABILITIES);
...
static inline bool pd_process_pe_msg(struct pd_port *pd_port,
  ...
  case PD_PE_RESET_PRL_COMPLETED:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_STARTUP, PE_SNK_DISCOVERY);

  case PD_PE_HARD_RESET_COMPLETED:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_HARD_RESET,
                                        PE_SNK_TRANSITION_TO_DEFAULT);

  case PD_PE_POWER_ROLE_AT_DEFAULT:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_TRANSITION_TO_DEFAULT,
                                        PE_SNK_STARTUP);
...
static inline bool pd_process_timer_msg(struct pd_port *pd_port,
  ...
  case PD_TIMER_SINK_REQUEST:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SNK_READY, PE_SNK_SELECT_CAPABILITY);
