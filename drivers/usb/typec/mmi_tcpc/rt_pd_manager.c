...
static void mmi_ignore_require_dpdm(struct rt_pd_manager_data *rpmd,
                                    bool value) {
  int rc = 0;
...
static inline void start_usb_peripheral(struct rt_pd_manager_data *rpmd) {
  ...
  val.intval = tcpm_inquire_cc_polarity(rpmd->tcpc);
  extcon_set_property(rpmd->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_POLARITY,
                      val);
...
static void usb_dwork_handler(struct work_struct *work) {
  ...
  case DR_MAX:
    ...
    break;
  case DR_DEVICE:
    ret = mmi_get_psy_iio_property(rpmd, POWER_SUPPLY_IIO_USB_REAL_TYPE, &val);
    if (ret) {
      dev_info(rpmd->dev, "%s charge type is failed\n", __func__);
      val.intval = POWER_SUPPLY_TYPE_UNKNOWN;
    }
    dev_info(rpmd->dev, "%s polling_cnt = %d, ret = %d type = %d\n", __func__,
             ++rpmd->usb_type_polling_cnt, ret, val.intval);
...
static void sink_vbus_set_vol_and_cur(struct rt_pd_manager_data *rpmd, int mv,
                                      int ma, uint8_t type) {
  /* ***
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event,
  ...
  case TCP_NOTIFY_SINK_VBUS:
    ...
    dev_info(rpmd->dev, "%s sink vbus %dmV %dmA type(0x%02X)\n", __func__,
             rpmd->sink_mv_new, rpmd->sink_ma_new, noti->vbus_state.type);
    ...
    sink_vbus_set_vol_and_cur(rpmd, rpmd->sink_mv_new, rpmd->sink_ma_new,
  ...
  case TCP_NOTIFY_SOURCE_VBUS:
    ...
    mmi_set_psy_iio_property(rpmd, POWER_SUPPLY_IIO_OTG_ENABLE, &val);
    break;
  case TCP_NOTIFY_TYPEC_STATE:
    ...
    if (old_state == TYPEC_UNATTACHED &&
      ...
      dev_info(rpmd->dev, "%s Charger plug in, polarity = %d, remote rp = %d\n",
      ...
      mmi_set_psy_iio_property(rpmd, POWER_SUPPLY_IIO_TYPEC_MODE, &val);
    } else if (old_state == TYPEC_UNATTACHED &&
               (new_state == TYPEC_ATTACHED_SRC ||
                new_state == TYPEC_ATTACHED_DEBUG)) {
      dev_info(rpmd->dev, "%s OTG plug in, polarity = %d, local_rp = %d\n",
               __func__, noti->typec_state.polarity,
    ...
    if (new_state == TYPEC_UNATTACHED) {
      ...
      rpmd->partner =
          typec_register_partner(rpmd->typec_port, &rpmd->partner_desc);
  ...
  case TCP_NOTIFY_PD_STATE:
    ...
    case PD_CONNECT_NONE:
      ...
      mmi_set_psy_iio_property(rpmd, POWER_SUPPLY_IIO_PD_ACTIVE, &val);
      break;
  ...
  case TCP_NOTIFY_PD_VDM_VERIFY:
    dev_info(rpmd->dev, "%s mmi pd vdm verify state = %d\n", __func__,
             noti->pd_state.vdm_verify);
    val.intval = noti->pd_state.vdm_verify;
    mmi_set_psy_iio_property(rpmd, POWER_SUPPLY_IIO_MMI_PD_VDM_VERIFY, &val);
    break;
  default:
    break;
  };
...
static int fg_bat_notifier_call(struct notifier_block *nb, unsigned long event,
                                void *data) {
...
static int tcpc_class_complete_init(void) {
  if (!IS_ERR(tcpc_class)) {
    class_for_each_device(tcpc_class, NULL, NULL, __tcpc_class_complete_work);
  }
  return 0;
}
...
static int rt_pd_manager_probe(struct platform_device *pdev) {
  ...
  rpmd->iio_channels = devm_kcalloc(rpmd->dev, POWER_SUPPLY_IIO_PROP_MAX,
                                    sizeof(rpmd->iio_channels[0]), GFP_KERNEL);
  ...
  ret =
      register_tcp_dev_notifier(rpmd->tcpc, &rpmd->pd_nb, TCP_NOTIFY_TYPE_ALL);
  if (ret < 0) {
