...
static int sgm7220_write_device(void *client, u32 reg, int len,
                                const void *src) {
  const u8 *data;
  struct i2c_client *i2c = (struct i2c_client *)client;
  struct sgm7220_chip *chip = i2c_get_clientdata(i2c);
...
static int sgm7220_reg_update(struct i2c_client *i2c, u8 reg, const u8 data,
                              u8 mask) {
  struct sgm7220_chip *chip = i2c_get_clientdata(i2c);
  int ret;
  u8 old_val, new_val;
...
static void sgm7220_process_mode_register(struct sgm7220_chip *chip,
                                          u8 status) {
  u8 val;
  u8 tmp = status;
  ...
  pr_info("sgm7220 %s , current_det = %d, accessory_attach = %d, "
          "active_cable_attach = %d\n",
          __func__, chip->type_c_param.current_det,
...
static void sgm7220_process_interrupt_register(struct sgm7220_chip *chip,
                                               u8 status) {
  struct tcpc_device *tcpc;
  u8 val;
  u8 tmp = status;
...
static void sgm7220_irq_work_handler(struct kthread_work *work) {
  ...
  ret = sgm7220_reg_update(chip->client, REG_INT,
                           (0x1 << INT_INTERRUPT_STATUS_SHIFT),
                           INT_INTERRUPT_STATUS);
  if (ret < 0) {
...
int sgm7220_set_watchdog(struct tcpc_device *tcpc, bool en) { return 0; }
#endif /* CONFIG_TCPC_WATCHDOG_EN */

#ifdef CONFIG_TCPC_INTRST_EN
int sgm7220_set_intrst(struct tcpc_device *tcpc, bool en) { return 0; }
#endif

#ifdef CONFIG_USB_POWER_DELIVERY
static int sgm7220_set_msg_header(struct tcpc_device *tcpc, uint8_t power_role,
                                  uint8_t data_role) {
  return 0;
}

static int sgm7220_protocol_reset(struct tcpc_device *tcpc) { return 0; }
...
static int sgm7220_set_bist_carrier_mode(struct tcpc_device *tcpc,
                                         uint8_t pattern) {
...
static int sgm7220_retransmit(struct tcpc_device *tcpc) { return 0; }
#endif
...
static void sgm7220_first_check_typec_work(struct work_struct *work) {
  struct sgm7220_chip *chip =
      container_of(work, struct sgm7220_chip, first_check_typec_work.work);
  struct tcpc_device *tcpc;
  u8 reg_val;
  int ret;
...
static int sgm7220_initialization(struct sgm7220_chip *chip) {
  ...
  ret = sgm7220_reg_update(chip->client, REG_MOD, reg_val,
...
static ssize_t chip_info_show(struct device *dev, struct device_attribute *attr,
  ...
  for (i = 0, j = 0; i < ID_REG_LEN; i++) {
    reg_val = 0;
    ret = sgm7220_read_device(chip->client, (REG_BASE + (ID_REG_LEN - 1) - i),
                              1, &reg_val);
    if (ret < 0) {
      pr_err("%s: read reg fail!\n", __func__);
...
static int sgm7220_i2c_probe(struct i2c_client *client,
  ...
  INIT_DELAYED_WORK(&chip->first_check_typec_work,
                    sgm7220_first_check_typec_work);
  chip->irq_wake_lock =
...
static const struct dev_pm_ops sgm7220_pm_ops = {
    SET_SYSTEM_SLEEP_PM_OPS(sgm7220_i2c_suspend, sgm7220_i2c_resume)
#ifdef CONFIG_PM_RUNTIME
        SET_RUNTIME_PM_OPS(sgm7220_pm_suspend_runtime,
