...
static inline int pd_handle_tcp_event_request(struct pd_port *pd_port) {
  ...
  case TCP_DPM_EVT_REQUEST:
    ret = pd_dpm_update_tcp_request(pd_port, &tcp_event->tcp_dpm_data.pd_req);
    break;
  case TCP_DPM_EVT_REQUEST_EX:
    ret = pd_dpm_update_tcp_request_ex(pd_port,
                                       &tcp_event->tcp_dpm_data.pd_req_ex);
    break;
...
static inline int pd_handle_tcp_dpm_event(struct pd_port *pd_port,
...
#ifdef CONFIG_USB_PD_PR_SWAP
    ret = pd_handle_tcp_event_pr_swap(pd_port, pd_event->msg -
                                                   TCP_DPM_EVT_PR_SWAP_AS_SNK);
#endif /* CONFIG_USB_PD_PR_SWAP */
    break;
...
#ifdef CONFIG_USB_PD_DR_SWAP
    ret = pd_handle_tcp_event_dr_swap(pd_port, pd_event->msg -
  ...
  case TCP_DPM_EVT_GET_STATUS:
    ret = pd_make_tcp_event_transit_ready2(pd_port, PE_SNK_GET_SOURCE_STATUS,
  ...
  case TCP_DPM_EVT_GET_COUNTRY_CODE:
    ret = pd_make_tcp_event_transit_ready(pd_port, PE_GET_COUNTRY_CODES);
    break;
