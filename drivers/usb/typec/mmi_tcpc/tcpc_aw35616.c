 ...
 *
 ******************************************************/
static int aw35616_i2c_write(struct aw35616_chip *chip, u8 reg_addr,
                             u8 reg_data) {
  int ret = -1;
  unsigned char cnt = 0;
...
static int aw35616_i2c_read(struct aw35616_chip *chip, u8 reg_addr,
                            u8 *reg_data) {
  int ret = -1;
  unsigned char cnt = 0;
...
static int aw35616_i2c_read_bits(struct aw35616_chip *chip, u8 reg_addr,
                                 u8 *buf, u8 len) {
  int ret = 0;
...
static void aw35616_first_check_typec_work(struct work_struct *work) {
  struct aw35616_chip *chip =
      container_of(work, struct aw35616_chip, first_check_typec_work.work);
  int i = 0;
  u8 reg_data[4];
  ...
  if (chip->reg.ints.intb_flag == ATTACHED) {
    ...
    case SINK:
      ...
      if (chip->tcpc->typec_attach_new != TYPEC_ATTACHED_SRC) {
        ...
        tcpci_source_vbus(chip->tcpc, TCP_VBUS_CTRL_TYPEC, TCPC_VBUS_SOURCE_5V,
                          1000);
    ...
    case DUG_ACC:
      if (chip->reg.status.snk_det_rp_dbg) {
        ...
        if (chip->tcpc->typec_attach_new != TYPEC_ATTACHED_SRC) {
          chip->tcpc->typec_attach_new = TYPEC_ATTACHED_SRC;
          tcpci_source_vbus(chip->tcpc, TCP_VBUS_CTRL_TYPEC,
                            TCPC_VBUS_SOURCE_5V, 1000);
          tcpci_notify_typec_state(chip->tcpc);
          chip->tcpc->typec_attach_old = TYPEC_ATTACHED_SRC;
        }
      }
    ...
    if ((chip->reg.status.plug_st == SINK) ||
      ...
      if (chip->tcpc->typec_attach_old == TYPEC_ATTACHED_SRC) {
        tcpci_source_vbus(chip->tcpc, TCP_VBUS_CTRL_TYPEC, TCPC_VBUS_SOURCE_0V,
                          0);
      }
...
static irqreturn_t aw35616_irq_work_handler(int irq, void *data) {
  ...
  if (chip->reg.ints.intb_flag == ATTACHED) {
    ...
    if (chip->reg.status.plug_ori > 0)
      chip->tcpc->typec_polarity = chip->reg.status.plug_ori - 1;
    AW_LOG("plug_ori = %d, typec_polarity = %d", chip->reg.status.plug_ori,
           chip->tcpc->typec_polarity);
    switch (chip->reg.status.plug_st) {
    case STANDBY:
      AW_LOG("plug status not connected\n");
...
static int aw35616_init_alert(struct tcpc_device *tcpc) {
  ...
  ret = devm_request_threaded_irq(
      chip->dev, chip->irq, NULL, aw35616_irq_work_handler,
...
static int aw35616_set_msg_header(struct tcpc_device *tcpc, uint8_t power_role,
                                  uint8_t data_role) {
  AW_LOG("enter\n");
  return 0;
}
...
static int aw35616_set_bist_carrier_mode(struct tcpc_device *tcpc,
                                         uint8_t pattern) {
  AW_LOG("enter\n");
  return 0;
}
...
static int aw35616_tcpcdev_init(struct aw35616_chip *chip, struct device *dev) {
  ...
  if (of_property_read_u32(np, "aw35616-tcpc,notifier_supply_num", &val) >= 0) {
    if (val < 0)
  ...
  chip->tcpc = tcpc_device_register(dev, desc, &aw35616_tcpc_ops, chip);
  if (IS_ERR(chip->tcpc))
    return -EINVAL;
...
static ssize_t regdump_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t typec_polarity_show(struct device *dev,
  ...
  return sprintf(buf, "%s\n",
                 ...
                 : (chip->reg.status.plug_ori == CC1_CC2) ? "CC1&&CC2"
                                                          : "None");
}
...
static const struct dev_pm_ops aw35616_pm_ops = {
    SET_SYSTEM_SLEEP_PM_OPS(aw35616_i2c_suspend, aw35616_i2c_resume)
#ifdef CONFIG_PM_RUNTIME
        SET_RUNTIME_PM_OPS(aw35616_pm_suspend_runtime,
                           aw35616_pm_resume_runtime, NULL)
#endif /* CONFIG_PM_RUNTIME */
};
...
static void __exit aw35616_exit(void) { i2c_del_driver(&aw35616_driver); }
