...
static int pd_set_cap(struct adapter_device *dev, enum adapter_cap_type type,
  ...
  if (type == MMI_PD_APDO_START) {
    tcpm_ret = tcpm_set_apdo_charging_policy(
        info->tcpc, DPM_CHARGING_POLICY_PPS, mV, mA, NULL);
...
static int pd_get_cap(struct adapter_device *dev, enum adapter_cap_type type,
  ...
  if (type == MMI_PD_APDO) {
    while (1) {
      ret = tcpm_inquire_pd_source_apdo(
          info->tcpc, TCPM_POWER_CAP_APDO_TYPE_PPS, &cap_i, &apdo_cap);
    ...
    for (i = 0; i < tacap->nr; i++) {
      pr_notice("pps_cap[%d:%d], %d mv ~ %d mv, %d ma pl:%d pdp:%d\n", i,
                (int)tacap->nr, tacap->min_mv[i], tacap->max_mv[i],
                tacap->ma[i], tacap->pwr_limit[i], tacap->pdp);
    }
    ...
    if (pd_cap.nr != 0) {
      ...
      for (i = 0; i < pd_cap.nr; i++) {
        if (pd_cap.type[i] == TCPM_POWER_CAP_VAL_TYPE_FIXED && j >= 0 &&
          ...
          tacap->maxwatt[j] = tacap->max_mv[j] * tacap->ma[i];
          tacap->minwatt[j] = tacap->min_mv[j] * tacap->ma[i];
          j++;
        }

        pr_notice("[%s]:%d mv:[%d,%d] mA:%d type:%d %d\n", __func__, i,
                  pd_cap.min_mv[i], pd_cap.max_mv[i], pd_cap.ma[i],
      ...
      for (i = 0; i < tacap->nr; i++) {
        pr_notice("[%s]:%d mv:[%d,%d] mA:%d max:%d min:%d type:%d %d\n",
                  __func__, i, tacap->min_mv[i], tacap->max_mv[i], tacap->ma[i],
                  tacap->maxwatt[i], tacap->minwatt[i], tacap->type[i],
                  tacap->type[i]);
      }
    }
    ...
    if (pd_cap.nr != 0) {
      ...
      for (i = 0; i < pd_cap.nr; i++) {
        ...
        pr_notice("[%s]:%d mv:[%d,%d] %d max:%d min:%d type:%d %d\n", __func__,
                  i, tacap->min_mv[i], tacap->max_mv[i], tacap->ma[i],
                  tacap->maxwatt[i], tacap->minwatt[i], tacap->type[i],
                  pd_cap.type[i]);
      }
    }
...
int pd_adapter_create(struct platform_device *pdev) {
  ...
  info =
      devm_kzalloc(&pdev->dev, sizeof(struct mmi_pd_adapter_info), GFP_KERNEL);
  if (!info)
    return -ENOMEM;

  info->adapter_dev = adapter_device_register("pd_adapter", &pdev->dev, info,
                                              &adapter_ops, NULL);
