...
static inline bool dc_dfp_send_en_unlock(struct pd_port *pd_port, uint32_t cmd,
  ...
  if (pd_port->pe_data.dc_pps_mode) {
    pd_port->uvdm_data[0] = VDO_S(USB_VID_DIRECTCHARGE, SVDM_REV20, CMDT_INIT,
...
bool dc_dfp_start_en_unlock1(struct pd_port *pd_port) {
  ...
  pd_port->dc_pass_code = dc_get_authorization_code((rn_code[0] & 0xffff) |
                                                    (rn_code[1] & 0xffff0000));
...
bool dc_dfp_start_en_unlock2(struct pd_port *pd_port) {
  ...
  dc_dfp_send_en_unlock(pd_port, RTDC_UVDM_EN_UNLOCK, pd_port->dc_pass_code,
                        rn_code);
...
bool dc_ufp_notify_uvdm(struct pd_port *pd_port,
  ...
  case DC_UFP_T0: {
    ...
    pass_code = dc_get_authorization_code((recv_code[0] & 0xffff) |
