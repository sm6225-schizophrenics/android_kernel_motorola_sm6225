...
#define APDO_PPS(min_mv, max_mv, ma, flags)                                    \
  (APDO_PPS_MIN_VOLT(min_mv) | APDO_PPS_MAX_VOLT(max_mv) | APDO_PPS_CURR(ma) | \
...
#define RDO_APDO(n, op_mv, op_ma, flags)                                       \
  (RDO_OBJ_POS(n) | (flags) | RDO_APDO_OP_MV(op_mv) | RDO_APDO_OP_MA(op_ma))
...
#define PD_EXT_HEADER_CK(data_size, req, chunk_nr, chunked)                    \
  ((data_size) | (req << 10) | ((chunk_nr) << 11) | (chunked << 15))
...
#define VDO_S(svid, ver, cmd_type, cmd, obj)                                   \
  VDO(svid, 1, VDO_SVDM_VERS(ver) | VDO_CMDT(cmd_type) | VDO_OPOS(obj) | cmd)

#define VDO_REPLY(ver, cmd_type, request_vdo)                                  \
  (VDO_SVDM_VERS(ver) | VDO_CMDT(cmd_type) | ((request_vdo) & (~0x60C0)))
...
#define VDO_IDH(usbh, usbd, ptype, ptype_dfp, is_modal, vid)                   \
  ((usbh) << 31 | (usbd) << 30 | ((ptype) & 0x7) << 27 |                       \
   ((ptype_dfp) & 0x07) << 23 | (is_modal) << 26 | ((vid) & 0xffff))
...
#define VDO_CABLE(hw, fw, cbl, gdr, lat, term, tx1d, tx2d, rx1d, rx2d, cur,    \
  ...
  (((hw) & 0x7) << 28 | ((fw) & 0x7) << 24 | ((cbl) & 0x3) << 18 |             \
   (gdr) << 17 | ((lat) & 0x7) << 13 | ((term) & 0x3) << 11 | (tx1d) << 10 |   \
   (tx2d) << 9 | (rx1d) << 8 | (rx2d) << 7 | ((cur) & 0x3) << 5 | (vps) << 4 | \
   (sopp) << 3 | ((usbss) & 0x7))
...
#define PD_DP_DFP_D_PIN_CAPS(x)                                                \
  (MODE_DP_RECEPT(x) ? MODE_DP_PIN_DFP(x) : MODE_DP_PIN_UFP(x))

#define PD_DP_UFP_D_PIN_CAPS(x)                                                \
  (MODE_DP_RECEPT(x) ? MODE_DP_PIN_UFP(x) : MODE_DP_PIN_DFP(x))
...
static inline bool pd_check_ctrl_msg_event(struct pd_port *pd_port,
                                           uint8_t msg) {
  return pd_event_ctrl_msg_match(pd_get_curr_pd_event(pd_port), msg);
}

static inline bool pd_check_data_msg_event(struct pd_port *pd_port,
                                           uint8_t msg) {
  return pd_event_data_msg_match(pd_get_curr_pd_event(pd_port), msg);
}

static inline bool pd_check_timer_msg_event(struct pd_port *pd_port,
                                            uint8_t msg) {
...
#define PD_RX_CAP_PE_SWAP (TCPC_RX_CAP_HARD_RESET | TCPC_RX_CAP_SOP)
...
uint32_t pd_reset_pdo_power(struct tcpc_device *tcpc, uint32_t pdo,
                            uint32_t imax);

void pd_extract_rdo_power(uint32_t rdo, uint32_t pdo, uint32_t *op_curr,
                          uint32_t *max_curr);
...
extern void pd_notify_tcp_event_buf_reset(struct pd_port *pd_port,
                                          uint8_t reason);
...
extern void pd_notify_tcp_vdm_event_2nd_result(struct pd_port *pd_port,
                                               uint8_t ret);

/* ---- pd_timer ---- */

static inline void pd_restart_timer(struct pd_port *pd_port,
                                    uint32_t timer_id) {
...
static inline void pd_enable_pe_state_timer(struct pd_port *pd_port,
                                            uint32_t timer_id) {
...
static inline void pd_enable_vdm_state_timer(struct pd_port *pd_port,
                                             uint32_t timer_id) {
  pd_port->pe_data.vdm_state_timer = timer_id;
}

static inline void pd_disable_timer(struct pd_port *pd_port,
                                    uint32_t timer_id) {
...
static inline bool pd_put_dpm_notify_event(struct pd_port *pd_port,
                                           uint8_t notify) {
  struct pd_event evt = {
...
static inline bool pd_put_dpm_nak_event(struct pd_port *pd_port,
                                        uint8_t notify) {
  struct pd_event evt = {
      .event_type = PD_EVT_DPM_MSG,
      .msg = PD_DPM_NAK,
...
static inline bool pd_put_tcp_vdm_event(struct pd_port *pd_port,
                                        uint8_t event) {
  struct pd_event evt = {
      .event_type = PD_EVT_TCP_MSG,
      .msg = event,
...
static inline bool vdm_put_hw_event(struct tcpc_device *tcpc,
                                    uint8_t hw_event) {
  struct pd_event evt = {
...
static inline bool vdm_put_pe_event(struct tcpc_device *tcpc,
                                    uint8_t pe_event) {
  struct pd_event evt = {
      .event_type = PD_EVT_PE_MSG,
      .msg = pe_event,
...
static inline bool pd_put_sink_tx_event(struct tcpc_device *tcpc,
                                        uint8_t cc_res) {
#ifdef CONFIG_USB_PD_REV30_COLLISION_AVOID
  struct pd_event evt = {
      .event_type = PD_EVT_HW_MSG,
...
int pd_send_message(struct pd_port *pd_port, uint8_t sop_type, uint8_t msg,
                    bool ext, uint16_t count, const uint32_t *data);

int pd_send_data_msg(struct pd_port *pd_port, uint8_t sop_type, uint8_t msg,
...
int pd_send_sop_data_msg(struct pd_port *pd_port, uint8_t msg, uint8_t cnt,
...
#ifdef CONFIG_USB_PD_REV30
int pd_send_ext_msg(struct pd_port *pd_port, uint8_t sop_type, uint8_t msg,
                    bool request, uint8_t chunk_nr, uint8_t size,
                    const uint8_t *data);

static inline int pd_send_sop_ext_msg(struct pd_port *pd_port, uint8_t msg,
                                      uint8_t size, const void *data) {
  PD_BUG_ON(size > MAX_EXTENDED_MSG_LEGACY_LEN);

  /* TODO: chunking */

  return pd_send_ext_msg(pd_port, TCPC_TX_SOP, msg, false, 0, size, data);
}
#endif /* CONFIG_USB_PD_REV30 */
...
int pd_send_svdm_request(struct pd_port *pd_port, uint8_t sop_type,
                         uint16_t svid, uint8_t vdm_cmd, uint8_t obj_pos,
                         uint8_t cnt, uint32_t *data_obj, uint32_t timer_id);

int pd_reply_svdm_request(struct pd_port *pd_port, uint8_t reply, uint8_t cnt,
                          uint32_t *data_obj);

#ifdef CONFIG_USB_POWER_DELIVERY

static inline int pd_send_vdm_discover_id(struct pd_port *pd_port,
                                          uint8_t sop_type) {
  return pd_send_svdm_request(pd_port, sop_type, USB_SID_PD, CMD_DISCOVER_IDENT,
                              0, 0, NULL, PD_TIMER_VDM_RESPONSE);
}

static inline int pd_send_vdm_discover_svids(struct pd_port *pd_port,
                                             uint8_t sop_type) {
  return pd_send_svdm_request(pd_port, sop_type, USB_SID_PD, CMD_DISCOVER_SVID,
                              0, 0, NULL, PD_TIMER_VDM_RESPONSE);
}

static inline int pd_send_vdm_discover_modes(struct pd_port *pd_port,
                                             uint8_t sop_type, uint16_t svid) {
  return pd_send_svdm_request(pd_port, sop_type, svid, CMD_DISCOVER_MODES, 0, 0,
                              NULL, PD_TIMER_VDM_RESPONSE);
}

static inline int pd_send_vdm_enter_mode(struct pd_port *pd_port,
                                         uint8_t sop_type, uint16_t svid,
                                         uint8_t obj_pos) {
  return pd_send_svdm_request(pd_port, sop_type, svid, CMD_ENTER_MODE, obj_pos,
                              0, NULL, PD_TIMER_VDM_MODE_ENTRY);
}

static inline int pd_send_vdm_exit_mode(struct pd_port *pd_port,
                                        uint8_t sop_type, uint16_t svid,
                                        uint8_t obj_pos) {
  return pd_send_svdm_request(pd_port, sop_type, svid, CMD_EXIT_MODE, obj_pos,
                              0, NULL, PD_TIMER_VDM_MODE_EXIT);
}

static inline int pd_send_vdm_attention(struct pd_port *pd_port,
                                        uint8_t sop_type, uint16_t svid,
                                        uint8_t obj_pos) {
  return pd_send_svdm_request(pd_port, sop_type, svid, CMD_ATTENTION, obj_pos,
                              0, NULL, 0);
}

static inline int pd_send_vdm_dp_attention(struct pd_port *pd_port,
                                           uint8_t sop_type, uint8_t obj_pos,
                                           uint32_t dp_status) {
...
static inline int pd_send_vdm_dp_status(struct pd_port *pd_port,
                                        uint8_t sop_type, uint8_t obj_pos,
                                        uint8_t cnt, uint32_t *data_obj) {
  return pd_send_svdm_request(pd_port, sop_type, USB_SID_DISPLAYPORT,
                              CMD_DP_STATUS, obj_pos, cnt, data_obj,
...
static inline int pd_send_vdm_dp_config(struct pd_port *pd_port,
                                        ...
                                        uint8_t cnt, uint32_t *data_obj) {
  return pd_send_svdm_request(pd_port, sop_type, USB_SID_DISPLAYPORT,
                              CMD_DP_CONFIG, obj_pos, cnt, data_obj,
                              PD_TIMER_VDM_RESPONSE);
}

static inline int pd_reply_svdm_request_simply(struct pd_port *pd_port,
                                               uint8_t reply) {
...
int pd_reply_custom_vdm(struct pd_port *pd_port, uint8_t sop_type, uint8_t cnt,
                        uint32_t *payload);
#endif /* CONFIG_USB_PD_CUSTOM_VDM */
