...
#define PE_TRANSIT_STATE(pd_port, state) (pd_port->pe_state_next = state)
...
pe_get_curr_evaluate_pr_swap_state(struct pd_port *pd_port) {
...
static inline uint8_t
pd_get_curr_hard_reset_recv_state(struct pd_port *pd_port) {
  if (pd_port->power_role == PD_ROLE_SINK)
...
static inline uint8_t
pd_get_curr_soft_reset_recv_state(struct pd_port *pd_port) {
  if (pd_port->power_role == PD_ROLE_SINK)
...
#define PE_MAKE_STATE_TRANSIT_SINGLE(reaction, next)                           \
  pd_make_pe_state_transit_single(pd_port, pd_port->pe_state_curr, reaction,   \
                                  next)
