 ...
 */
typedef enum { SNK = 0, SRC = 1, DRP = 2, WAKE_MODE = 3 } WKMD;
 ...
 */
typedef enum { NO_TRY = 0, TRY_SNK = 1, TRY_SRC = 2, OR_NO_TRY = 3 } TRY_MD;
 ...
 * intb flag
 */
typedef enum { NO_INTB = 0, ATTACHED = 1, DETACHED = 2 } INTB_FLAG;

/**
 * plag direction
 */
typedef enum { STANDBY = 0, CC1 = 1, CC2 = 2, CC1_CC2 = 3 } PLAG_ORI;

/**
 * plag status
 */
typedef enum { SINK = 1, SOURCE = 2, AUD_ACC = 3, DUG_ACC = 4 } PLAG_ST;

/**
 * charging current detection as sink
 */
typedef enum { SNK_DEF = 1, SNK_1_5A = 2, SNK_3_0A = 3 } SINK_CUR;

/**
 * vbus detection as sink
 */
typedef enum { VBUS_DIS = 0, VBUS_OK = 1 } VBUS_ST;
...
#define AW_LOG(format, arg...)                                                 \
  pr_err("[%s] %s %d: " format, AWINIC_LOG_NAME, __func__, __LINE__, ##arg)
#else
#define AW_LOG(format, arg...)
#endif
