...
enum dpm_charging_policy {
  ...
  DPM_CHARGING_POLICY_MAX_POWER_LV =
      DPM_CHARGING_POLICY_MAX_POWER | DPM_CHARGING_POLICY_PREFER_LOW_VOLTAGE,
  DPM_CHARGING_POLICY_MAX_POWER_LVIC = DPM_CHARGING_POLICY_MAX_POWER_LV |
                                       DPM_CHARGING_POLICY_IGNORE_MISMATCH_CURR,

  DPM_CHARGING_POLICY_MAX_POWER_HV =
      DPM_CHARGING_POLICY_MAX_POWER | DPM_CHARGING_POLICY_PREFER_HIGH_VOLTAGE,
  DPM_CHARGING_POLICY_MAX_POWER_HVIC = DPM_CHARGING_POLICY_MAX_POWER_HV |
                                       DPM_CHARGING_POLICY_IGNORE_MISMATCH_CURR,
...
extern int tcpm_inquire_remote_cc(struct tcpc_device *tcpc, uint8_t *cc1,
                                  uint8_t *cc2, bool from_ic);
...
extern int tcpm_typec_change_mode(struct tcpc_device *tcpc, uint8_t mode);

extern int tcpm_typec_change_role_postpone(struct tcpc_device *tcpc,
                                           uint8_t typec_role, bool postpone);
...
extern int tcpm_put_tcp_dpm_event(struct tcpc_device *tcpc,
                                  struct tcp_dpm_event *event);
...
extern int tcpm_inquire_pd_partner_inform(struct tcpc_device *tcpc,
                                          uint32_t *vdos);
extern int tcpm_inquire_pd_partner_svids(struct tcpc_device *tcpc,
                                         struct tcpm_svid_list *list);
extern int tcpm_inquire_pd_partner_modes(struct tcpc_device *tcpc,
                                         uint16_t svid,
                                         struct tcpm_mode_list *list);
extern int tcpm_inquire_pd_source_cap(struct tcpc_device *tcpc,
                                      struct tcpm_power_cap *cap);
extern int tcpm_inquire_pd_sink_cap(struct tcpc_device *tcpc,
                                    struct tcpm_power_cap *cap);

extern bool tcpm_extract_power_cap_val(uint32_t pdo,
                                       struct tcpm_power_cap_val *cap);

extern bool tcpm_extract_power_cap_list(struct tcpm_power_cap *cap,
                                        struct tcpm_power_cap_list *cap_list);

extern int tcpm_get_remote_power_cap(struct tcpc_device *tcpc,
                                     struct tcpm_remote_power_cap *cap);

extern int tcpm_inquire_select_source_cap(struct tcpc_device *tcpc,
                                          struct tcpm_power_cap_val *cap_val);

/* Request TCPM to send PD Request */

extern int tcpm_dpm_pd_power_swap(struct tcpc_device *tcpc, uint8_t role,
                                  const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_pd_data_swap(struct tcpc_device *tcpc, uint8_t role,
                                 const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_pd_vconn_swap(struct tcpc_device *tcpc, uint8_t role,
                                  const struct tcp_dpm_event_cb_data *data);
...
extern int tcpm_dpm_pd_get_sink_cap(struct tcpc_device *tcpc,
                                    const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_pd_request(struct tcpc_device *tcpc, int mv, int ma,
                               const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_pd_request_ex(struct tcpc_device *tcpc, uint8_t pos,
                                  uint32_t max, uint32_t oper,
                                  const struct tcp_dpm_event_cb_data *data);
...
#ifdef CONFIG_USB_PD_REV30
extern int
tcpm_dpm_pd_get_source_cap_ext(struct tcpc_device *tcpc,
                               const struct tcp_dpm_event_cb_data *data,
                               struct pd_source_cap_ext *src_cap_ext);
extern int tcpm_dpm_pd_fast_swap(struct tcpc_device *tcpc, uint8_t role,
                                 const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_pd_get_status(struct tcpc_device *tcpc,
                                  const struct tcp_dpm_event_cb_data *data,
                                  struct pd_status *status);
extern int
tcpm_dpm_pd_get_pps_status_raw(struct tcpc_device *tcpc,
...
extern int
tcpm_dpm_pd_get_country_code(struct tcpc_device *tcpc,
                             const struct tcp_dpm_event_cb_data *data,
                             struct pd_country_codes *ccdb);
extern int
tcpm_dpm_pd_get_country_info(struct tcpc_device *tcpc, uint32_t ccdo,
                             const struct tcp_dpm_event_cb_data *data,
...
extern int tcpm_dpm_pd_alert(struct tcpc_device *tcpc, uint32_t ado,
                             const struct tcp_dpm_event_cb_data *data);
#endif /* CONFIG_USB_PD_REV30 */
...
extern int
tcpm_dpm_pd_cable_soft_reset(struct tcpc_device *tcpc,
                             const struct tcp_dpm_event_cb_data *data);
extern int
tcpm_dpm_vdm_discover_cable(struct tcpc_device *tcpc,
                            const struct tcp_dpm_event_cb_data *data);
...
extern int tcpm_dpm_vdm_discover_svid(struct tcpc_device *tcpc,
                                      const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_vdm_discover_mode(struct tcpc_device *tcpc, uint16_t svid,
                                      const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_vdm_enter_mode(struct tcpc_device *tcpc, uint16_t svid,
                                   uint8_t ops,
                                   const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_vdm_exit_mode(struct tcpc_device *tcpc, uint16_t svid,
                                  uint8_t ops,
                                  const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_vdm_attention(struct tcpc_device *tcpc, uint16_t svid,
                                  uint8_t ops,
                                  const struct tcp_dpm_event_cb_data *data);
...
extern int tcpm_inquire_dp_ufp_u_state(struct tcpc_device *tcpc,
                                       uint8_t *state);

extern int tcpm_dpm_dp_attention(struct tcpc_device *tcpc, uint32_t dp_status,
                                 uint32_t mask,
                                 const struct tcp_dpm_event_cb_data *data);

#ifdef CONFIG_USB_PD_ALT_MODE_DFP

extern int tcpm_inquire_dp_dfp_u_state(struct tcpc_device *tcpc,
                                       uint8_t *state);
...
extern int tcpm_dpm_dp_config(struct tcpc_device *tcpc, uint32_t dp_config,
                              uint32_t mask,
                              const struct tcp_dpm_event_cb_data *data);
#endif /* CONFIG_USB_PD_ALT_MODE_DFP */
#endif /* CONFIG_USB_PD_ALT_MODE */
...
extern int
tcpm_dpm_send_custom_vdm(struct tcpc_device *tcpc,
                         struct tcp_dpm_custom_vdm_data *vdm_data,
                         const struct tcp_dpm_event_cb_data *cb_data);
...
extern int
tcpm_reset_pd_charging_policy(struct tcpc_device *tcpc,
                              const struct tcp_dpm_event_cb_data *data);

extern int
tcpm_set_pd_charging_policy(struct tcpc_device *tcpc, uint8_t policy,
                            const struct tcp_dpm_event_cb_data *data);

extern int tcpm_set_pd_charging_policy_default(struct tcpc_device *tcpc,
                                               uint8_t policy);

extern uint8_t tcpm_inquire_pd_charging_policy(struct tcpc_device *tcpc);
extern uint8_t
tcpm_inquire_pd_charging_policy_default(struct tcpc_device *tcpc);
...
extern int tcpm_dpm_set_vconn_supply_mode(struct tcpc_device *tcpc,
                                          uint8_t mode);
...
extern int
tcpm_set_apdo_charging_policy(struct tcpc_device *tcpc, uint8_t policy, int mv,
                              int ma, const struct tcp_dpm_event_cb_data *data);
...
extern int tcpm_update_pd_status_temp(struct tcpc_device *tcpc,
                                      enum pd_present_temperature_flag ptf,
                                      uint8_t temperature);

extern int tcpm_update_pd_status_input(struct tcpc_device *tcpc, uint8_t input,
                                       uint8_t mask);

extern int tcpm_update_pd_status_bat_input(struct tcpc_device *tcpc,
                                           uint8_t bat_input, uint8_t bat_mask);
...
static inline struct tcpc_device *tcpc_dev_get_by_name(const char *name) {
  return NULL;
}
...
static inline int tcpm_inquire_remote_cc(struct tcpc_device *tcpc, uint8_t *cc1,
                                         uint8_t *cc2, bool from_ic) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_vbus_level(struct tcpc_device *tcpc,
                                          bool from_ic) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
tcpm_inquire_typec_attach_state(struct tcpc_device *tcpc) {
...
static inline int tcpm_typec_set_wake_lock(struct tcpc_device *tcpc,
                                           bool user_lock) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_typec_set_usb_sink_curr(struct tcpc_device *tcpc,
                                               int curr) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_typec_set_rp_level(struct tcpc_device *tcpc,
                                          uint8_t level) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_typec_change_role(struct tcpc_device *tcpc,
                                         uint8_t typec_role) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_typec_change_mode(struct tcpc_device *tcpc,
                                         uint8_t mode) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_typec_change_role_postpone(struct tcpc_device *tcpc,
                                                  uint8_t typec_role,
                                                  bool postpone) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_put_tcp_dpm_event(struct tcpc_device *tcpc,
                                         struct tcp_dpm_event *event) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_pd_contract(struct tcpc_device *tcpc, int *mv,
                                           int *ma) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_cable_inform(struct tcpc_device *tcpc,
                                            uint32_t *vdos) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_pd_partner_inform(struct tcpc_device *tcpc,
                                                 uint32_t *vdos) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_pd_partner_svids(struct tcpc_device *tcpc,
                                                struct tcpm_svid_list *list) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_pd_partner_modes(struct tcpc_device *tcpc,
                                                uint16_t svid,
                                                struct tcpm_mode_list *list) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_pd_source_cap(struct tcpc_device *tcpc,
                                             struct tcpm_power_cap *cap) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_inquire_pd_sink_cap(struct tcpc_device *tcpc,
                                           struct tcpm_power_cap *cap) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline bool tcpm_extract_power_cap_val(uint32_t pdo,
                                              struct tcpm_power_cap_val *cap) {
  return false;
}

static inline bool
tcpm_extract_power_cap_list(struct tcpm_power_cap *cap,
                            struct tcpm_power_cap_list *cap_list) {
  return false;
}
...
static inline int
tcpm_inquire_select_source_cap(struct tcpc_device *tcpc,
                               struct tcpm_power_cap_val *cap_val) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_power_swap(struct tcpc_device *tcpc, uint8_t role,
                       const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_data_swap(struct tcpc_device *tcpc, uint8_t role,
                      const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_vconn_swap(struct tcpc_device *tcpc, uint8_t role,
                       const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_goto_min(struct tcpc_device *tcpc,
                     const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_soft_reset(struct tcpc_device *tcpc,
                       const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_get_source_cap(struct tcpc_device *tcpc,
                           const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_get_sink_cap(struct tcpc_device *tcpc,
                         const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_request(struct tcpc_device *tcpc, int mv, int ma,
                    const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_request_ex(struct tcpc_device *tcpc, uint8_t pos, uint32_t max,
                       uint32_t oper,
                       const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_dpm_pd_bist_cm2(struct tcpc_device *tcpc,
                     const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_dpm_pd_fast_swap(struct tcpc_device *tcpc, uint8_t role,
                      const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_get_status(struct tcpc_device *tcpc,
                       const struct tcp_dpm_event_cb_data *data,
                       struct pd_status *status) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_dpm_pd_get_pps_status(struct tcpc_device *tcpc,
                           const struct tcp_dpm_event_cb_data *data,
                           struct pd_pps_status *pps_status) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_dpm_pd_get_bat_cap(struct tcpc_device *tcpc,
                        struct pd_get_battery_capabilities *gbcdb,
                        const struct tcp_dpm_event_cb_data *data,
                        struct pd_battery_capabilities *bcdb) {
  return TCPM_ERROR_NO_IMPLEMENT;
...
static inline int tcpm_dpm_pd_get_bat_status(
    struct tcpc_device *tcpc, struct pd_get_battery_status *gbsdb,
    const struct tcp_dpm_event_cb_data *data, uint32_t *bsdo) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_pd_get_mfrs_info(struct tcpc_device *tcpc,
                          struct pd_get_manufacturer_info *gmidb,
                          const struct tcp_dpm_event_cb_data *data,
                          struct pd_manufacturer_info *midb) {
  return TCPM_ERROR_NO_IMPLEMENT;
...
static inline int tcpm_dpm_pd_alert(struct tcpc_device *tcpc, uint32_t ado,
                                    const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_dpm_pd_hard_reset(struct tcpc_device *tcpc,
                       const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_dpm_vdm_discover_id(struct tcpc_device *tcpc,
                         const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_vdm_discover_svid(struct tcpc_device *tcpc,
                           const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_vdm_discover_mode(struct tcpc_device *tcpc, uint16_t svid,
                           const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_vdm_enter_mode(struct tcpc_device *tcpc, uint16_t svid, uint8_t ops,
                        const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_vdm_attention(struct tcpc_device *tcpc, uint16_t svid, uint8_t ops,
                       const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_vdm_exit_mode(struct tcpc_device *tcpc, uint16_t svid, uint8_t ops,
                       const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
#endif /* USB_POWER_DELIVERY_NA */
...
static inline int tcpm_inquire_dp_ufp_u_state(struct tcpc_device *tcpc,
                                              uint8_t *state) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_dp_attention(struct tcpc_device *tcpc, uint32_t dp_status,
                      uint32_t mask, const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
#endif /* USB_PD_ALT_MODE_NA */

#ifdef USB_PD_ALT_MODE_DFP_NA
static inline int tcpm_inquire_dp_dfp_u_state(struct tcpc_device *tcpc,
                                              uint8_t *state) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int
tcpm_dpm_dp_status_update(struct tcpc_device *tcpc, uint32_t dp_status,
                          uint32_t mask,
                          const struct tcp_dpm_event_cb_data *data) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_set_pd_charging_policy_default(struct tcpc_device *tcpc,
                                                      uint8_t policy) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline uint8_t
tcpm_inquire_pd_charging_policy(struct tcpc_device *tcpc) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_dpm_set_vconn_supply_mode(struct tcpc_device *tcpc,
                                                 uint8_t mode) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_update_bat_status_wh_no_mutex(struct tcpc_device *tcpc,
                                   enum pd_battery_reference ref,
                                   uint8_t status, uint16_t wh) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int
tcpm_update_bat_last_full_no_mutex(struct tcpc_device *tcpc,
...
static inline int
tcpm_update_pd_status_temp(struct tcpc_device *tcpc,
                           enum pd_present_temperature_flag ptf,
                           uint8_t temperature) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_update_pd_status_input(struct tcpc_device *tcpc,
                                              uint8_t input, uint8_t mask) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_update_pd_status_bat_input(struct tcpc_device *tcpc,
                                                  uint8_t bat_input,
                                                  uint8_t bat_mask) {
  return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_update_pd_status_event(struct tcpc_device *tcpc,
                                              uint8_t evt) {
  return TCPM_ERROR_NO_IMPLEMENT;
}
