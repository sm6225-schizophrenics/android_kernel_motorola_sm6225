...
#define TCPC_ENABLE_ANYMSG                                                     \
  ((TCPC_DBG_ENABLE) | (TCPC_DBG2_ENABLE) | (DPM_DBG_ENABLE) |                 \
   (PD_ERR_ENABLE) | (PE_INFO_ENABLE) | (TCPC_TIMER_INFO_EN) |                 \
   (PE_DBG_ENABLE) | (PE_EVENT_DBG_ENABLE) | (PE_STATE_INFO_ENABLE) |          \
   (TCPC_INFO_ENABLE) | (TCPC_TIMER_DBG_EN) | (TYPEC_DBG_ENABLE) |             \
   (TYPEC_INFO_ENABLE) | (DP_INFO_ENABLE) | (DP_DBG_ENABLE) |                  \
...
struct tcpc_ops {
  ...
  int (*set_port_type)(struct tcpc_device *tcpc, int mode);
...
#ifdef CONFIG_USB_POWER_DELIVERY
  int (*set_msg_header)(struct tcpc_device *tcpc, uint8_t power_role,
                        uint8_t data_role);
  ...
  int (*transmit)(struct tcpc_device *tcpc, enum tcpm_transmit_type type,
