...
static const char *const role_text[] = {
    "Unknown", "SNK Only", "SRC Only", "DRP", "Try.SRC", "Try.SNK",
};
...
static ssize_t tcpc_show_property(struct device *dev,
  ...
  case TCPC_DESC_CAP_INFO:
    ...
    snprintf(buf + strlen(buf), 256, "%s = %d\n%s = %d\n", "local_selected_cap",
             pe_data->local_selected_cap, "remote_selected_cap",
    ...
    for (i = 0; i < pd_port->local_src_cap.nr; i++) {
      tcpm_extract_power_cap_val(pd_port->local_src_cap.pdos[i], &cap);
    ...
    for (i = 0; i < pd_port->local_snk_cap.nr; i++) {
      tcpm_extract_power_cap_val(pd_port->local_snk_cap.pdos[i], &cap);
      snprintf(buf + strlen(buf), 256, "%d %d %d %d\n", cap.type, cap.min_mv,
               cap.max_mv, cap.ma);
    }
    snprintf(buf + strlen(buf), 256, "%s\n",
  ...
  case TCPC_DESC_PD_TEST:
    snprintf(buf, 256, "%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n", "1: pr_swap",
             "2: dr_swap", "3: vconn_swap", "4: soft reset", "5: hard reset",
             "6: get_src_cap", "7: get_sink_cap", "8: discover_id",
             "9: discover_cable");
    break;
  case TCPC_DESC_INFO:
...
struct tcpc_device *tcpc_dev_get_by_name(const char *name) {
  struct device *dev = class_find_device(tcpc_class, NULL, (const void *)name,
...
static void bat_update_work_func(struct work_struct *work) {
  ...
  ret = power_supply_get_property(tcpc->bat_psy, POWER_SUPPLY_PROP_CAPACITY,
                                  &value);
  if (ret == 0) {
  ...
  ret = power_supply_get_property(tcpc->bat_psy, POWER_SUPPLY_PROP_STATUS,
                                  &value);
  if (ret == 0) {
  ...
  tcpm_update_bat_status_soc(tcpc, PD_BAT_REF_FIXED0, tcpc->charging_status,
                             tcpc->bat_soc * 10);
}

static int bat_nb_call_func(struct notifier_block *nb, unsigned long val,
                            void *v) {
...
static int __register_tcp_dev_notifier(struct tcpc_device *tcp_dev,
  ...
  if (retval < 0)
    dev_warn(&tcp_dev->dev, "Failed to add resource to manager(%d)\n", retval);
...
static void *__remove_wrapper_from_managed_res_list(struct tcpc_device *tcp_dev,
                                                    void *key, int prv_id) {
  void *retval = NULL;
...
static int __unregister_tcp_dev_notifier(struct tcpc_device *tcp_dev,
  ...
  if (nb_wrapper) {
    retval = srcu_notifier_chain_unregister(tcp_dev->evt_nh + idx,
                                            &nb_wrapper->stub_nb);
    devm_kfree(&tcp_dev->dev, nb_wrapper);
    return retval;
  }
...
void *tcpc_get_dev_data(struct tcpc_device *tcpc) { return tcpc->drv_data; }
