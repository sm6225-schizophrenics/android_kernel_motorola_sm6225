...
static const char *const mmi_state[] = {
    "None",         "Request cap", "Encrypt",
    "Request temp", "Verify",      "mmi adapter ready",
};

static int hmac_sha256(u8 *key, u8 ksize, char *plaintext, u8 psize,
                       u8 *output) {
...
static int mmi_hmac_sha256(u8 *key, u8 ksize, uint32_t *plaintext, u8 psize,
                           uint32_t *output) {
  uint8_t *data_in;
  uint8_t *data_out;
  int i = 0, j = 0;
...
static void mmi_adapter_cap_request(struct pd_port *pd_port,
  ...
  pd_port->uvdm_data[0] =
      MMI_UVDM_CMD(svid_data->svid, CMD_HEADER_REQUEST, CMD_CHARGER_CAPACITY);
...
static void mmi_adapter_encrypt(struct pd_port *pd_port,
  ...
  pd_port->uvdm_data[0] = MMI_UVDM_CMD(svid_data->svid, CMD_HEADER_REQUEST,
                                       CMD_CHARGER_AUTHENTICATION);
...
static void mmi_adapter_verify(struct pd_port *pd_port,
  ...
  pd_port->uvdm_wait_resp = true;
  pd_port->uvdm_data[0] =
      MMI_UVDM_CMD(svid_data->svid, CMD_HEADER_ACK, CMD_CHARGER_VERIFY);
  pd_put_tcp_vdm_event(pd_port, TCP_DPM_EVT_UVDM);
}

bool mmi_dfp_notify_pe_startup(struct pd_port *pd_port,
                               struct svdm_svid_data *svid_data) {
  mmi_adapter_set_state(pd_port, MMI_NONE);
  return true;
}

bool mmi_dfp_u_notify_discover_svid(struct pd_port *pd_port,
                                    struct svdm_svid_data *svid_data,
                                    bool ack) {
  MMI_INFO("discover\n");
  if (!ack) {
...
bool mmi_dfp_notify_uvdm(struct pd_port *pd_port,
  ...
  case MMI_ENCRYPT:
    ...
    break;
  case MMI_VERIFY:
    if (mmi_uvdm_ack(pd_port, svid_data) && pd_port->uvdm_data[1] == 0x01) {
