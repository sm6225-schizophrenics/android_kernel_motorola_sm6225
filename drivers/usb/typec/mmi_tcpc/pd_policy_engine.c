...
#define PE_STATE_ACTIONS(state)                                                \
  {                                                                            \
      .entry_action = state##_entry,                                           \
  }
...
void (*pe_get_exit_action(uint8_t pe_state))(struct pd_port *) {
...
static inline void print_state(struct pd_port *pd_port, uint8_t state) {
  ...
  PE_DBG("%s -> %s (%c%c%c)\n", vdm_evt ? "VDM" : "PD", pe_state_name[state],
         pd_port->power_role ? 'P' : 'C', pd_port->data_role ? 'D' : 'U',
...
static inline void pd_pe_state_change(struct pd_port *pd_port,
  ...
  if (pe_data->pe_state_flags & PE_STATE_FLAG_ENABLE_SENDER_RESPONSE_TIMER) {
...
static int pd_handle_event(struct pd_port *pd_port, struct pd_event *pd_event) {
  bool dpm_imme;
  struct pe_data *pe_data = &pd_port->pe_data;

  if (pd_curr_is_vdm_evt(pd_port)) {
    dpm_imme =
        pd_port->pe_data.vdm_state_flags & VDM_STATE_FLAG_DPM_ACK_IMMEDIATELY;
...
static inline uint8_t pd_try_get_active_event(struct tcpc_device *tcpc,
  ...
  if ((ret != 0) && (ret != DPM_READY_REACTION_BUSY)) {
    DPM_DBG("from_pe: %d, evt:%d, reaction:0x%x\n", from_pe, ret,
            pd_port->pe_data.dpm_reaction_id);
  }
...
static inline int pd_handle_dpm_immediately(struct pd_port *pd_port,
  ...
  if (pd_curr_is_vdm_evt(pd_port)) {
    ...
    dpm_immediately =
        pd_port->pe_data.pe_state_flags2 & PE_STATE_FLAG_DPM_ACK_IMMEDIATELY;
  }
