...
static const struct vdm_state_transition pe_vdm_state_reactions[] = {
    ...
    VDM_DFP_CMD_REACTION_PD30(CMD_DISCOVER_IDENT, PE_UFP_VDM_GET_IDENTITY,
                              PE_DFP_UFP_VDM_IDENTITY_REQUEST),

    VDM_DFP_CMD_REACTION_PD30(CMD_DISCOVER_SVID, PE_UFP_VDM_GET_SVIDS,
                              PE_DFP_VDM_SVIDS_REQUEST),

    VDM_DFP_CMD_REACTION_PD30(CMD_DISCOVER_MODES, PE_UFP_VDM_GET_MODES,
                              PE_DFP_VDM_MODES_REQUEST),

    VDM_DFP_CMD_REACTION(CMD_ENTER_MODE, PE_UFP_VDM_EVALUATE_MODE_ENTRY,
                         PE_DFP_VDM_MODE_ENTRY_REQUEST),

    VDM_DFP_CMD_REACTION(CMD_EXIT_MODE, PE_UFP_VDM_MODE_EXIT,
                         PE_DFP_VDM_MODE_EXIT_REQUEST),

    VDM_UFP_CMD_REACTION_PD30(CMD_ATTENTION, PE_DFP_VDM_ATTENTION_REQUEST,
                              PE_UFP_VDM_ATTENTION_REQUEST),

#ifdef CONFIG_USB_PD_ALT_MODE_DFP
    VDM_DFP_CMD_REACTION(CMD_DP_STATUS, PE_UFP_VDM_DP_STATUS_UPDATE,
                         PE_DFP_VDM_DP_STATUS_UPDATE_REQUEST),

    VDM_DFP_CMD_REACTION(CMD_DP_CONFIG, PE_UFP_VDM_DP_CONFIGURE,
                         PE_DFP_VDM_DP_CONFIGURATION_REQUEST),
#endif /* CONFIG_USB_PD_ALT_MODE_DFP */

#ifdef CONFIG_USB_PD_CUSTOM_VDM
    /* Only handle Timeout Case */
    VDM_DFP_CMD_REACTION_PD30(0, PE_UFP_UVDM_RECV, PE_DFP_UVDM_SEND),
#endif /* CONFIG_USB_PD_CUSTOM_VDM */

#ifdef CONFIG_USB_PD_SRC_STARTUP_DISCOVER_ID
    VDM_CABLE_CMD_REACTION(CMD_DISCOVER_IDENT, 0, PE_SRC_VDM_IDENTITY_REQUEST),
#endif /* CONFIG_USB_PD_SRC_STARTUP_DISCOVER_ID */

#ifdef CONFIG_USB_PD_DFP_READY_DISCOVER_ID
    VDM_CABLE_CMD_REACTION(CMD_DISCOVER_IDENT, 0,
                           PE_DFP_CBL_VDM_IDENTITY_REQUEST),
#endif /* CONFIG_USB_PD_DFP_READY_DISCOVER_ID */
};

static inline bool
pd_vdm_state_transit_rx(struct pd_port *pd_port,
...
static inline bool
pd_vdm_state_transit_tx(struct pd_port *pd_port, uint8_t vdm_cmdt,
                        const struct vdm_state_transition *state_transition) {
...
bool vdm_is_state_transition_available(
    struct pd_port *pd_port, bool recv,
    const struct vdm_state_transition *state_transition) {
  uint8_t shift = 1;
  uint8_t vdm_cmd_flags = state_transition->vdm_cmd_flags;

  if (pd_check_rev30(pd_port) && (vdm_cmd_flags & VDM_CMD_FLAG_PD30_DUPLEX)) {
    return true;
  }
...
static bool
pd_vdm_state_transit(struct pd_port *pd_port, uint8_t vdm_cmdt,
                     const struct vdm_state_transition *state_transition) {
  uint8_t curr_state;
  ...
  if (curr_state == PE_UFP_VDM_ATTENTION_REQUEST) {
    ...
    pd_notify_tcp_vdm_event_2nd_result(pd_port, TCP_DPM_RET_TIMEOUT);
    return false;
  }

  if (vdm_cmdt == CMDT_INIT) { /* Recv */
    if (!vdm_is_state_transition_available(pd_port, true, state_transition))
      return false;
  ...
  /* Send */
  if (!vdm_is_state_transition_available(pd_port, false, state_transition))
    return false;

  if (curr_state != state_transition->vdm_request_state)
    return false;

  return pd_vdm_state_transit_tx(pd_port, vdm_cmdt, state_transition);
}
...
static bool pd_make_vdm_state_transit(struct pd_port *pd_port,
                                      uint8_t transit_type) {
  int i;
  bool check_tx;
  uint8_t vdm_cmdt;
  const struct vdm_state_transition *state_transition;
  ...
  const struct vdm_state_transition *state_reaction = pe_vdm_state_reactions;
  ...
  for (i = 0; i < nr_transition; i++) {
    ...
    if (!pe_check_vdm_state_transit_valid(pd_port, transit_type, &vdm_cmdt,
                                          state_transition))
      continue;

    if (check_tx) {
      return vdm_is_state_transition_available(pd_port, false,
                                               state_transition);
    } else {
      return pd_vdm_state_transit(pd_port, vdm_cmdt, state_transition);
    }
  }
...
static inline bool pd_make_vdm_state_transit_sop(struct pd_port *pd_port) {
  return pd_make_vdm_state_transit(pd_port, VDM_STATE_TRANSIT_SOP_CMD);
}

static inline bool pd_make_vdm_state_transit_cable(struct pd_port *pd_port) {
  return pd_make_vdm_state_transit(pd_port, VDM_STATE_TRANSIT_SOP_PRIME_CMD);
}

static inline bool pd_make_vdm_state_transit_nak(struct pd_port *pd_port) {
  return pd_make_vdm_state_transit(pd_port, VDM_STATE_TRANSIT_NAK);
}
...
static inline bool pd_process_ctrl_msg(struct pd_port *pd_port,
  ...
  if (pe_data->vdm_state_flags & VDM_STATE_FLAG_ENABLE_VDM_RESPONSE_TIMER)
  ...
  case PE_UFP_VDM_ATTENTION_REQUEST:
    pd_notify_tcp_vdm_event_2nd_result(pd_port, TCP_DPM_RET_VDM_ACK);
    break;
#endif /* CONFIG_USB_PD_ALT_MODE */
  ...
  if (pe_data->vdm_state_flags & VDM_STATE_FLAG_BACK_READY_IF_RECV_GOOD_CRC) {
...
static inline bool pd_process_uvdm(struct pd_port *pd_port,
                                   struct pd_event *pd_event) {
  struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static const char *const pe_vdm_cmd_name[] = {
    "DiscoverID", "DiscoverSVID", "DiscoverMode",
    "EnterMode",  "ExitMode",     "Attention",
};
...
static inline void print_vdm_msg(struct pd_port *pd_port,
                                 struct pd_event *pd_event) {
#if (PE_EVT_INFO_VDM_DIS == 0)
  uint8_t cmd;
  uint8_t cmd_type;
...
static inline bool pd_process_sop_vdm(struct pd_port *pd_port,
                                      struct pd_event *pd_event) {
  struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static inline bool pd_process_sop_prime_vdm(struct pd_port *pd_port,
                                            struct pd_event *pd_event) {
...
static inline bool pd_process_dpm_msg(struct pd_port *pd_port,
  ...
  if (pd_port->pe_data.vdm_state_flags & VDM_STATE_FLAG_BACK_READY_IF_DPM_ACK) {
    pe_transit_ready_state(pd_port);
    return true;
  }
...
static inline bool pd_process_tcp_cable_event(struct pd_port *pd_port,
                                              struct pd_event *pd_event) {
  bool ret;
  int tcp_ret;
#ifdef CONFIG_PD_DISCOVER_CABLE_ID
...
static inline uint32_t tcpc_update_bits(uint32_t var, uint32_t update,
                                        uint32_t mask) {
...
static inline void pd_parse_tcp_dpm_evt_dp_status(struct pd_port *pd_port) {
  ...
  dp_data->local_status = tcpc_update_bits(dp_data->local_status,
...
static inline void pd_parse_tcp_dpm_evt_dp_config(struct pd_port *pd_port) {
  ...
  pd_port->mode_svid = USB_SID_DISPLAYPORT;
  dp_data->local_config = tcpc_update_bits(dp_data->local_config,
                                           dp_data_tcp->val, dp_data_tcp->mask);
}
...
static inline void pd_parse_tcp_dpm_evt_uvdm(struct pd_port *pd_port) {
  ...
  memcpy(pd_port->uvdm_data, vdm_data->vdos, sizeof(uint32_t) * vdm_data->cnt);

#ifdef CONFIG_USB_PD_SVDM
  if (pd_check_rev30(pd_port) && (pd_port->uvdm_data[0] & VDO_SVDM_TYPE))
    pd_port->uvdm_data[0] |= VDO_SVDM_VERS(SVDM_REV20);
#endif /* CONFIG_USB_PD_SVDM */
}
#endif /* CONFIG_USB_PD_CUSTOM_VDM */

static inline void pd_parse_tcp_dpm_evt_from_tcpm(struct pd_port *pd_port,
                                                  struct pd_event *pd_event) {
  switch (pd_event->msg) {
...
static inline bool pd_check_tcp_msg_valid(struct pd_port *pd_port,
                                          uint8_t new_state) {
  return pd_make_vdm_state_transit(pd_port, new_state);
}

static inline bool pd_process_tcp_msg(struct pd_port *pd_port,
                                      struct pd_event *pd_event) {
  uint8_t new_state;
  struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
  ...
  if (!pd_check_pe_state_ready(pd_port)) {
    pd_notify_tcp_event_1st_result(pd_port, TCP_DPM_RET_DENIED_NOT_READY);
  ...
  if (!pd_check_tcp_msg_valid(pd_port, new_state)) {
    pd_notify_tcp_event_1st_result(pd_port, TCP_DPM_RET_DENIED_WRONG_DATA_ROLE);
