...
#define DP_SELECT_CONNECTED(b)                                                 \
  ((b == DPSTS_DFP_D_CONNECTED) ? DPSTS_UFP_D_CONNECTED : DPSTS_DFP_D_CONNECTED)
...
static inline bool dp_update_dp_connected_one(struct pd_port *pd_port,
  ...
  if (dp_local_connected != DPSTS_BOTH_CONNECTED) {
    valid_connected =
        DP_CHECK_DP_CONNECTED_MATCH(dp_connected, dp_local_connected);
  } else {
...
static const char *const dp_dfp_u_state_name[] = {
    "dp_dfp_u_none",           "dp_dfp_u_discover_id",
    "dp_dfp_u_discover_svids", "dp_dfp_u_discover_modes",
    "dp_dfp_u_enter_mode",     "dp_dfp_u_status_update",
    "dp_dfp_u_wait_attention", "dp_dfp_u_configure",
    "dp_dfp_u_operation",
};
#endif /* DP_DBG_ENABLE */
...
bool dp_notify_pe_shutdown(struct pd_port *pd_port,
  ...
  if (svid_data->active_mode) {
    pd_send_vdm_exit_mode(pd_port, TCPC_TX_SOP, svid_data->svid,
                          svid_data->active_mode);
  }
...
static inline int eval_dp_match_score(uint32_t local_mode, uint32_t remote_mode,
...
static inline uint8_t dp_dfp_u_select_mode(struct pd_port *pd_port,
                                           ...
                                           struct svdm_svid_data *svid_data) {
  uint32_t dp_local_mode, dp_remote_mode, remote_dp_config = 0,
                                          local_dp_config = 0;
  ...
  for (i = 0; i < svid_data->remote_mode.mode_cnt; i++) {
    DP_INFO("Mode%d=0x%08x\n", i, svid_data->remote_mode.mode_vdo[i]);
  }
...
bool dp_dfp_u_notify_discover_modes(struct pd_port *pd_port,
  ...
  pd_port->mode_obj_pos = dp_dfp_u_select_mode(pd_port, dp_data, svid_data);
...
static inline bool dp_dfp_u_update_dp_connected(struct pd_port *pd_port) {
  ...
  case DPSTS_UFP_D_CONNECTED:
    valid_connected =
        dp_update_dp_connected_one(pd_port, dp_connected, dp_local_connected);
  ...
  case DPSTS_BOTH_CONNECTED:
    valid_connected =
        dp_update_dp_connected_both(pd_port, dp_local_connected, false);
...
bool dp_dfp_u_notify_dp_status_update(struct pd_port *pd_port, bool ack) {
  ...
  if (oper_mode) {
    tcpci_dp_notify_status_update_done(tcpc, dp_data->remote_status, ack);
  } else {
    valid_connected = dp_dfp_u_update_dp_connected(pd_port);
    if (valid_connected)
...
bool dp_dfp_u_notify_dp_configuration(struct pd_port *pd_port, bool ack) {
  ...
  tcpci_dp_notify_config_done(tcpc, dp_data->local_config,
                              dp_data->remote_config, ack);
...
static inline bool dp_ufp_u_update_dp_connected(struct pd_port *pd_port) {
  ...
  case DPSTS_BOTH_CONNECTED:
    valid_connected =
        dp_update_dp_connected_both(pd_port, dp_local_connected, true);
    break;
...
static inline int dp_ufp_u_request_dp_status(struct pd_port *pd_port) {
  ...
  case DP_UFP_U_OPERATION:
    ack = true;
    tcpci_dp_status_update(pd_port->tcpc, dp_data->remote_status);
    break;
  ...
  if (ack) {
    return pd_reply_svdm_request(pd_port, CMDT_RSP_ACK, 1,
...
static inline void dp_ufp_u_send_dp_attention(struct pd_port *pd_port) {
  ...
  case DP_UFP_U_OPERATION:
    ...
    pd_send_vdm_dp_attention(pd_port, TCPC_TX_SOP, svid_data->active_mode,
                             dp_data->local_status);
    break;

  default:
    VDM_STATE_DPM_INFORMED(pd_port);
    pd_notify_tcp_vdm_event_2nd_result(pd_port, TCP_DPM_RET_DENIED_NOT_READY);
    break;
  }
}
...
void pd_dpm_dfp_send_dp_status_update(struct pd_port *pd_port) {
  ...
  pd_send_vdm_dp_status(pd_port, TCPC_TX_SOP, pd_port->mode_obj_pos, 1,
                        &dp_data->local_status);
}
...
void pd_dpm_dfp_send_dp_configuration(struct pd_port *pd_port) {
  ...
  pd_send_vdm_dp_config(pd_port, TCPC_TX_SOP, pd_port->mode_obj_pos, 1,
                        &dp_data->remote_config);
}
...
bool dp_parse_svid_data(struct pd_port *pd_port,
  ...
  svid_data->local_mode.mode_vdo[0] = VDO_MODE_DP(
      ufp_d_pin_cap, dfp_d_pin_cap, usb2, receptacle, signal,
