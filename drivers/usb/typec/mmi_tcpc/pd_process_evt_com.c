...
static inline bool pd_evaluate_reject_dr_swap(struct pd_port *pd_port) {
  if (pd_port->dpm_caps & DPM_CAP_LOCAL_DR_DATA) {
    if (pd_port->power_role == PD_ROLE_DFP)
      return pd_port->dpm_caps & DPM_CAP_DR_SWAP_REJECT_AS_UFP;
...
static inline bool pd_evaluate_reject_pr_swap(struct pd_port *pd_port) {
  if (pd_port->dpm_caps & DPM_CAP_LOCAL_DR_POWER) {
    if (pd_port->power_role == PD_ROLE_SOURCE)
      return pd_port->dpm_caps & DPM_CAP_PR_SWAP_REJECT_AS_SNK;
...
static inline bool pd_process_ctrl_msg(struct pd_port *pd_port,
  ...
  case PD_CTRL_GET_COUNTRY_CODE:
    if (pd_port->country_nr) {
      ret = PE_MAKE_STATE_TRANSIT_SINGLE(pe_get_curr_ready_state(pd_port),
  ...
  case PD_CTRL_NOT_SUPPORTED:
    pd_cancel_dpm_reaction(pd_port);
    pd_notify_tcp_event_2nd_result(pd_port, TCP_DPM_RET_NOT_SUPPORT);
...
static inline bool pd_process_data_msg(struct pd_port *pd_port,
  ...
  case PD_DATA_BAT_STATUS:
    ret = PE_MAKE_STATE_TRANSIT_SINGLE(PE_GET_BATTERY_STATUS, ready_state);
    break;
  ...
  case PD_DATA_GET_COUNTRY_INFO:
    if (pd_port->country_nr) {
      ret = PE_MAKE_STATE_TRANSIT_SINGLE(ready_state, PE_GIVE_COUNTRY_INFO);
    }
    break;
#endif /* CONFIG_USB_PD_REV30_COUNTRY_CODE_LOCAL */
#endif /* CONFIG_USB_PD_REV30 */
...
static inline bool pd_process_ext_msg(struct pd_port *pd_port,
  ...
  if (pd_port->pe_state_curr == ready_state && pd_is_multi_chunk_msg(pd_port)) {
  ...
  case PD_EXT_GET_BAT_CAP:
    if (pd_port->bat_nr) {
      ret = PE_MAKE_STATE_TRANSIT_SINGLE(ready_state, PE_GIVE_BATTERY_CAP);
    }
    break;
  ...
  case PD_EXT_GET_BAT_STATUS:
    if (pd_port->bat_nr) {
      ret = PE_MAKE_STATE_TRANSIT_SINGLE(ready_state, PE_GIVE_BATTERY_STATUS);
    }
    break;
...
static inline bool pd_process_hw_msg_tx_failed(struct pd_port *pd_port,
  ...
  if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_BACK_READY_IF_TX_FAILED) {
    pd_notify_tcp_event_2nd_result(pd_port, TCP_DPM_RET_NO_RESPONSE);
    pe_transit_ready_state(pd_port);
    return true;
...
static inline bool pd_process_timer_msg(struct pd_port *pd_port,
  ...
  case PD_TIMER_SENDER_RESPONSE:
    ...
    if (pd_port->pe_data.pe_state_flags & PE_STATE_FLAG_HRESET_IF_SR_TIMEOUT) {
      pe_transit_hard_reset_state(pd_port);
      return true;
    }
    break;
  ...
  case PD_TIMER_BIST_CONT_MODE:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_BIST_CARRIER_MODE_2, ready_state))
      return true;
    break;
  ...
  case PD_TIMER_DFP_FLOW_DELAY:
    if (pd_port->pe_state_curr == ready_state &&
      ...
      dpm_reaction_set_clear(pd_port, DPM_REACTION_CAP_READY_ONCE,
  ...
  case PD_TIMER_DEFERRED_EVT:
    pd_notify_tcp_event_buf_reset(pd_port, TCP_DPM_RET_DROP_PE_BUSY);
    break;
#endif
