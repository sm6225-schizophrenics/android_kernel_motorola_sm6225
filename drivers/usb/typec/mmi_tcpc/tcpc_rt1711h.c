...
static int rt1711_write_device(void *client, u32 reg, int len,
                               const void *src) {
  struct i2c_client *i2c = client;
  int ret = 0, count = 5;
  u64 t1 = 0, t2 = 0;
...
static int rt1711_block_read(struct i2c_client *i2c, u8 reg, int len,
                             void *dst) {
  struct rt1711_chip *chip = i2c_get_clientdata(i2c);
  int ret = 0;
#ifdef CONFIG_RT_REGMAP
...
static int rt1711_regmap_init(struct rt1711_chip *chip) {
  ...
  chip->m_dev = rt_regmap_device_register(props, &rt1711_regmap_fops, chip->dev,
                                          chip->client, chip);
  if (!chip->m_dev) {
...
static int rt1711_init_fault_mask(struct tcpc_device *tcpc) {
  const uint8_t mask =
      TCPC_V10_REG_FAULT_STATUS_VCONN_OV | TCPC_V10_REG_FAULT_STATUS_VCONN_OC;
...
static int rt1711_init_alert(struct tcpc_device *tcpc) {
  ...
  ret =
      devm_request_threaded_irq(chip->dev, chip->irq, NULL, rt1711_intr_handler,
...
static int rt1711h_set_clock_gating(struct tcpc_device *tcpc, bool en) {
  ...
  int i = 0;
  uint8_t clk2 = RT1711H_REG_CLK_DIV_600K_EN | RT1711H_REG_CLK_DIV_300K_EN |
...
static inline int rt1711h_init_cc_params(struct tcpc_device *tcpc,
                                         uint8_t cc_res) {
  int rv = 0;
...
static int rt1711_tcpc_init(struct tcpc_device *tcpc, bool sw_reset) {
  ...
  rt1711_i2c_write8(tcpc, RT1711H_REG_I2CRST_CTRL,
  ...
  rt1711_i2c_write16(tcpc, RT1711H_REG_DRP_DUTY_CTRL, TCPC_NORMAL_RP_DUTY);
...
static int rt1711_get_cc(struct tcpc_device *tcpc, int *cc1, int *cc2) {
  ...
  *cc2 = TCPC_V10_REG_CC_STATUS_CC2(status);
  ...
  rt1711h_init_cc_params(tcpc, (uint8_t)tcpc->typec_polarity ? *cc2 : *cc1);
...
static int rt1711_set_low_power_mode(struct tcpc_device *tcpc, bool en,
                                     int pull) {
  int ret = 0;
  uint8_t data;
...
int rt1711h_set_watchdog(struct tcpc_device *tcpc, bool en) {
  ...
  return rt1711_i2c_write8(tcpc, RT1711H_REG_WATCHDOG_CTRL, data);
}
#endif /* CONFIG_TCPC_WATCHDOG_EN */

#ifdef CONFIG_TCPC_INTRST_EN
int rt1711h_set_intrst(struct tcpc_device *tcpc, bool en) {
  return rt1711_i2c_write8(tcpc, RT1711H_REG_INTRST_CTRL,
...
#ifdef CONFIG_USB_POWER_DELIVERY
static int rt1711_set_msg_header(struct tcpc_device *tcpc, uint8_t power_role,
                                 uint8_t data_role) {
...
static int rt1711_set_bist_carrier_mode(struct tcpc_device *tcpc,
                                        uint8_t pattern) {
...
static int rt1711_transmit(struct tcpc_device *tcpc,
  ...
  if (type < TCPC_TX_HARD_RESET) {
    ...
    rv = rt1711_block_write(chip->client, TCPC_V10_REG_TX_BYTE_CNT,
...
#ifdef CONFIG_PM_RUNTIME
        SET_RUNTIME_PM_OPS(rt1711_pm_suspend_runtime, rt1711_pm_resume_runtime,
                           NULL)
#endif /* #ifdef CONFIG_PM_RUNTIME */
};
