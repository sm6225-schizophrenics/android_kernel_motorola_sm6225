...

{
  ...
  case PD_CTRL_GET_SOURCE_CAP:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_READY, PE_SRC_SEND_CAPABILITIES))
      return true;
    break;
  ...
  case PD_CTRL_GET_SOURCE_CAP_EXT:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_READY, PE_SRC_GIVE_SOURCE_CAP_EXT))
      return true;
    break;
  ...
  case PD_CTRL_GET_STATUS:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_READY, PE_SRC_GIVE_SOURCE_STATUS))
      return true;
    break;
  ...
  case PD_CTRL_GET_PPS_STATUS:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_READY, PE_SRC_GIVE_PPS_STATUS))
      return true;
    break;
...

{
  ...
  case PD_DATA_SOURCE_CAP:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_DR_SRC_GET_SOURCE_CAP, PE_SRC_READY))
      return true;
    break;
#endif /* CONFIG_USB_PD_PR_SWAP */

  case PD_DATA_SINK_CAP:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_GET_SINK_CAP, PE_SRC_READY))
      return true;
    break;
  ...
  case PD_DATA_ALERT:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_READY, PE_SRC_SINK_ALERT_RECEIVED))
      return true;
    break;
...
#ifdef CONFIG_USB_PD_REV30_STATUS_LOCAL
  case PD_EXT_STATUS:
    if (PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_GET_SINK_STATUS, PE_SRC_READY))
      return true;
    break;
#endif /* CONFIG_USB_PD_REV30_STATUS_LOCAL */
...
static inline bool pd_process_dpm_msg(struct pd_port *pd_port,
  ...
  case PD_DPM_NAK:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_NEGOTIATE_CAPABILITIES,
...
static inline bool pd_process_pe_msg(struct pd_port *pd_port,
  ...
  case PD_PE_POWER_ROLE_AT_DEFAULT:
    return PE_MAKE_STATE_TRANSIT_SINGLE(PE_SRC_TRANSITION_TO_DEFAULT,
                                        PE_SRC_STARTUP);
 ...
 */
static inline bool
pd_process_timer_msg_source_start(struct pd_port *pd_port,
                                  struct pd_event *pd_event) {
