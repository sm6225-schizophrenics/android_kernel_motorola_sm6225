...
int dpm_check_supported_modes(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(svdm_svid_ops); i++) {
    if (i < (ARRAY_SIZE(svdm_svid_ops) - 1)) {
      if (svdm_svid_ops[i + 1].svid <= svdm_svid_ops[i].svid)
        is_disorder = true;
    }
...
static void pd_dpm_update_pdos_flags(struct pd_port *pd_port, uint32_t pdo) {
  uint16_t dpm_flags =
      pd_port->pe_data.dpm_flags & ~DPM_FLAGS_RESET_PARTNER_MASK;
...
int pd_dpm_send_sink_caps(struct pd_port *pd_port) {
  ...
  return pd_send_sop_data_msg(pd_port, PD_DATA_SINK_CAP, snk_cap->nr,
                              snk_cap->pdos);
}
...
int pd_dpm_send_source_caps(struct pd_port *pd_port) {
  ...
  for (i = 0; i < src_cap0->nr; i++) {
    src_cap1->pdos[i] = pd_reset_pdo_power(tcpc, src_cap0->pdos[i], cable_curr);
  }

  return pd_send_sop_data_msg(pd_port, PD_DATA_SOURCE_CAP, src_cap1->nr,
                              src_cap1->pdos);
}
...
void pd_dpm_inform_cable_id(struct pd_port *pd_port, bool src_startup) {
  ...
  if (payload) {
    memcpy(pd_port->pe_data.cable_vdos, payload, pd_get_msg_data_size(pd_port));
...
static bool dpm_build_request_info_apdo(struct pd_port *pd_port,
  ...
  return dpm_find_match_req_info(req_info, &sink_pdo_info, src_cap->nr,
                                 src_cap->pdos, -1, charging_policy);
}
#endif /* CONFIG_USB_PD_REV30_PPS_SINK */
...
static bool dpm_build_request_info_pdo(struct pd_port *pd_port,
  ...
  for (i = 0; i < snk_cap->nr; i++) {
    ...
    find_cap = dpm_find_match_req_info(req_info, &sink_pdo_info, src_cap->nr,
                                       src_cap->pdos, max_uw, charging_policy);
    ...
    if (find_cap) {
      ...
      DPM_DBG("Find SrcCap%d(%s):%d mw\n", req_info->pos,
              req_info->mismatch ? "Mismatch" : "Match", max_uw / 1000);
...
#ifdef CONFIG_USB_PD_REV30_PPS_SINK
  if ((charging_policy & DPM_CHARGING_POLICY_MASK) == DPM_CHARGING_POLICY_PPS) {
    return dpm_build_request_info_apdo(pd_port, req_info, src_cap,
                                       charging_policy);
  }
#endif /* CONFIG_USB_PD_REV30_PPS_SINK */

  return dpm_build_request_info_pdo(pd_port, req_info, src_cap,
                                    charging_policy);
}
...
static inline void dpm_update_request_not_bat(struct pd_port *pd_port,
  ...
  if (req_info->type == DPM_PDO_TYPE_APDO) {
    ...
    pd_port->last_rdo =
        RDO_APDO(req_info->pos, req_info->vmin, req_info->oper_ma, flags);
    return;
  }
#endif /* CONFIG_USB_PD_REV30_PPS_SINK */
  ...
  if (req_info->mismatch && (pd_port->cap_miss_match == 0x3)) {
    ...
    pd_port->last_rdo =
        RDO_FIXED(req_info->pos, req_info->oper_ma, req_info->oper_ma, flags);
  } else {
    pd_port->last_rdo =
        RDO_FIXED(req_info->pos, req_info->oper_ma, req_info->max_ma, flags);
  }
}
...
#ifdef CONFIG_USB_PD_REV30_PPS_SINK
  if (pd_port->request_apdo && (sink_pdo_info.vmin == pd_port->request_v) &&
...
void pd_dpm_snk_standby_power(struct pd_port *pd_port) {
  ...
  tcpci_sink_vbus(pd_port->tcpc, TCP_VBUS_CTRL_REQUEST, pd_port->request_v,
...
void pd_dpm_snk_transition_power(struct pd_port *pd_port) {
  tcpci_sink_vbus(pd_port->tcpc, TCP_VBUS_CTRL_REQUEST, pd_port->request_v_new,
                  pd_port->request_i_new);
  ...
  if (pd_port->request_apdo != pd_port->request_apdo_new) {
    ...
    pd_dpm_start_pps_request_thread(pd_port, pd_port->request_apdo_new);
  }
#endif /* CONFIG_USB_PD_REV30_PPS_SINK */
}
...
void pd_dpm_snk_hard_reset(struct pd_port *pd_port) {
  ...
  if (!ignore_hreset) {
    tcpci_sink_vbus(pd_port->tcpc, TCP_VBUS_CTRL_HRESET, mv, ma);
  }
...
static inline bool dpm_evaluate_request(struct pd_port *pd_port, uint32_t rdo,
                                        uint8_t rdo_pos) {
  uint32_t pdo;
  uint32_t sink_v;
...
void pd_dpm_src_transition_power(struct pd_port *pd_port) {
  ...
  if (pd_port->request_v > pd_port->request_v_new) {
    ...
    tcpci_enable_force_discharge(pd_port->tcpc, true, pd_port->request_v_new);
...
void pd_dpm_src_hard_reset(struct pd_port *pd_port) {
  tcpci_source_vbus(pd_port->tcpc, TCP_VBUS_CTRL_HRESET, TCPC_VBUS_SOURCE_0V,
                    0);
...
static inline bool dpm_ufp_update_svid_data_enter_mode(struct pd_port *pd_port,
                                                       uint16_t svid,
                                                       uint8_t ops) {
...
static inline bool dpm_ufp_update_svid_data_exit_mode(struct pd_port *pd_port,
                                                      uint16_t svid,
                                                      uint8_t ops) {
  uint8_t i;
...
void pd_dpm_ufp_request_enter_mode(struct pd_port *pd_port) {
  bool ack = dpm_ufp_update_svid_data_enter_mode(
      pd_port, dpm_vdm_get_svid(pd_port), dpm_vdm_get_ops(pd_port));

  dpm_vdm_reply_svdm_request(pd_port, ack);
}

void pd_dpm_ufp_request_exit_mode(struct pd_port *pd_port) {
  bool ack = dpm_ufp_update_svid_data_exit_mode(
      pd_port, dpm_vdm_get_svid(pd_port), dpm_vdm_get_ops(pd_port));
...
static inline void dpm_dfp_update_partner_id(struct pd_port *pd_port,
                                             uint32_t *payload) {
...
static inline void dpm_dfp_update_svid_data_exist(struct pd_port *pd_port,
                                                  uint16_t svid) {
  uint8_t k;
  struct svdm_svid_data *svid_data;
  struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static inline void dpm_dfp_update_svid_enter_mode(struct pd_port *pd_port,
  ...
  tcpci_enter_mode(tcpc, svid_data->svid, ops,
                   svid_data->remote_mode.mode_vdo[ops]);
}

static inline void dpm_dfp_update_svid_data_exit_mode(struct pd_port *pd_port,
                                                      uint16_t svid,
                                                      uint8_t ops) {
  uint8_t i;
  bool modal_operation;
...
void pd_dpm_dfp_inform_id(struct pd_port *pd_port, bool ack) {
  ...
  if (!payload) {
    dpm_reaction_clear(pd_port,
                       DPM_REACTION_DISCOVER_ID | DPM_REACTION_DISCOVER_SVID);
    return;
  }
...
static inline int dpm_dfp_consume_svids(struct pd_port *pd_port,
...
void pd_dpm_dfp_inform_modes(struct pd_port *pd_port, bool ack) {
  ...
  if (ack) {
    ...
    if (svid != expected_svid) {
      ...
      if (payload)
        dpm_dfp_update_svid_data_modes(pd_port, svid, payload, count);
    }
  }
...
void pd_dpm_dfp_inform_exit_mode(struct pd_port *pd_port) {
  ...
  dpm_dfp_update_svid_data_exit_mode(pd_port, expected_svid, expected_ops);
...
void pd_dpm_ufp_recv_uvdm(struct pd_port *pd_port) {
  ...
  memcpy(pd_port->uvdm_data, pd_get_msg_data_payload(pd_port),
...
void pd_dpm_dfp_inform_uvdm(struct pd_port *pd_port, bool ack) {
  ...
  pd_notify_tcp_vdm_event_2nd_result(pd_port, ack ? TCP_DPM_RET_VDM_ACK
                                                  : TCP_DPM_RET_VDM_NAK);
  VDM_STATE_DPM_INFORMED(pd_port);
}
...
void pd_dpm_dr_inform_sink_cap(struct pd_port *pd_port) {
  const uint32_t reaction_clear =
      DPM_REACTION_GET_SINK_CAP | DPM_REACTION_ATTEMPT_GET_FLAG;
  ...
  memcpy(snk_cap->pdos, pd_get_msg_data_payload(pd_port),
         pd_get_msg_data_size(pd_port));
...
void pd_dpm_dr_inform_source_cap(struct pd_port *pd_port) {
  uint32_t reaction_clear =
      DPM_REACTION_GET_SOURCE_CAP | DPM_REACTION_ATTEMPT_GET_FLAG;
  ...
  memcpy(src_cap->pdos, pd_get_msg_data_payload(pd_port),
         pd_get_msg_data_size(pd_port));
...
int dpm_check_good_power(struct pd_port *pd_port) {
  ...
  partner_ex = (pd_port->pe_data.dpm_flags & DPM_FLAGS_PARTNER_EXTPOWER) != 0;
...
void pd_dpm_prs_evaluate_swap(struct pd_port *pd_port, uint8_t role) {
  ...
  bool check_src =
      (pd_port->dpm_caps & DPM_CAP_PR_SWAP_CHECK_GP_SRC) ? true : false;
  bool check_snk =
      (pd_port->dpm_caps & DPM_CAP_PR_SWAP_CHECK_GP_SNK) ? true : false;

#ifdef CONFIG_USB_PD_SRC_REJECT_PR_SWAP_IF_GOOD_PW
  bool check_ext = (pd_port->dpm_caps & DPM_CAP_CHECK_EXT_POWER) ? true : false;
...
void pd_dpm_prs_turn_off_power_sink(struct pd_port *pd_port) {
  /* iSnkSwapStdby : 2.5mA */
  tcpci_sink_vbus(pd_port->tcpc, TCP_VBUS_CTRL_PR_SWAP, TCPC_VBUS_SINK_0V, 0);
}
...
void pd_dpm_prs_enable_power_source(struct pd_port *pd_port, bool en) {
  ...
  tcpci_source_vbus(pd_port->tcpc, TCP_VBUS_CTRL_PR_SWAP, vbus_level, -1);
...
void pd_dpm_inform_source_cap_ext(struct pd_port *pd_port) {
  ...
  if (dpm_check_ext_msg_event(pd_port, PD_EXT_SOURCE_CAP_EXT)) {
    ...
    dpm_reaction_clear(pd_port, DPM_REACTION_GET_SOURCE_CAP_EXT);
  }
}
...
int pd_dpm_send_source_cap_ext(struct pd_port *pd_port) {
  return pd_send_sop_ext_msg(pd_port, PD_EXT_SOURCE_CAP_EXT, PD_SCEDB_SIZE,
                             &pd_port->src_cap_ext);
}
...
int pd_dpm_send_battery_cap(struct pd_port *pd_port) {
  ...
  if (bat_info != NULL) {
    tcpci_notify_request_bat_info(tcpc, gbcdb->bat_cap_ref);
    bcdb = &bat_info->bat_cap;
  } else
    bcdb = &c_invalid_bcdb;

  return pd_send_sop_ext_msg(pd_port, PD_EXT_BAT_CAP, PD_BCDB_SIZE, bcdb);
}
...
static const uint32_t c_invalid_bsdo = BSDO(0xffff, BSDO_BAT_INFO_INVALID_REF);

int pd_dpm_send_battery_status(struct pd_port *pd_port) {
  const uint32_t *bsdo;
  struct pd_battery_info *bat_info;
  struct pd_get_battery_status *gbsdb = pd_get_msg_data_payload(pd_port);
  struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
  ...
  if (bat_info != NULL) {
    tcpci_notify_request_bat_info(tcpc, gbsdb->bat_status_ref);
  ...
  return pd_send_sop_data_msg(pd_port, PD_DATA_BAT_STATUS, PD_BSDO_SIZE, bsdo);
}
...
int pd_dpm_send_mfrs_info(struct pd_port *pd_port) {
  ...
  struct pd_get_manufacturer_info *gmidb = pd_get_msg_data_payload(pd_port);
  ...
  if (len < sizeof(midb->mfrs_string))
    len++;
  return pd_send_sop_ext_msg(pd_port, PD_EXT_MFR_INFO, PD_MIDB_MIN_SIZE + len,
                             midb);
}
...
int pd_dpm_send_country_codes(struct pd_port *pd_port) {
  ...
  return pd_send_sop_ext_msg(pd_port, PD_EXT_COUNTRY_CODES, 2 + ccdb.length * 2,
                             &ccdb);
}
...
int pd_dpm_send_country_info(struct pd_port *pd_port) {
  ...
  for (i = 0; i < pd_port->country_nr; i++) {
    if (ca[i].code == cc) {
      cidb_size += ca[i].len;
      memcpy(cidb.country_special_data, ca[i].data, ca[i].len);
      break;
    }
  }

  return pd_send_sop_ext_msg(pd_port, PD_EXT_COUNTRY_INFO, cidb_size, &cidb);
}
...
int pd_dpm_send_alert(struct pd_port *pd_port) {
  ...
  return pd_send_sop_data_msg(pd_port, PD_DATA_ALERT, PD_ADO_SIZE, &ado);
}
#endif /* CONFIG_USB_PD_REV30_ALERT_LOCAL */
...
int pd_dpm_send_status(struct pd_port *pd_port) {
  ...
  if (sdb.present_input & PD_STATUS_INPUT_INT_POWER_BAT) {
  ...
  return pd_send_sop_ext_msg(pd_port, PD_EXT_STATUS, PD_SDB_SIZE, &sdb);
}
...
void pd_dpm_inform_not_support(struct pd_port *pd_port) { /* TODO */ }
...
int pd_dpm_notify_pe_startup(struct pd_port *pd_port) {
  ...
  reactions |= DPM_REACTION_ATTEMPT_GET_FLAG | DPM_REACTION_REQUEST_PR_SWAP;
#else
  ...
  reactions |= DPM_REACTION_DISCOVER_ID | DPM_REACTION_DISCOVER_SVID;
#else
...
struct svdm_svid_data *dpm_get_svdm_svid_data(struct pd_port *pd_port,
                                              uint16_t svid) {
  uint8_t i;
  struct svdm_svid_data *svid_data;
...
int pd_dpm_core_init(struct pd_port *pd_port) {
  ...
  pd_port->svid_data = devm_kzalloc(
      &tcpc->dev, sizeof(struct svdm_svid_data) * svid_ops_nr, GFP_KERNEL);

  if (!pd_port->svid_data)
    return -ENOMEM;

  for (i = 0, j = 0; i < svid_ops_nr; i++) {
    ret = dpm_register_svdm_ops(pd_port, &pd_port->svid_data[j],
                                &svdm_svid_ops[i]);
