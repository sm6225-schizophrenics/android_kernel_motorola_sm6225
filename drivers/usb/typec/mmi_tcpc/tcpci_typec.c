...
static const char *const typec_wait_ps_name[] = {
    "Disable", "SNK_VSafe5V", "SRC_VSafe0V", "SRC_VSafe5V", "DBG_VSafe5V",
};
...
static inline int typec_enable_low_power_mode(struct tcpc_device *tcpc,
                                              uint8_t pull);
...
#define TYPEC_NEW_STATE(state) (typec_transfer_state(tcpc, state))
...
static const char *const typec_attach_name[] = {
    "NULL",       "SINK",       "SOURCE",   "AUDIO", "DEBUG",

    "DBGACC_SNK", "CUSTOM_SRC", "NORP_SRC",
};
...
static bool typec_try_enter_norp_src(struct tcpc_device *tcpc) {
  if (tcpci_check_vbus_valid_from_ic(tcpc) && typec_is_cc_no_res() &&
...
static inline void
typec_unattached_src_and_drp_entry(struct tcpc_device *tcpc) {
...
static inline void
typec_unattached_snk_and_drp_entry(struct tcpc_device *tcpc) {
...
static inline int typec_set_polarity(struct tcpc_device *tcpc, bool polarity) {
  tcpc->typec_polarity = polarity;
  return tcpci_set_polarity(tcpc, polarity);
}

static inline int typec_set_plug_orient(struct tcpc_device *tcpc, uint8_t pull,
                                        bool polarity) {
...
static inline void typec_legacy_enable_discharge(struct tcpc_device *tcpc,
                                                 bool en) {
...
static inline void typec_legacy_keep_default_rp(struct tcpc_device *tcpc,
                                                bool en) {
#ifdef CONFIG_TYPEC_CHECK_LEGACY_CABLE2
...
static inline bool typec_legacy_charge(struct tcpc_device *tcpc) {
  ...
  tcpci_source_vbus(tcpc, TCP_VBUS_CTRL_TYPEC, TCPC_VBUS_SOURCE_5V, 100);
...
#if TCPC_LEGACY_CABLE_SUSPECT_THD
  if (tcpc->typec_legacy_cable_suspect < TCPC_LEGACY_CABLE_SUSPECT_THD)
    check_legacy = false;
...
static inline void typec_legacy_handle_ps_change(struct tcpc_device *tcpc,
                                                 int vbus_level) {
#ifdef CONFIG_TYPEC_CHECK_LEGACY_CABLE2
...
static inline bool typec_audio_acc_sink_vbus(struct tcpc_device *tcpc,
                                             bool vbus_valid) {
  if (vbus_valid) {
    tcpci_report_power_control(tcpc, true);
    tcpci_sink_vbus(tcpc, TCP_VBUS_CTRL_TYPEC, TCPC_VBUS_SINK_5V, 500);
...
static inline void typec_attach_wait_entry(struct tcpc_device *tcpc) {
  ...
  if (tcpc->typec_attach_old || tcpc->typec_state == typec_attached_src) {
    tcpc_reset_typec_debounce_timer(tcpc);
...
static inline bool typec_check_false_ra_detach(struct tcpc_device *tcpc) {
  ...
  if (drp) {
    tcpci_set_cc(tcpc, TYPEC_CC_DRP);
    tcpci_alert_status_clear(tcpc, TCPC_REG_ALERT_EXT_RA_DETACH);
  }
...
int tcpc_typec_enter_lpm_again(struct tcpc_device *tcpc) {
  ...
  tcpci_set_low_power_mode(tcpc, true,
                           (tcpc->typec_role != TYPEC_ROLE_SRC) ? TYPEC_CC_DRP
                                                                : TYPEC_CC_RP);
...
#ifdef CONFIG_TYPEC_CHECK_LEGACY_CABLE
  if (tcpc->typec_legacy_cable && typec_legacy_handle_cc_change(tcpc)) {
    return true;
  }
#endif /* CONFIG_TYPEC_CHECK_LEGACY_CABLE */
...
static inline int
typec_handle_error_recovery_timeout(struct tcpc_device *tcpc) {
#ifdef CONFIG_USB_POWER_DELIVERY
  tcpc->pd_wait_pe_idle = false;
#endif /* CONFIG_USB_POWER_DELIVERY */
...
static inline void typec_handle_pd_wait_bc12(struct tcpc_device *tcpc) {
  ...
  ret = power_supply_get_property(tcpc->usb_psy, POWER_SUPPLY_PROP_REAL_TYPE,
                                  &val);
...
static inline int typec_attached_snk_vbus_absent(struct tcpc_device *tcpc) {
  ...
  if (tcpc->pd_during_direct_charge && !tcpci_check_vsafe0v(tcpc, true)) {
...
static const char *const typec_role_name[] = {
    "UNKNOWN", "SNK", "SRC", "DRP", "TrySRC", "TrySNK",
};
#endif /* TYPEC_INFO_ENABLE */
...
int tcpc_typec_change_role(struct tcpc_device *tcpc, uint8_t typec_role,
                           bool postpone) {
...
int tcpc_typec_handle_ctd(struct tcpc_device *tcpc,
  ...
  if (cable_type == TCPC_CABLE_TYPE_C2C) {
    ret = tcpci_get_cc(tcpc);
    if (ret >= 0) {
      if (typec_is_cc_no_res() && (tcpc->typec_state == typec_unattached_snk ||
                                   tcpc->typec_state == typec_unattached_src)) {
