...
static inline void pd_parse_pdata_bat_info(struct pd_port *pd_port,
  ...
  ret = snprintf(mfrs_info->mfrs_string, sizeof(mfrs_info->mfrs_string), "%s",
                 mstring);
...
static inline int pd_parse_pdata_bats(struct pd_port *pd_port,
  ...
  pd_port->fix_bat_info = devm_kzalloc(
      &pd_port->tcpc->dev, sizeof(struct pd_battery_info) * pd_port->bat_nr,
      GFP_KERNEL);
  ...
  for (i = 0; i < pd_port->bat_nr; i++) {
    pr_info(
        "%s fix_bat_info[%d].mfrs_info.vid = 0x%x, .mfrs_info.pid = 0x%x, "
        ".mfrs_string = %s, .bat_design_cap = %d\n",
        __func__, i, pd_port->fix_bat_info[i].mfrs_info.vid,
...
static inline int
pd_parse_pdata_country(struct pd_port *pd_port, struct device_node *sub,
  ...
  country_info->data = devm_kzalloc(
      &pd_port->tcpc->dev, sizeof(uint8_t) * country_info->len, GFP_KERNEL);
  ...
  temp_u32 = devm_kzalloc(&pd_port->tcpc->dev, sizeof(u32) * country_info->len,
                          GFP_KERNEL);

  ret = of_property_read_u32_array(sub, "pd,country_data", temp_u32,
                                   country_info->len);
  if (ret < 0)
...
static inline int pd_parse_pdata_countries(struct pd_port *pd_port,
  ...
  ret = of_property_read_u32(np, "pd,country_nr", (u32 *)&pd_port->country_nr);
  if (ret < 0) {
  ...
  pd_port->country_info = devm_kzalloc(
      &pd_port->tcpc->dev,
  ...
  for (i = 0; i < pd_port->country_nr; i++) {
    pr_info("%s country_info[%d].code = 0x%x, .len = %d\n", __func__, i,
            pd_port->country_info[i].code, pd_port->country_info[i].len);
...
static void pd_parse_log_src_cap_ext(struct pd_source_cap_ext *cap) {
  pr_info(
      "%s vid = 0x%x, pid = 0x%x, xid = 0x%x, fw_ver = 0x%x, hw_ver = 0x%0x\n",
      __func__, cap->vid, cap->pid, cap->xid, cap->fw_ver, cap->hw_ver);

  pr_info("%s voltage_regulation = %d, hold_time_ms = %d, compliance = 0x%x, "
          "touch_current = 0x%x, peak_current = %d %d %d\n",
          __func__, cap->voltage_regulation, cap->hold_time_ms, cap->compliance,
          cap->touch_current, cap->peak_current[0], cap->peak_current[1],
          cap->peak_current[2]);

  pr_info("%s touch_temp = %d, source_inputs = 0x%x, batteries = 0x%x, "
          "source_pdp = 0x%x\n",
          __func__, cap->touch_temp, cap->source_inputs, cap->batteries,
          cap->source_pdp);
}
...
#ifdef CONFIG_USB_PD_REV30_BAT_INFO
  pd_port->src_cap_ext.batteries = PD_SCEDB_BATTERIES(0, pd_port->bat_nr);
...
static int pd_parse_pdata(struct pd_port *pd_port) {
  ...
  if (np) {
    ...
    ret =
        of_property_read_u32(np, "pd,id-vdo-size", (u32 *)&pd_port->id_vdo_nr);
    if (ret < 0)
      pr_err("%s get id vdo size fail\n", __func__);
    ret = of_property_read_u32_array(
        np, "pd,id-vdo-data", (u32 *)pd_port->id_vdos, pd_port->id_vdo_nr);
    if (ret < 0)
      pr_err("%s get id vdo data fail\n", __func__);

    pr_info("%s id vdos data =\n", __func__);
    for (i = 0; i < pd_port->id_vdo_nr; i++)
      pr_info("%s %d: 0x%08x\n", __func__, i, pd_port->id_vdos[i]);
...
static inline bool
pd_is_cable_communication_available(struct pd_port *pd_port) {
  /*
...
int pd_enable_vbus_valid_detection(struct pd_port *pd_port, bool wait_valid) {
  ...
  PE_DBG("WaitVBUS=%d\n", wait_valid);
  pd_notify_pe_wait_vbus_once(pd_port, wait_valid ? PD_WAIT_VBUS_VALID_ONCE
...
static inline int pd_update_msg_header(struct pd_port *pd_port) {
  return tcpci_set_msg_header(pd_port->tcpc, pd_port->power_role,
                              pd_port->data_role);
}
...
int pd_send_sop_ctrl_msg(struct pd_port *pd_port, uint8_t msg) {
  return pd_send_message(pd_port, TCPC_TX_SOP, msg, false, 0, NULL);
}

int pd_send_sop_prime_ctrl_msg(struct pd_port *pd_port, uint8_t msg) {
  return pd_send_message(pd_port, TCPC_TX_SOP_PRIME, msg, false, 0, NULL);
}

int pd_send_sop_data_msg(struct pd_port *pd_port, uint8_t msg, uint8_t cnt,
                         const uint32_t *payload) {
  return pd_send_message(pd_port, TCPC_TX_SOP, msg, false, cnt, payload);
}
...
int pd_reply_wait_reject_msg_no_resp(struct pd_port *pd_port) {
  ...
  uint8_t msg =
      pd_event->msg_sec == PD_DPM_NAK_REJECT ? PD_CTRL_REJECT : PD_CTRL_WAIT;
...
int pd_send_svdm_request(struct pd_port *pd_port, uint8_t sop_type,
  ...
  ret =
      pd_send_data_msg(pd_port, sop_type, PD_DATA_VENDOR_DEF, 1 + cnt, payload);
...
int pd_reply_svdm_request(struct pd_port *pd_port, uint8_t reply, uint8_t cnt,
  ...
  return pd_send_sop_data_msg(pd_port, PD_DATA_VENDOR_DEF, 1 + cnt, payload);
}

#ifdef CONFIG_USB_PD_CUSTOM_VDM

int pd_send_custom_vdm(struct pd_port *pd_port, uint8_t sop_type) {
  return pd_send_data_msg(pd_port, sop_type, PD_DATA_VENDOR_DEF,
...
int pd_reply_custom_vdm(struct pd_port *pd_port, uint8_t sop_type, uint8_t cnt,
  ...
  return pd_send_data_msg(pd_port, sop_type, PD_DATA_VENDOR_DEF, cnt, payload);
}
