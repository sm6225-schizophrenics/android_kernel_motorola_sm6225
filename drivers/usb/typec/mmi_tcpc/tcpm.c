...
int tcpm_inquire_remote_cc(struct tcpc_device *tcpc, uint8_t *cc1, uint8_t *cc2,
                           bool from_ic) {
  int rv = 0;
...
int tcpm_typec_change_mode(struct tcpc_device *tcpc, uint8_t mode) {
  int ret = 0;
...
int tcpm_typec_change_role_postpone(struct tcpc_device *tcpc,
...
int tcpm_inquire_cable_inform(struct tcpc_device *tcpc, uint32_t *vdos) {
  ...
  mutex_lock(&pd_port->pd_lock);
  if (pd_port->pe_data.power_cable_present) {
    memcpy(vdos, pd_port->pe_data.cable_vdos, sizeof(uint32_t) * VDO_MAX_NR);
  } else
...
int tcpm_inquire_pd_partner_inform(struct tcpc_device *tcpc, uint32_t *vdos) {
  ...
  if (pd_port->pe_data.partner_id_present) {
    memcpy(vdos, pd_port->pe_data.partner_vdos, sizeof(uint32_t) * VDO_MAX_NR);
  } else
...
int tcpm_inquire_pd_partner_svids(struct tcpc_device *tcpc,
  ...
  if (svdm_list->cnt) {
    ...
    memcpy(list->svids, svdm_list->svids, sizeof(uint16_t) * svdm_list->cnt);
  } else
    ret = TCPM_ERROR_NO_PARTNER_INFORM;
  mutex_unlock(&pd_port->pd_lock);
...
int tcpm_inquire_pd_partner_modes(struct tcpc_device *tcpc, uint16_t svid,
...
int tcpm_inquire_pd_source_cap(struct tcpc_device *tcpc,
...
int tcpm_inquire_pd_sink_cap(struct tcpc_device *tcpc,
                             struct tcpm_power_cap *cap) {
  int ret;
  struct pd_port *pd_port = &tcpc->pd_port;
...
int tcpm_get_remote_power_cap(struct tcpc_device *tcpc,
  ...
  for (i = 0; i < remote_cap->nr; i++) {
    tcpm_extract_power_cap_val(pd_port->pe_data.remote_src_cap.pdos[i], &cap);
...
static inline int
__tcpm_inquire_select_source_cap(struct pd_port *pd_port,
...
int tcpm_inquire_select_source_cap(struct tcpc_device *tcpc,
                                   struct tcpm_power_cap_val *cap_val) {
  int ret;
  struct pd_port *pd_port = &tcpc->pd_port;
...
static int tcpm_put_tcp_dpm_event_cbk1(
...
int tcpm_dpm_pd_power_swap(struct tcpc_device *tcpc, uint8_t role,
                           const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
      .event_id = TCP_DPM_EVT_PR_SWAP_AS_SNK + role,
  };

  return tcpm_put_tcp_dpm_event_cbk1(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PR_SWAP_TOUT);
}
EXPORT_SYMBOL(tcpm_dpm_pd_power_swap);

int tcpm_dpm_pd_data_swap(struct tcpc_device *tcpc, uint8_t role,
                          const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_pd_vconn_swap(struct tcpc_device *tcpc, uint8_t role,
                           const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_pd_request(struct tcpc_device *tcpc, int mv, int ma,
                        const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_pd_request_ex(struct tcpc_device *tcpc, uint8_t pos, uint32_t max,
                           uint32_t oper,
                           const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
static int
tcpm_put_tcp_dpm_event_cbk2(struct tcpc_device *tcpc,
                            struct tcp_dpm_event *event,
                            const struct tcp_dpm_event_cb_data *cb_data,
...
int tcpm_dpm_pd_fast_swap(struct tcpc_device *tcpc, uint8_t role,
                          const struct tcp_dpm_event_cb_data *cb_data) {
  return TCPM_ERROR_NO_SUPPORT;
}
...
int tcpm_dpm_pd_get_status(struct tcpc_device *tcpc,
  ...
  return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PD_CMD_TOUT, (uint8_t *)status,
                                     PD_SDB_SIZE);
}
...
int tcpm_dpm_pd_get_pps_status_raw(struct tcpc_device *tcpc,
  ...
  return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PD_CMD_TOUT, (uint8_t *)pps_status,
                                     PD_PPSSDB_SIZE);
}
...
int tcpm_dpm_pd_get_pps_status(struct tcpc_device *tcpc,
  ...
  if (pps_status_raw.output_vol_raw == 0xffff)
    ...
    pps_status->output_mv = PD_PPS_GET_OUTPUT_MV(pps_status_raw.output_vol_raw);
...
int tcpm_dpm_pd_get_country_code(struct tcpc_device *tcpc,
  ...
  return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PD_CMD_TOUT, (uint8_t *)ccdb,
                                     PD_CCDB_MAX_SIZE);
}
...
int tcpm_dpm_pd_get_country_info(struct tcpc_device *tcpc, uint32_t ccdo,
  ...
  return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PD_CMD_TOUT, (uint8_t *)cidb,
                                     PD_CIDB_MAX_SIZE);
}
...
int tcpm_dpm_pd_get_bat_cap(struct tcpc_device *tcpc,
  ...
  return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PD_CMD_TOUT, (uint8_t *)bcdb,
                                     PD_BCDB_SIZE);
}
...
int tcpm_dpm_pd_get_bat_status(struct tcpc_device *tcpc,
  ...
  return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PD_CMD_TOUT, (uint8_t *)bsdo,
                                     sizeof(uint32_t) * PD_BSDO_SIZE);
}
...
int tcpm_dpm_pd_get_mfrs_info(struct tcpc_device *tcpc,
  ...
  return tcpm_put_tcp_dpm_event_cbk2(tcpc, &tcp_event, cb_data,
                                     TCPM_BK_PD_CMD_TOUT, (uint8_t *)midb,
                                     PD_MIDB_MAX_SIZE);
}

int tcpm_dpm_pd_alert(struct tcpc_device *tcpc, uint32_t ado,
                      const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_vdm_discover_mode(struct tcpc_device *tcpc, uint16_t svid,
                               const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_vdm_enter_mode(struct tcpc_device *tcpc, uint16_t svid,
                            uint8_t ops,
                            const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_vdm_exit_mode(struct tcpc_device *tcpc, uint16_t svid, uint8_t ops,
                           const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_vdm_attention(struct tcpc_device *tcpc, uint16_t svid, uint8_t ops,
                           const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_dp_attention(struct tcpc_device *tcpc, uint32_t dp_status,
                          uint32_t mask,
                          const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_dp_status_update(struct tcpc_device *tcpc, uint32_t dp_status,
                              uint32_t mask,
                              const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_dp_config(struct tcpc_device *tcpc, uint32_t dp_config,
                       uint32_t mask,
                       const struct tcp_dpm_event_cb_data *cb_data) {
  struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_send_custom_vdm(struct tcpc_device *tcpc,
  ...
  memcpy(&tcp_event.tcp_dpm_data.vdm_data, vdm_data,
         sizeof(struct tcp_dpm_custom_vdm_data));

  /* Check it later */
  ret = tcpm_put_tcp_dpm_event_cbk1(tcpc, &tcp_event, cb_data,
                                    TCPM_BK_PD_CMD_TOUT);

#ifdef CONFIG_USB_PD_TCPM_CB_2ND
  if ((ret == TCP_DPM_RET_SUCCESS) && (cb_data == NULL) &&
      vdm_data->wait_resp) {
    mutex_lock(&pd_port->pd_lock);
    vdm_data->cnt = tcpc->pd_port.uvdm_cnt;
    memcpy(vdm_data->vdos, tcpc->pd_port.uvdm_data,
...
#ifdef CONFIG_USB_PD_TCPM_CB_2ND
void tcpm_replace_curr_tcp_event(struct pd_port *pd_port,
                                 struct tcp_dpm_event *event) {
...
int tcpm_put_tcp_dpm_event(struct tcpc_device *tcpc,
                           struct tcp_dpm_event *event) {
  int ret;
  ...
  ret = tcpm_check_pd_attached(tcpc);
  if (ret != TCPM_SUCCESS && (imme && ret == TCPM_ERROR_UNATTACHED))
    return ret;
...
int tcpm_set_pd_charging_policy_default(struct tcpc_device *tcpc,
                                        uint8_t policy) {
  struct pd_port *pd_port = &tcpc->pd_port;
...
static void tcpm_alert_bat_changed(struct pd_port *pd_port,
  ...
  pd_port->pe_data.local_alert |= ADO(ADO_ALERT_BAT_CHANGED, fixed, swap);
#endif /* CONFIG_USB_PD_REV30_ALERT_LOCAL */
}

static inline uint16_t
tcpm_calc_battery_cap(struct pd_battery_info *battery_info, uint16_t soc) {
  uint16_t wh;
...
static int tcpm_update_bsdo(struct pd_port *pd_port,
                            enum pd_battery_reference ref, uint16_t soc,
                            uint16_t wh, uint8_t status) {
  uint8_t status_old;
...
int tcpm_update_bat_status_wh(struct tcpc_device *tcpc,
                              enum pd_battery_reference ref, uint8_t status,
                              uint16_t wh) {
  int ret;
...
static void tcpm_alert_status_changed(struct tcpc_device *tcpc,
                                      uint8_t ado_type) {
#ifdef CONFIG_USB_PD_REV30_ALERT_LOCAL
...
int tcpm_update_pd_status_temp(struct tcpc_device *tcpc,
                               enum pd_present_temperature_flag ptf_new,
                               uint8_t temperature) {
  ...
  if (ptf_now != ptf_new) {
    if ((ptf_new >= PD_PTF_WARNING) || (ptf_now >= PD_PTF_WARNING))
    ...
    pd_port->pd_status_temp_status = PD_STATUS_TEMP_SET_PTF(ptf_new);
  }
...
int tcpm_update_pd_status_input(struct tcpc_device *tcpc, uint8_t input_new,
                                uint8_t mask) {
  uint8_t input;
  uint8_t ado_type = 0;
  struct pd_port *pd_port = &tcpc->pd_port;
...
int tcpm_update_pd_status_bat_input(struct tcpc_device *tcpc, uint8_t bat_input,
                                    uint8_t bat_mask) {
  uint8_t input;
  uint8_t ado_type = 0;
...
static const char *const bk_event_ret_name[] = {
    "OK",        "Unknown", /* or not support by TCPM */
    "NotReady",  "LocalCap",   "PartnerCap", "SameRole", "InvalidReq",
    "RepeatReq", "WrongDR",    "PDRev",

    "Detach",    "SReset0",    "SReset1",    "HReset0",  "HReset1",
    "Recovery",  "BIST",       "PEBusy",

    "Wait",      "Reject",     "TOUT",       "NAK",      "NotSupport",
...
static inline void tcpm_dpm_bk_copy_data(struct pd_port *pd_port) {
  ...
  if (pd_port->tcpm_bk_cb_data != NULL) {
    memcpy(pd_port->tcpm_bk_cb_data, pd_get_msg_data_payload(pd_port), size);
  }
}
#endif /* CONFIG_USB_PD_TCPM_CB_2ND */
...
static inline int __tcpm_dpm_wait_bk_event(struct pd_port *pd_port,
                                           uint32_t tout_ms) {
...
int tcpm_dpm_wait_bk_event(struct pd_port *pd_port, uint32_t tout_ms) {
  ...
  if (ret < TCP_DPM_RET_NR && ret >= 0 && ret < ARRAY_SIZE(bk_event_ret_name))
