...
void pd_dbg_info_lock(void) { atomic_inc(&busy); }

void pd_dbg_info_unlock(void) { atomic_dec_if_positive(&busy); }
...
int pd_dbg_info(const char *fmt, ...) {
  ...
  if (r > 0)
    used += r;
  r = vsnprintf(pd_dbg_buffer[index].buf + used, PD_INFO_BUF_SIZE - used, fmt,
                args);
  if (r > 0)
    used += r;
