...
static ssize_t name_show(struct device *dev, struct device_attribute *attr,
  ...
  return snprintf(buf, 20, "%s\n",
                  adapter_dev->props.alias_name ? adapter_dev->props.alias_name
                                                : "anonymous");
}

static ssize_t pdo_info_show(struct device *dev, struct device_attribute *attr,
                             char *buf) {
  struct adapter_device *adapter_dev = to_adapter_device(dev);
  ...
  for (i = 0; i < cap.nr; i++) {
    ...
    size +=
        snprintf(buf + size, PAGE_SIZE - size, "%d: mV:%d,%d mA:%d type:%d\n",
...
static ssize_t apdo_info_show(struct device *dev, struct device_attribute *attr,
                              char *buf) {
  struct adapter_device *adapter_dev = to_adapter_device(dev);
  struct adapter_power_cap cap;
  ssize_t size = 0;
...
static ssize_t pdo_test_show(struct device *dev, struct device_attribute *attr,
                             char *buf) {
  struct adapter_device *adapter_dev = to_adapter_device(dev);
  int ret;
  ...
  ret = adapter_dev_set_cap(adapter_dev, MMI_PD_FIXED, 1, 9000, 3000);
...
static ssize_t apdo_test_show(struct device *dev, struct device_attribute *attr,
                              char *buf) {
  struct adapter_device *adapter_dev = to_adapter_device(dev);
  int ret;
  int vol = 5000;
  ...
  while (vol < 10000) {
    adapter_dev_set_cap(adapter_dev, MMI_PD_APDO, 3, vol, 3000);
    vol += 200;
    msleep(200);
  }
...
int adapter_dev_get_output(struct adapter_device *adapter_dev, int *mV,
                           int *mA) {
  if (adapter_dev != NULL && adapter_dev->ops != NULL &&
...
int adapter_dev_set_cap(struct adapter_device *adapter_dev,
                        enum adapter_cap_type type, int pdo_idx, int mV,
                        int mA) {
  if (adapter_dev != NULL && adapter_dev->ops != NULL &&
...
static struct attribute *adapter_class_attrs[] = {
    &dev_attr_name.attr,     &dev_attr_pdo_info.attr,  &dev_attr_apdo_info.attr,
    &dev_attr_pdo_test.attr, &dev_attr_apdo_test.attr, NULL,
};
...
static void __exit adapter_class_exit(void) { class_destroy(adapter_class); }
